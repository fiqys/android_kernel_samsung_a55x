...
#define NUMBER_OF_ACL_RX        (BSMHCP_USED_ENTRIES(bt_svc->mailbox_acl_rx_write, bt_svc->mailbox_acl_rx_read, BSMHCP_TRANSFER_RING_ACL_SIZE))
...
static uint8_t fw_log_circ_buf_push(uint8_t data)
{
	...
	int head = bt_svc->fw_log.circ_buf.head;

	if (CIRC_SPACE(head, bt_svc->fw_log.circ_buf.tail, bt_svc->fw_log.circ_buf_size) >= 1) {
		/* Insert one item into the buffer */
		bt_svc->fw_log.circ_buf.buf[head] = data;
		/* Finish writing descriptor before incrementing head. */
		bt_svc->fw_log.circ_buf.head = (head + 1) & (bt_svc->fw_log.circ_buf_size - 1);
		return 1;
	} else {
...
static uint8_t fw_log_circ_buf_pop(void)
{
	...
	int tail = bt_svc->fw_log.circ_buf.tail;

	if (CIRC_CNT(bt_svc->fw_log.circ_buf.head, tail, bt_svc->fw_log.circ_buf_size) >= 1) {
		/* Extract one item from the buffer */
		data = bt_svc->fw_log.circ_buf.buf[tail];
		/* Finish reading descriptor before incrementing tail. */
		bt_svc->fw_log.circ_buf.tail = (tail + 1) & (bt_svc->fw_log.circ_buf_size - 1);
	} else {
...
static size_t fw_log_circ_buf_remove(size_t len)
{
	...
	if (bt_svc->fw_log.circ_buf.buf == NULL || bt_svc->fw_log.fw_acl_read == NULL) {
		...
		cnt = CIRC_CNT(bt_svc->fw_log.circ_buf.head, bt_svc->fw_log.circ_buf.tail,
			       bt_svc->fw_log.circ_buf_size);
		len = min(cnt, len);

		/* Remove binary FW log until remained space of circ_buf is larger than len */
		while (consumed < len) {
			head_len = bt_svc->fw_log.circ_buf.buf[bt_svc->fw_log.circ_buf.tail];
...
static size_t scsc_bt_fw_log_circ_buf_read(uint8_t *data, size_t len)
{
	size_t cnt = 0;
	size_t i = 0;
	size_t head_len = 0;
...
static void scsc_bt_fw_log_update_filter(u8 trigger, u32 bt1_high, u32 bt1_low, u32 bt0_high, u32 bt0_low)
{
	if (bt_svc == NULL)
		return;
	...
	if (trigger == 1) {
		...
		SCSC_TAG_INFO(BT_COMMON,
			...
			bt_svc->fw_log.circ_buf_size, bt_svc->fw_log.circ_buf_threshold, bt1_high);
	/* Trigger OFF */
	} else if (trigger == 0) {
		if (bt_svc->fw_log.observers_registered) {
	...
	if (bt_svc->fw_log.btlog_enables0_low == 0 &&
		...
		if (bt_svc->service) {
			...
			bt_svc->bsmhcp_protocol->header.btlog_enables0_high = bt0_high;
			bt_svc->bsmhcp_protocol->header.btlog_enables1_low = bt1_low;
			bt_svc->bsmhcp_protocol->header.btlog_enables1_high = bt1_high;
...
{
	if (bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_write !=
	    bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_read ||
	    bt_svc->bsmhcp_protocol->header.mailbox_acl_rx_write !=
	    bt_svc->bsmhcp_protocol->header.mailbox_acl_rx_read ||
	    bt_svc->bsmhcp_protocol->header.mailbox_acl_free_write !=
	    bt_svc->bsmhcp_protocol->header.mailbox_acl_free_read ||
	    bt_svc->bsmhcp_protocol->header.mailbox_iq_report_write !=
	    bt_svc->bsmhcp_protocol->header.mailbox_iq_report_read ||
	    bt_svc->bsmhcp_protocol->header_2.mailbox_iso_rx_write !=
	    bt_svc->bsmhcp_protocol->header_2.mailbox_iso_rx_read ||
	    bt_svc->bsmhcp_protocol->header_2.mailbox_iso_free_write !=
	    bt_svc->bsmhcp_protocol->header_2.mailbox_iso_free_read)
		return true;
...
static void scsc_bt_shm_irq_handler(int irqbit, void *data)
{
	...
	if (scsc_read_data_available() ||
	    atomic_read(&bt_svc->error_count) != 0 ||
	    bt_svc->bsmhcp_protocol->header.panic_deathbed_confession) {
		bt_svc->interrupt_read_count++;
		if (!wake_lock_active(&bt_svc->read_wake_lock))
			wake_lock(&bt_svc->read_wake_lock);
		wake_up(&bt_svc->read_wait);
	}

	if (bt_svc->bsmhcp_protocol->header.mailbox_hci_cmd_write ==
	    bt_svc->bsmhcp_protocol->header.mailbox_hci_cmd_read &&
	    bt_svc->bsmhcp_protocol->header.mailbox_acl_tx_write ==
	    bt_svc->bsmhcp_protocol->header.mailbox_acl_tx_read &&
	    bt_svc->bsmhcp_protocol->header_2.mailbox_iso_tx_write ==
	    bt_svc->bsmhcp_protocol->header_2.mailbox_iso_tx_read) {
...
static bool scsc_read_bt_fw_data_available(void)
{
	if ((bt_svc->fw_log.observers_registered == true &&
	     (bt_svc->fw_log.fw_acl_read_state & FW_READ_ENABLE &&
	     bt_svc->fw_log.fw_acl_read_state & (FW_READ_HCI_EVENT | FW_READ_THRESHOLD)) &&
...
void scsc_bt_fw_log_state_handler(void)
{
	...
	/* Wake the reader operation */
	if (waitqueue_active(&bt_svc->read_wait) &&
...
static void scsc_bt_clear_paused_acl_rx(u16 conn_hdl)
{
	...
	u32 search = 0;
	u32 dst = BSMHCP_PREV_INDEX(bt_svc->mailbox_acl_rx_write, BSMHCP_TRANSFER_RING_ACL_SIZE);
	u32 stop = BSMHCP_PREV_INDEX(bt_svc->mailbox_acl_rx_read, BSMHCP_TRANSFER_RING_ACL_SIZE);
	...
	for (search = dst; search != stop;
		...
		if (conn_hdl != bt_svc->bsmhcp_protocol->acl_rx_transfer_ring[search].header.hci_connection_handle) {
			if (search != dst)
				memcpy(&bt_svc->bsmhcp_protocol->acl_rx_transfer_ring[dst],
				       &bt_svc->bsmhcp_protocol->acl_rx_transfer_ring[search],
				       sizeof(struct BSMHCP_TD_ACL_RX));

			dst = BSMHCP_PREV_INDEX(dst, BSMHCP_TRANSFER_RING_ACL_SIZE);
		} else
			BSMHCP_INCREASE_INDEX(bt_svc->mailbox_acl_rx_read, BSMHCP_TRANSFER_RING_ACL_SIZE);
	}
}
...
static int scsc_bt_shm_init_interrupt(void)
{
	...
	irq_ret = scsc_service_mifintrbit_register_tohost(
	    bt_svc->service, scsc_bt_shm_irq_handler, NULL, SCSC_MIFINTR_TARGET_WPAN, SCSC_BT_SHM_IRQ_TYPE);

	if (irq_ret < 0)
		return irq_ret;

	bt_svc->bsmhcp_protocol->header.bg_to_ap_int_src = irq_ret;
	h4_irq_mask |= 1 << irq_num++;

	/* From-host f/w IRQ allocations */
	irq_ret = scsc_service_mifintrbit_alloc_fromhost(
	    bt_svc->service, SCSC_MIFINTR_TARGET_WPAN);
	if (irq_ret < 0)
		return irq_ret;

	bt_svc->bsmhcp_protocol->header.ap_to_bg_int_src = irq_ret;
	h4_irq_mask |= 1 << irq_num++;
	...
	bt_svc->bsmhcp_protocol->header.ap_to_fg_int_src = irq_ret;
	h4_irq_mask |= 1 << irq_num++;

	SCSC_TAG_DEBUG(BT_COMMON, "Registered to-host IRQ bits %d, from-host IRQ bits %d:%d\n",
		       bt_svc->bsmhcp_protocol->header.bg_to_ap_int_src,
		       bt_svc->bsmhcp_protocol->header.ap_to_bg_int_src,
		       bt_svc->bsmhcp_protocol->header.ap_to_fg_int_src);
...
static void scsc_bt_big_handle_list_init(void)
{
	...
	for (i = 0; i < SCSC_BT_BIG_INFO_MAX; i++) {
		bt_svc->big_handle_list[i].big_handle = BSMHCP_INVALID_BIG_HANDLE;
		bt_svc->big_handle_list[i].num_of_connections = 0;
	}
}
...
static void scsc_bt_big_reset_connection_handles(u16 big_handle)
{
	...
	for (i = 0; i < SCSC_BT_CONNECTION_INFO_MAX; i++) {
		if (bt_svc->connection_handle_list[i].state != CONNECTION_NONE &&
		    bt_svc->connection_handle_list[i].big_handle == big_handle) {
			bt_svc->connection_handle_list[i].big_handle = BSMHCP_INVALID_BIG_HANDLE;
...
bool scsc_bt_shm_h4_avdtp_detect_write(uint32_t flags, uint16_t l2cap_cid, uint16_t hci_connection_handle)
{
	...
	spin_lock(&bt_svc->avdtp_detect.fw_write_lock);

	/* Store the read/write pointer on the stack since both are placed in unbuffered/uncached memory */
	tr_read = bt_svc->bsmhcp_protocol->header.mailbox_avdtp_read;
	tr_write = bt_svc->bsmhcp_protocol->header.mailbox_avdtp_write;

	td = &bt_svc->bsmhcp_protocol->avdtp_transfer_ring[tr_write];
	...
	if (tr_read >= BSMHCP_TRANSFER_RING_AVDTP_SIZE || tr_write >= BSMHCP_TRANSFER_RING_AVDTP_SIZE) {
		spin_unlock(&bt_svc->avdtp_detect.fw_write_lock);
		SCSC_TAG_ERR(BT_H4,
	...
	if (BSMHCP_HAS_ROOM(tr_write, tr_read, BSMHCP_TRANSFER_RING_AVDTP_SIZE)) {
		...
		if (!wake_lock_active(&bt_svc->write_wake_lock)) {
			...
			wake_lock(&bt_svc->write_wake_lock);
		}

		/* Increate the write pointer */
		BSMHCP_INCREASE_INDEX(tr_write, BSMHCP_TRANSFER_RING_AVDTP_SIZE);
		bt_svc->bsmhcp_protocol->header.mailbox_avdtp_write = tr_write;
		...
		scsc_service_force_panic(bt_svc->service);
		return false;
	}
	return true;
}
...
{
	/* Store the read/write pointer on the stack since both are placed in unbuffered/uncached memory */
	uint32_t tr_read = bt_svc->bsmhcp_protocol->header.mailbox_hci_cmd_read;
	uint32_t tr_write = bt_svc->bsmhcp_protocol->header.mailbox_hci_cmd_write;
	uint16_t op_code = *(uint16_t *)data;

	/* Temp vars */
	struct BSMHCP_TD_CONTROL *td = &bt_svc->bsmhcp_protocol->hci_cmd_transfer_ring[tr_write];
	...
	if (BSMHCP_HAS_ROOM(tr_write, tr_read, BSMHCP_TRANSFER_RING_CMD_SIZE)) {
		...
		bt_svc->bsmhcp_protocol->header.mailbox_hci_cmd_write = tr_write;
...
{
	/* Store the read/write pointer on the stack since both are placed in unbuffered/uncached memory */
	uint32_t tr_read = bt_svc->bsmhcp_protocol->header.mailbox_acl_tx_read;
	uint32_t tr_write = bt_svc->bsmhcp_protocol->header.mailbox_acl_tx_write;

	/* Temp vars */
	struct BSMHCP_TD_ACL_TX_DATA *td = &bt_svc->bsmhcp_protocol->acl_tx_data_transfer_ring[tr_write];
	...
	for (i = 0; i < BSMHCP_DATA_BUFFER_TX_ACL_SIZE; i++) {
		...
		if (++bt_svc->last_alloc == BSMHCP_DATA_BUFFER_TX_ACL_SIZE)
			bt_svc->last_alloc = 0;
		/* Claim a free slot */
		if (bt_svc->allocated[bt_svc->last_alloc] == 0) {
			bt_svc->allocated[bt_svc->last_alloc] = 1;
			acldata_buf_index = bt_svc->last_alloc;
	...
	if (BSMHCP_HAS_ROOM(tr_write, tr_read, BSMHCP_TRANSFER_RING_ACL_SIZE)) {
		...
		if ((td->header.flags & BSMHCP_ACL_BC_FLAG_BCAST_ACTIVE) ||
			...
			bt_svc->connection_handle_list[conn_hdl].state = CONNECTION_ACTIVE;
			bt_svc->connection_handle_list[conn_hdl].big_handle = BSMHCP_INVALID_BIG_HANDLE;
		}
		...
		if (pb_flag == BSMHCP_ACL_PB_FLAG_START_NONFLUSH || pb_flag == BSMHCP_ACL_PB_FLAG_START_FLUSH) {
			...
			if (l2cap_length == payload_len - L2CAP_HEADER_SIZE)
				 ...
				 */
				bt_svc->connection_handle_list[conn_hdl].remaining_length =
					(u16)(l2cap_length - payload_len + L2CAP_HEADER_SIZE);

				bt_svc->connection_handle_list[conn_hdl].l2cap_cid = HCI_L2CAP_GET_CID(data);
			} else {
			...
			u16 remaining_length = bt_svc->connection_handle_list[conn_hdl].remaining_length;

			td->header.l2cap_cid = bt_svc->connection_handle_list[conn_hdl].l2cap_cid;
			...
			SCSC_TAG_DEBUG(BT_TX,
				       ...
				       GET_AVAILABLE_TX_CREDITS(BSMHCP_DATA_BUFFER_TX_ACL_SIZE,
								bt_svc->allocated_count,
								bt_svc->freed_count),
				       bt_svc->connection_handle_list[conn_hdl].l2cap_cid,
				       remaining_length);
			...
			if (remaining_length == payload_len) {
				...
				bt_svc->connection_handle_list[conn_hdl].remaining_length = 0;
				 ...
				 * to complete the packet
				 */
				bt_svc->connection_handle_list[conn_hdl].remaining_length -= (u16)payload_len;
			else if (remaining_length < payload_len) {
				/* The packet is larger than the L2CAP payload length - protocol error */
				SCSC_TAG_ERR(BT_H4,
		...
		memcpy(&bt_svc->bsmhcp_protocol->acl_tx_buffer[acldata_buf_index][0],
		       &data[ACLDATA_HEADER_SIZE],
		       payload_len);

		/* Increate the write pointer */
		BSMHCP_INCREASE_INDEX(tr_write, BSMHCP_TRANSFER_RING_ACL_SIZE);
		bt_svc->bsmhcp_protocol->header.mailbox_acl_tx_write = tr_write;
...
static ssize_t scsc_bt_shm_h4_iso_write(const unsigned char *data, size_t count)
{
	 ...
	 */
	uint32_t tr_read = bt_svc->bsmhcp_protocol->header_2.mailbox_iso_tx_read;
	uint32_t tr_write = bt_svc->bsmhcp_protocol->header_2.mailbox_iso_tx_write;
	...
	for (i = 0; i < BSMHCP_DATA_BUFFER_TX_ISO_SIZE; i++) {
		/* Wrap the offset index around the buffer max */
		if (++bt_svc->iso_last_alloc == BSMHCP_DATA_BUFFER_TX_ISO_SIZE)
			bt_svc->iso_last_alloc = 0;
		/* Claim a free slot */
		if (bt_svc->iso_allocated[bt_svc->iso_last_alloc] == 0) {
			bt_svc->iso_allocated[bt_svc->iso_last_alloc] = 1;
			isodata_buf_index = bt_svc->iso_last_alloc;
	 ...
	 */
	td = &bt_svc->bsmhcp_protocol->iso_tx_data_transfer_ring[tr_write];
	...
	if (pb_flag == BSMHCP_ISO_PB_FLAG_FIRST || pb_flag == BSMHCP_ISO_PB_FLAG_COMPLETE) {
		...
		if (bt_svc->connection_handle_list[conn_hdl].remaining_length >= td->header.length) {
			/* This is another fragment of the packet. Save the remaining number of octets required
			 * to complete the packet.
			 */
			bt_svc->connection_handle_list[conn_hdl].remaining_length -= td->header.length;
		} else {
			SCSC_TAG_ERR(BT_H4,
	...
	SCSC_TAG_DEBUG(BT_H4,
		...
		conn_hdl, bt_svc->connection_handle_list[conn_hdl].remaining_length);
	...
	memcpy(&bt_svc->bsmhcp_protocol->iso_tx_buffer[td->header.buffer_index][0],
	       &data[ISODATA_HEADER_SIZE + data_load_header_len],
	       td->header.length);

	/* Increate the write pointer */
	BSMHCP_INCREASE_INDEX(tr_write, BSMHCP_TRANSFER_RING_ISO_TX_SIZE);
	bt_svc->bsmhcp_protocol->header_2.mailbox_iso_tx_write = tr_write;
...
static ssize_t scsc_iq_report_evt_read(char __user *buf, size_t len)
{
	...
	len = min(h4_iq_report_evt_len - bt_svc->read_offset, len);

	SCSC_TAG_DEBUG(BT_H4,
		       "SCSC_IQ_REPORT_EVT_READ: td(h4_iq_len=%u offset=%u)\n",
		       h4_iq_report_evt_len, bt_svc->read_offset);

	/* Copy the data to the user buffer */
	ret = copy_to_user(buf, &h4_iq_report_evt[bt_svc->read_offset], len);
	if (ret == 0) {
		/* All good - Update our consumed information */
		bt_svc->read_offset += len;
		consumed = len;

		SCSC_TAG_DEBUG(BT_H4,
			       "SCSC_IQ_REPORT_EVT_READ: (offset=%u consumed: %u)\n",
			       bt_svc->read_offset, consumed);

		/* Have all data been copied to the userspace buffer */
		if (bt_svc->read_offset == h4_iq_report_evt_len) {
...
{
	struct BSMHCP_TD_HCI_EVT *td = &bt_svc->bsmhcp_protocol->hci_evt_transfer_ring[bt_svc->read_index];
	...
	SCSC_TAG_DEBUG(BT_H4,
		       ...
		       len, bt_svc->read_offset);

	/* Is this the start of the copy operation */
	if (bt_svc->read_offset == 0) {
		...
		if (ret == 0) {
			...
			bt_svc->read_offset = sizeof(h4_hci_event_header);
		} else {
	...
	if (ret == 0 && (len - consumed)) {
		/* Calculate the amount of data that can be transferred */
		len = min((td->length - (bt_svc->read_offset - sizeof(h4_hci_event_header))), (len - consumed));

		/* Copy the data to the user buffer */
		ret = copy_to_user(&buf[consumed],
				   &td->data[bt_svc->read_offset - sizeof(h4_hci_event_header)],
				   len);
		...
		if (ret == 0) {
			...
			/* Have all data been copied to the userspace buffer */
			if (bt_svc->read_offset == (sizeof(h4_hci_event_header) + td->length)) {
				/* All good - read operation is completed */
				bt_svc->read_offset = 0;
				bt_svc->read_operation = BT_READ_OP_NONE;
			}
...
static ssize_t scsc_hci_evt_error_read(char __user *buf, size_t len)
{
	...
	if (bt_svc->system_error_info != NULL) {
		/* Calculate the amount of data that can be transferred */
		len = min(HCI_VSE_SYSTEM_ERROR_INFO_LEN - bt_svc->read_offset, len);
		/* Copy the data to the user buffer */
		ret = copy_to_user(buf, &bt_svc->system_error_info[bt_svc->read_offset], len);
	} else {
		/* Calculate the amount of data that can be transferred */
		len = min(sizeof(h4_hci_event_hardware_error) - bt_svc->read_offset, len);
		/* Copy the data to the user buffer */
		ret = copy_to_user(buf, &h4_hci_event_hardware_error[bt_svc->read_offset], len);
	}
	...
	if (ret == 0) {
		...
		/* Have all data been copied to the userspace buffer */
		if ((bt_svc->system_error_info != NULL) && (bt_svc->read_offset == HCI_VSE_SYSTEM_ERROR_INFO_LEN)) {
		...
		} else if (bt_svc->read_offset == sizeof(h4_hci_event_hardware_error)){
			/* All good - h4_hci_event_hardware_error read operation is completed */
			bt_svc->read_offset = 0;
			bt_svc->read_operation = BT_READ_OP_NONE;
#ifdef CONFIG_SCSC_LOG_COLLECTION
			if (bt_svc->recovery_level == 0)
...
static ssize_t scsc_rx_read_data(char __user *buf, size_t len, uint16_t data_length, uint8_t *data)
{
	...
	if (bt_svc->read_offset < h4_read_data_header_len) {
		copy_len = min(h4_read_data_header_len - bt_svc->read_offset, len);

		/* Copy the header to the userspace buffer */
		ret = copy_to_user(buf, &h4_read_data_header[bt_svc->read_offset], copy_len);

		if (ret == 0) {
			/* All good - Update our consumed information */
			consumed = copy_len;
			bt_svc->read_offset += copy_len;
		} else {
			SCSC_TAG_WARNING(BT_H4, "copy_to_user returned: %zu\n", ret);
			ret = -EACCES;
		}
	...
	/* Can more data be put into the userspace buffer */
	if (ret == 0 && bt_svc->read_offset >= h4_read_data_header_len && (len - consumed)) {
		/* Calculate the amount of data that can be transferred */
		copy_len = min((data_length - (bt_svc->read_offset - h4_read_data_header_len)),
			       (len - consumed));

		/* Copy the data to the user buffer */
		ret = copy_to_user(&buf[consumed],
				   &data[bt_svc->read_offset - h4_read_data_header_len],
				   copy_len);
		...
		if (ret == 0) {
			...
			/* Have all data been copied to the userspace buffer */
			if (bt_svc->read_offset == (h4_read_data_header_len + data_length)) {
				/* All good - read operation is completed */
				bt_svc->read_offset = 0;
				bt_svc->read_operation = BT_READ_OP_NONE;
			}
	...
	SCSC_TAG_DEBUG(BT_H4,
		       ...
		       bt_svc->read_offset, consumed, ret, len, copy_len);
...
{
	struct BSMHCP_TD_ISO_RX *td = &bt_svc->bsmhcp_protocol->iso_rx_transfer_ring[bt_svc->read_index];
	...
	SCSC_TAG_DEBUG(BT_H4,
		...
		td->header.flag, len, bt_svc->read_offset);
...
{
	ssize_t ret;
	struct BSMHCP_TD_ACL_RX *td = &bt_svc->bsmhcp_protocol->acl_rx_transfer_ring[bt_svc->read_index];
	...
	if (bt_svc->read_operation == BT_READ_OP_NONE && td->header.pb_flag == HCI_ACL_PACKET_BOUNDARY_START_FLUSH)
...
static ssize_t scsc_read_credit(char __user *buf, size_t len)
{
	...
	/* Calculate the amount of data that can be transferred */
	len = min(h4_hci_event_ncp_header_len - bt_svc->read_offset, len);

	/* Copy the data to the user buffer */
	ret = copy_to_user(buf, &h4_hci_event_ncp_header[bt_svc->read_offset], len);
	if (ret == 0) {
		/* All good - Update our consumed information */
		bt_svc->read_offset += len;
		consumed = len;
		...
		/* Have all data been copied to the userspace buffer */
		if (bt_svc->read_offset == h4_hci_event_ncp_header_len) {
			/* All good - read operation is completed */
			bt_svc->read_offset = 0;
			bt_svc->read_operation = BT_READ_OP_NONE;
		}
...
static ssize_t scsc_hci_fw_log_read(char __user *buf, size_t len)
{
	...
	if (ret == 0 && bt_svc->read_offset >= h4_read_data_header_len && (len - consumed)) {
		/* Calculate the amount of data that can be transferred */
		copy_len = min((bt_svc->fw_log.fw_acl_read_len - (bt_svc->read_offset - h4_read_data_header_len)),
			       (len - consumed));

		/* Copy the data to the user buffer */
		ret = copy_to_user(&buf[consumed],
				   &bt_svc->fw_log.fw_acl_read[bt_svc->read_offset - h4_read_data_header_len],
				   copy_len);
		...
		if (ret == 0) {
			...
			/* Have all data been copied to the userspace buffer */
			if (bt_svc->read_offset == (h4_read_data_header_len + bt_svc->fw_log.fw_acl_read_len)) {
...
static ssize_t scsc_bt_shm_h4_read_continue(char __user *buf, size_t len)
{
	...
	if (bt_svc->read_operation == BT_READ_OP_FW_LOG_DATA) {
		...
		if (bt_svc->read_operation == BT_READ_OP_NONE) {
			 ...
			 * sync helper
			 */
			if (bt_svc->read_index == bt_svc->mailbox_hci_evt_read) {
				BSMHCP_INCREASE_INDEX(bt_svc->mailbox_hci_evt_read, BSMHCP_TRANSFER_RING_EVT_SIZE);
				if (bt_svc->fw_log.observers_registered == true &&
				    bt_svc->fw_log.fw_acl_read_state == FW_READ_ENABLE)
					srvman_wake_up_mxlog_thread_for_fwsnoop(bt_svc->service);
			}
		}
		/* Is a ACL data read operation ongoing */
	} else if (bt_svc->read_operation == BT_READ_OP_ACL_DATA) {
	...
	} else if (bt_svc->read_operation == BT_READ_OP_ISO_DATA) {
		SCSC_TAG_DEBUG(BT_H4, "BT_READ_OP_ISO_DATA\n");
		/* Copy data into the userspace buffer */
		ret = scsc_rx_read_iso_data(buf, len);
		if (bt_svc->read_operation == BT_READ_OP_NONE)
			/* All done - increase the read pointer and continue */
			BSMHCP_INCREASE_INDEX(bt_svc->mailbox_iso_rx_read, BSMHCP_TRANSFER_RING_ISO_RX_SIZE);
		/* Is a ACL credit update operation ongoing */
	} else if (bt_svc->read_operation == BT_READ_OP_CREDIT) {
		SCSC_TAG_DEBUG(BT_H4, "BT_READ_OP_CREDIT\n");

		/* Copy data into the userspace buffer */
		ret = scsc_read_credit(buf, len);
	} else if (bt_svc->read_operation == BT_READ_OP_IQ_REPORT) {
		...
		if (bt_svc->read_operation == BT_READ_OP_NONE)
			/* All done - increase the read pointer and continue */
			BSMHCP_INCREASE_INDEX(bt_svc->mailbox_iq_report_read, BSMHCP_TRANSFER_RING_IQ_REPORT_SIZE);
	} else if (bt_svc->read_operation == BT_READ_OP_HCI_EVT_ERROR) {
		...
		if (bt_svc->read_operation == BT_READ_OP_NONE)
			...
			bt_svc->read_operation = BT_READ_OP_STOP;
	} else if (bt_svc->read_operation == BT_READ_OP_STOP) {
		ret = -EIO;
	}
...
static ssize_t scsc_bt_shm_h4_read_iq_report_evt(char __user *buf, size_t len)
{
	...
	if (bt_svc->read_operation == BT_READ_OP_NONE &&
	    bt_svc->mailbox_iq_report_read != bt_svc->mailbox_iq_report_write) {
		struct BSMHCP_TD_IQ_REPORTING_EVT *td =
			&bt_svc->bsmhcp_protocol->iq_reporting_transfer_ring[bt_svc->mailbox_iq_report_read];
		u32 index = 0;
		u32 j = 0;
		u32 i;

		if (!bt_svc->iq_reports_enabled) {
			BSMHCP_INCREASE_INDEX(bt_svc->mailbox_iq_report_read, BSMHCP_TRANSFER_RING_IQ_REPORT_SIZE);
		} else {
			...
			bt_svc->read_operation = BT_READ_OP_IQ_REPORT;
			bt_svc->read_index = bt_svc->mailbox_iq_report_read;
			...
			if (ret > 0) {
				 ...
				 */
				if (bt_svc->read_operation == BT_READ_OP_NONE)
					BSMHCP_INCREASE_INDEX(bt_svc->mailbox_iq_report_read,
...
static ssize_t scsc_bt_shm_h4_read_hci_evt(char __user *buf, size_t len)
{
	...
	while (bt_svc->read_operation == BT_READ_OP_NONE &&
	       ret == 0 &&
	       !bt_svc->hci_event_paused &&
	       bt_svc->mailbox_hci_evt_read != bt_svc->mailbox_hci_evt_write) {
		struct BSMHCP_TD_HCI_EVT *td =
				&bt_svc->bsmhcp_protocol->hci_evt_transfer_ring[bt_svc->mailbox_hci_evt_read];
		...
		if (bt_svc->processed[bt_svc->mailbox_hci_evt_read]) {
			bt_svc->processed[bt_svc->mailbox_hci_evt_read] = false;
			BSMHCP_INCREASE_INDEX(bt_svc->mailbox_hci_evt_read, BSMHCP_TRANSFER_RING_EVT_SIZE);
			continue;
		}
		...
		if (event_type == BSMHCP_EVENT_TYPE_CONNECTED ||
			...
			if (event_type == BSMHCP_EVENT_TYPE_BIG_CONNECTED) {
				...
				if (bt_svc->connection_handle_list[td->hci_connection_handle].state == CONNECTION_NONE) {
					bt_svc->big_handle_list[big_handle].big_handle = big_handle;
					bt_svc->big_handle_list[big_handle].num_of_connections++;
				} else
			...
			if (update_connection_handle_list) {
				SCSC_TAG_DEBUG(BT_H4,
					     ...
					     td->hci_connection_handle,
					     bt_svc->connection_handle_list[td->hci_connection_handle].state,
					     big_handle);

				/* Update the connection table to mark it as active */
				bt_svc->connection_handle_list[td->hci_connection_handle].state = CONNECTION_ACTIVE;
				bt_svc->connection_handle_list[td->hci_connection_handle].remaining_length = 0;
				bt_svc->connection_handle_list[td->hci_connection_handle].big_handle = big_handle;
			...
			if (bt_svc->connection_handle_list[td->hci_connection_handle].state == CONNECTION_ACTIVE) {
			...
			bt_svc->connection_handle_list[td->hci_connection_handle].state = CONNECTION_NONE;

			/* Clear the ACL and ISO data processing to allow for the ACL and ISO
			 * disconnect event to be transferred to userspace
			 */
			if (bt_svc->data_paused &&
			    bt_svc->data_paused_conn_hdl == td->hci_connection_handle) {
				/* ACL and ISO data processing can now continue */
				bt_svc->data_paused = false;

				/* Clear all ACL data having data_paused_conn_hdl from acl_rx_transfer_ring */
				scsc_bt_clear_paused_acl_rx(bt_svc->data_paused_conn_hdl);

				/* Initialize the data_paused_conn_hdl for next data_paused */
				bt_svc->data_paused_conn_hdl = 0;
			}
		...
		/* A group disconnection event has been detected by the firmware */
			...
			if (bt_svc->big_handle_list[big_handle].num_of_connections > 0) {
				/**
			...
			bt_svc->big_handle_list[big_handle].big_handle = BSMHCP_INVALID_BIG_HANDLE;

		} else if (event_type == BSMHCP_EVENT_TYPE_IQ_REPORT_ENABLED) {
			bt_svc->iq_reports_enabled = true;
		} else if (event_type == BSMHCP_EVENT_TYPE_IQ_REPORT_DISABLED) {
			bt_svc->iq_reports_enabled = false;
		}

		/* Start a HCI event copy to userspace */
		bt_svc->read_operation = BT_READ_OP_HCI_EVT;
		bt_svc->read_index = bt_svc->mailbox_hci_evt_read;
...
static bool scsc_rx_read_iso_data_start_copy(char __user *buf, size_t len, ssize_t *ret, ssize_t *consumed)
{
	...
	struct BSMHCP_TD_ISO_RX *td =
		&bt_svc->bsmhcp_protocol->iso_rx_transfer_ring[bt_svc->mailbox_iso_rx_read];

	u8  ts_flag = td->header.flag & BSMHCP_ISO_TS_FLAG_MASK;

	bt_svc->read_operation = BT_READ_OP_ISO_DATA;
	bt_svc->read_index = bt_svc->mailbox_iso_rx_read;
...
static bool scsc_rx_read_acl_data_start_copy(char __user *buf, size_t len, ssize_t *ret, ssize_t *consumed)
{
	struct BSMHCP_TD_ACL_RX *td =
		&bt_svc->bsmhcp_protocol->acl_rx_transfer_ring[bt_svc->mailbox_acl_rx_read];

	bt_svc->read_operation = BT_READ_OP_ACL_DATA;
	bt_svc->read_index = bt_svc->mailbox_acl_rx_read;
...
static bool scsc_rx_read_connection_active(uint16_t hci_connection_handle, uint8_t disconnected)
{
	 ...
	 */
	if (bt_svc->connection_handle_list[hci_connection_handle].state == CONNECTION_ACTIVE) {
		/* Is this the final packet for the indicated ACL or ISO connection */
		if (disconnected) {
			u16 big_handle = bt_svc->connection_handle_list[hci_connection_handle].big_handle;

			SCSC_TAG_DEBUG(BT_H4,
				       "CONN disconnected (hci_connection_handle=0x%03x, state=%u, big_handle=0x%04x)\n",
				       hci_connection_handle,
				       bt_svc->connection_handle_list[hci_connection_handle].state,
				       big_handle);

			/* Update the connection table to mark it as disconnected */
			bt_svc->connection_handle_list[hci_connection_handle].state = CONNECTION_DISCONNECTED;

			if (big_handle != BSMHCP_INVALID_BIG_HANDLE &&
			    bt_svc->big_handle_list[big_handle].num_of_connections > 0) {
	 ...
	 */
	if (bt_svc->connection_handle_list[hci_connection_handle].state == CONNECTION_NONE) {
		SCSC_TAG_DEBUG(BT_H4,
		...
		bt_svc->data_paused_conn_hdl = hci_connection_handle;
...
static ssize_t scsc_bt_shm_h4_read_iso_data(char __user *buf, size_t len)
{
	...
	while (bt_svc->read_operation == BT_READ_OP_NONE &&
	       !bt_svc->data_paused &&
	       bt_svc->mailbox_iso_rx_read != bt_svc->mailbox_iso_rx_write) {
		struct BSMHCP_TD_ISO_RX *td =
			&bt_svc->bsmhcp_protocol->iso_rx_transfer_ring[bt_svc->mailbox_iso_rx_read];
...
static ssize_t scsc_bt_shm_h4_read_acl_data(char __user *buf, size_t len)
{
	...
	while (bt_svc->read_operation == BT_READ_OP_NONE &&
	       !bt_svc->data_paused &&
	       bt_svc->mailbox_acl_rx_read != bt_svc->mailbox_acl_rx_write) {
		struct BSMHCP_TD_ACL_RX *td =
			&bt_svc->bsmhcp_protocol->acl_rx_transfer_ring[bt_svc->mailbox_acl_rx_read];
...
static void scsc_bt_shm_h4_insert_credit_entries(uint16_t conn_hdl, u32 start_index, u16 tr_size, u32 *entries)
{
	...
	if (bt_svc->connection_handle_list[sanitized_conn_handle].state == CONNECTION_ACTIVE) {
		for (i = 0; i < tr_size; i++) {
...
static void scsc_bt_shm_h4_read_acl_credit(u32 *entries)
{
	...
	while (bt_svc->mailbox_acl_free_read != bt_svc->mailbox_acl_free_write) {
		struct BSMHCP_TD_ACL_TX_FREE *td =
			&bt_svc->bsmhcp_protocol->acl_tx_free_transfer_ring[bt_svc->mailbox_acl_free_read];

		scsc_bt_shm_h4_insert_credit_entries(td->hci_connection_handle,
				start_index, BSMHCP_TRANSFER_RING_ACL_SIZE, entries);

		BSMHCP_INCREASE_INDEX(bt_svc->mailbox_acl_free_read, BSMHCP_TRANSFER_RING_ACL_SIZE);
	}
}
...
static void scsc_bt_shm_h4_read_iso_credit(u32 *entries)
{
	...
	while (bt_svc->mailbox_iso_free_read != bt_svc->mailbox_iso_free_write) {
		struct BSMHCP_TD_ISO_TX_FREE *td =
			&bt_svc->bsmhcp_protocol->iso_tx_free_transfer_ring[bt_svc->mailbox_iso_free_read];

		scsc_bt_shm_h4_insert_credit_entries(td->hci_connection_handle,
				start_index, BSMHCP_TRANSFER_RING_ISO_TX_SIZE, entries);

		BSMHCP_INCREASE_INDEX(bt_svc->mailbox_iso_free_read, BSMHCP_TRANSFER_RING_ISO_TX_SIZE);
	}
}
...
static ssize_t scsc_bt_shm_h4_read_credit(char __user *buf, size_t len)
{
	...
	if (bt_svc->read_operation == BT_READ_OP_NONE) {
		...
		if (entries > 0) {
			...
			bt_svc->read_operation = BT_READ_OP_CREDIT;
			ret = scsc_read_credit(buf, len);
		}
	}
...
static ssize_t scsc_bt_circ_h4_read_fw_log_data(char __user *buf, size_t len)
{
	...
	while (bt_svc->read_operation == BT_READ_OP_NONE &&
		...
		bt_svc->fw_log.fw_acl_read_state |= FW_READ_BUSY;
		bt_svc->read_operation = BT_READ_OP_FW_LOG_DATA;
		...
		if (bt_svc->fw_log.fw_acl_read_state & FW_READ_VSC) {
			...
			bt_svc->fw_log.fw_acl_read[0] = 0x01;
			bt_svc->fw_log.fw_acl_read[1] = (u8)(HCI_VSC_SET_FW_LOG_BTSNOOP & 0x00ff);
			bt_svc->fw_log.fw_acl_read[2] = (u8)((HCI_VSC_SET_FW_LOG_BTSNOOP & 0xff00) >> 8);
			bt_svc->fw_log.fw_acl_read[3] = 0x00;
			bt_svc->fw_log.fw_acl_read_len = HCI_EVENT_COMMAND_COMPLETE_LEN;
		...
		*/
		read_len = scsc_bt_fw_log_circ_buf_read(bt_svc->fw_log.fw_acl_read + L2CAP_HEADER_SIZE,
						bt_svc->fw_log.circ_buf_threshold);
		...
		HCI_L2CAP_DATA_SET_LENGTH(bt_svc->fw_log.fw_acl_read, read_len);
		HCI_L2CAP_DATA_SET_CON_HDL(bt_svc->fw_log.fw_acl_read, 0x0000);
		bt_svc->fw_log.fw_acl_read_len = read_len + L2CAP_HEADER_SIZE;
...
ssize_t scsc_bt_shm_h4_queue_sync_helper(char __user *buf, size_t len)
{
	...
	ssize_t ret = 0;
	bool found = false;
	u32 mailbox_hci_evt_read = bt_svc->mailbox_hci_evt_read;

	if (!bt_svc->hci_event_paused)
		return ret;

	conn_hdl_acl = bt_svc->bsmhcp_protocol->acl_rx_transfer_ring[
				bt_svc->mailbox_acl_rx_read].header.hci_connection_handle;

	conn_hdl_iso = bt_svc->bsmhcp_protocol->iso_rx_transfer_ring[
				bt_svc->mailbox_iso_rx_read].header.hci_connection_handle;
	 ...
	 */
	while (bt_svc->data_paused) {
		while (mailbox_hci_evt_read != bt_svc->mailbox_hci_evt_write) {
			struct BSMHCP_TD_HCI_EVT *td =
				&bt_svc->bsmhcp_protocol->hci_evt_transfer_ring[mailbox_hci_evt_read];
			...
			if ((event_type == BSMHCP_EVENT_TYPE_CONNECTED ||
				...
				bt_svc->connection_handle_list[td->hci_connection_handle].state = CONNECTION_ACTIVE;
				bt_svc->connection_handle_list[td->hci_connection_handle].remaining_length = 0;
				bt_svc->connection_handle_list[td->hci_connection_handle].big_handle =
				...
				bt_svc->processed[mailbox_hci_evt_read] = true;
				...
				/* Start a HCI event copy to userspace */
				bt_svc->read_operation = BT_READ_OP_HCI_EVT;
				bt_svc->read_index = mailbox_hci_evt_read;
				ret = scsc_hci_evt_read(buf, len);
				break;
			}
		...
		if (!found) {
			ret = wait_event_interruptible_timeout(
				bt_svc->read_wait,
				((mailbox_hci_evt_read != bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_write ||
				atomic_read(&bt_svc->error_count) != 0 ||
				bt_svc->bsmhcp_protocol->header.panic_deathbed_confession)), HZ);
...
{
	while (bt_svc->mailbox_acl_free_read_scan != bt_svc->mailbox_acl_free_write) {
		struct BSMHCP_TD_ACL_TX_FREE *td =
			&bt_svc->bsmhcp_protocol->acl_tx_free_transfer_ring[bt_svc->mailbox_acl_free_read_scan];

		/* Free the buffer in the allocation table */
		if (td->buffer_index < BSMHCP_DATA_BUFFER_TX_ACL_SIZE) {
			bt_svc->allocated[td->buffer_index] = 0;
			bt_svc->freed_count++;

			SCSC_TAG_DEBUG(BT_TX,
				       "ACL[CREDIT] (index=%u, buffer=%u, credits=%u)\n",
				       bt_svc->mailbox_acl_free_read_scan, td->buffer_index,
				       GET_AVAILABLE_TX_CREDITS(BSMHCP_DATA_BUFFER_TX_ACL_SIZE,
								bt_svc->allocated_count,
								bt_svc->freed_count));
		}

		BSMHCP_INCREASE_INDEX(bt_svc->mailbox_acl_free_read_scan, BSMHCP_TRANSFER_RING_ACL_SIZE);
	}
}

static void scsc_bt_free_iso_credits(void)
{
	while (bt_svc->mailbox_iso_free_read_scan != bt_svc->mailbox_iso_free_write) {
		struct BSMHCP_TD_ISO_TX_FREE *td =
		       &bt_svc->bsmhcp_protocol->iso_tx_free_transfer_ring[bt_svc->mailbox_iso_free_read_scan];

		/* Free the buffer in the ISO allocation table */
		if (td->buffer_index < BSMHCP_DATA_BUFFER_TX_ISO_SIZE) {
			bt_svc->iso_allocated[td->buffer_index] = 0;
			bt_svc->iso_freed_count++;

			SCSC_TAG_DEBUG(BT_TX,
				       "ISO[CREDIT] (index=%u, buffer=%u, credits=%u)\n",
				       bt_svc->mailbox_iso_free_read_scan, td->buffer_index,
				       GET_AVAILABLE_TX_CREDITS(BSMHCP_DATA_BUFFER_TX_ISO_SIZE,
								bt_svc->iso_allocated_count,
								bt_svc->iso_freed_count));
		}

		BSMHCP_INCREASE_INDEX(bt_svc->mailbox_iso_free_read_scan, BSMHCP_TRANSFER_RING_ISO_TX_SIZE);
	}
}
...
static void scsc_update_cached_write(void)
{
	...
	bt_svc->mailbox_hci_evt_write   = bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_write;
	bt_svc->mailbox_acl_rx_write    = bt_svc->bsmhcp_protocol->header.mailbox_acl_rx_write;
	bt_svc->mailbox_acl_free_write  = bt_svc->bsmhcp_protocol->header.mailbox_acl_free_write;
	bt_svc->mailbox_iq_report_write = bt_svc->bsmhcp_protocol->header.mailbox_iq_report_write;
	bt_svc->mailbox_iso_rx_write    = bt_svc->bsmhcp_protocol->header_2.mailbox_iso_rx_write;
	bt_svc->mailbox_iso_free_write  = bt_svc->bsmhcp_protocol->header_2.mailbox_iso_free_write;
}

static void scsc_update_read_indexes(void)
{
	/* Update the read indexes for all transfer rings */
	bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_read = bt_svc->mailbox_hci_evt_read;
	bt_svc->bsmhcp_protocol->header.mailbox_acl_rx_read = bt_svc->mailbox_acl_rx_read;
	bt_svc->bsmhcp_protocol->header.mailbox_acl_free_read = bt_svc->mailbox_acl_free_read;
	bt_svc->bsmhcp_protocol->header.mailbox_iq_report_read = bt_svc->mailbox_iq_report_read;
	bt_svc->bsmhcp_protocol->header_2.mailbox_iso_rx_read = bt_svc->mailbox_iso_rx_read;
	bt_svc->bsmhcp_protocol->header_2.mailbox_iso_free_read = bt_svc->mailbox_iso_free_read;
}
...
ssize_t scsc_bt_shm_h4_read(struct file *file, char __user *buf, size_t len, loff_t *offset)
{
	...
	if (bt_svc == NULL || !bt_svc->service_started)
		return -EIO;

	/* Only 1 reader is allowed */
	if (atomic_inc_return(&bt_svc->h4_readers) != 1) {
		atomic_dec(&bt_svc->h4_readers);
		return -EIO;
	}
	 ...
	 */
	if (atomic_read(&bt_svc->error_count) != 0 && bt_svc->read_operation == BT_READ_OP_NONE)
		bt_svc->read_operation = BT_READ_OP_HCI_EVT_ERROR;
	 ...
	 */
	while (0 == atomic_read(&bt_svc->error_count) && 0 == consumed) {
		...
		if ((bt_svc->mailbox_hci_evt_read == bt_svc->mailbox_hci_evt_write ||
		     bt_svc->hci_event_paused) &&
		    (bt_svc->mailbox_acl_rx_read == bt_svc->mailbox_acl_rx_write ||
		     bt_svc->data_paused) &&
		    bt_svc->mailbox_acl_free_read == bt_svc->mailbox_acl_free_write &&
		    bt_svc->mailbox_iq_report_read == bt_svc->mailbox_iq_report_write &&
		    bt_svc->mailbox_iso_free_read == bt_svc->mailbox_iso_free_write &&
		    (bt_svc->mailbox_iso_rx_read == bt_svc->mailbox_iso_rx_write || bt_svc->data_paused) &&
		    (bt_svc->fw_log.fw_acl_read_state <= FW_READ_ENABLE) &&
		    atomic_read(&bt_svc->error_count) == 0 &&
		    bt_svc->bsmhcp_protocol->header.panic_deathbed_confession == 0) {
			...
			ret = wait_event_interruptible(
				...
				((bt_svc->mailbox_hci_evt_read !=
				  bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_write &&
				  !bt_svc->hci_event_paused) ||
			...
			if (atomic_read(&bt_svc->error_count) != 0)
				break;
		...
		SCSC_TAG_DEBUG(BT_H4,
			...
			bt_svc->mailbox_iq_report_read,
			bt_svc->mailbox_iq_report_write);

		SCSC_TAG_DEBUG(BT_H4,
			"read_operation=%u, hci_event_paused=%u, data_paused=%u, fw_acl_read_state=%u\n",
			bt_svc->read_operation,
			bt_svc->hci_event_paused,
			bt_svc->data_paused,
			bt_svc->fw_log.fw_acl_read_state);
	...
	if (ret == 0 && consumed == 0) {
		...
		if (bt_svc->read_operation == BT_READ_OP_HCI_EVT_ERROR) {
			SCSC_TAG_ERR(BT_H4, "BT_READ_OP_HCI_EVT_ERROR\n");
	...
	if (bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_read !=
	    bt_svc->mailbox_hci_evt_read)
		gen_bg_int = true;

	if (bt_svc->bsmhcp_protocol->header.mailbox_acl_rx_read !=
	    bt_svc->mailbox_acl_rx_read ||
	    bt_svc->bsmhcp_protocol->header.mailbox_acl_free_read !=
	    bt_svc->mailbox_acl_free_read ||
	    bt_svc->bsmhcp_protocol->header_2.mailbox_iso_rx_read !=
	...
	if (bt_svc->bsmhcp_protocol->header.mailbox_iq_report_read !=
	    bt_svc->mailbox_iq_report_read)
		gen_fg_int = true;
	...
	if (gen_bg_int)
		scsc_service_mifintrbit_bit_set(bt_svc->service,
						bt_svc->bsmhcp_protocol->header.ap_to_bg_int_src, SCSC_MIFINTR_TARGET_WPAN);

	if (gen_fg_int)
		/* Trigger the interrupt in the mailbox */
		scsc_service_mifintrbit_bit_set(bt_svc->service,
						bt_svc->bsmhcp_protocol->header.ap_to_fg_int_src, SCSC_MIFINTR_TARGET_WPAN);

	if (!gen_bg_int && !gen_fg_int)
		if (wake_lock_active(&bt_svc->read_wake_lock))
			wake_unlock(&bt_svc->read_wake_lock);

	if (BT_READ_OP_STOP != bt_svc->read_operation && BT_READ_OP_FW_LOG_DATA != bt_svc->read_operation)
		SCSC_TAG_DEBUG(BT_H4, "hci_evt_read=%u, acl_rx_read=%u, acl_free_read=%u, read_operation=%u, consumed=%zd, ret=%zd\n",
			       bt_svc->mailbox_hci_evt_read, bt_svc->mailbox_acl_rx_read, bt_svc->mailbox_acl_free_read, bt_svc->read_operation, consumed, ret);
...
ssize_t scsc_bt_shm_h4_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
	...
	if (atomic_inc_return(&bt_svc->h4_writers) != 1) {
		atomic_dec(&bt_svc->h4_writers);
		return -EIO;
	}
	...
	if (atomic_read(&bt_svc->error_count) != 0) {
		ret = -EIO;
		if (bt_svc->recovery_waiting) {
			SCSC_TAG_DEBUG(BT_H4, "waiting for reset after recovery\n");
			ret = wait_event_interruptible_timeout(bt_svc->read_wait, bt_svc->recovery_waiting, HZ);
			if (ret == 0)
				ret = -EAGAIN;
		}
		atomic_dec(&bt_svc->h4_writers);
	...
	while (written != count && ret == 0) {
		length = min(count - written, sizeof(h4_write_buffer) - bt_svc->h4_write_offset);

		SCSC_TAG_DEBUG(BT_H4,
			       "count: %zu, length: %zu, h4_write_offset: %zu, written:%zu, size:%zu\n",
			       count, length, bt_svc->h4_write_offset, written, sizeof(h4_write_buffer));
		...
		ret = copy_from_user(&h4_write_buffer[bt_svc->h4_write_offset], &buf[written], length);
		if (ret == 0) {
			/* Is there enough data to include a HCI command header and is the type a HCI_COMMAND_PKT */
			if ((length + bt_svc->h4_write_offset) >= H4DMUX_HEADER_HCI &&
				...
				if ((hci_pkt_len + 1) <= (length + bt_svc->h4_write_offset)) {
					...
					if (HCI_COMMAND_GET_OPCODE(h4_write_buffer) == HCI_VSC_SET_FW_LOG_BTSNOOP &&
						...
						written += ((hci_pkt_len + 1) - bt_svc->h4_write_offset);
					...
					SCSC_TAG_WARNING(BT_H4,
							 ...
							 (hci_pkt_len + 1), (length + bt_svc->h4_write_offset));

					written += length;
					bt_svc->h4_write_offset += (u32)length;
				}
				/* Is there enough data to include a ACL data header and is the
				 * type a HCI_ACLDATA_PKT
				 */
			} else if ((length + bt_svc->h4_write_offset) >= H4DMUX_HEADER_ACL &&
				...
				if ((hci_pkt_len + H4DMUX_HEADER_ACL) <= (length + bt_svc->h4_write_offset)) {
					...
					SCSC_TAG_WARNING(BT_H4,
							 "missing data (need=%zu, got=%zu)\n",
							 (hci_pkt_len + 5), (length - bt_svc->h4_write_offset));
			 ...
			 */
			} else if ((length + bt_svc->h4_write_offset) >= H4DMUX_HEADER_ISO &&
				...
				if ((hci_pkt_len + H4DMUX_HEADER_ISO) <= (length + bt_svc->h4_write_offset)) {
	...
	SCSC_TAG_DEBUG(BT_H4,
		       ...
		       bt_svc->h4_write_offset, ret, written);
...
unsigned int scsc_bt_shm_h4_poll(struct file *file, poll_table *wait)
{
	...
	poll_wait(file, &bt_svc->read_wait, wait);
...
int scsc_bt_shm_init(void)
{
	...
	bt_svc->bsmhcp_protocol = scsc_mx_service_mif_addr_to_ptr(bt_svc->service, bt_svc->bsmhcp_ref);
	if (bt_svc->bsmhcp_protocol == NULL) {
		SCSC_TAG_ERR(BT_COMMON, "couldn't map kmem to shm_ref 0x%08x\n", (u32)bt_svc->bsmhcp_ref);
		return -ENOMEM;
	}

	/* Clear the protocol shared memory area */
	memset(bt_svc->bsmhcp_protocol, 0, sizeof(*bt_svc->bsmhcp_protocol));
	bt_svc->bsmhcp_protocol->header.magic_value = BSMHCP_PROTOCOL_MAGICVALUE;
	bt_svc->bsmhcp_protocol->header_2.magic_value = BSMHCP_PROTOCOL_V2_MAGICVALUE;
	bt_svc->bsmhcp_protocol->header.bsmhcp_version = BSMHCP_VERSION;
	bt_svc->mailbox_hci_evt_read = 0;
	bt_svc->mailbox_acl_rx_read = 0;
	bt_svc->mailbox_acl_free_read = 0;
	bt_svc->mailbox_acl_free_read_scan = 0;
	bt_svc->mailbox_iq_report_read = 0;
	bt_svc->mailbox_iso_rx_read = 0;
	bt_svc->mailbox_iso_free_read = 0;
	bt_svc->mailbox_iso_free_read_scan = 0;
	bt_svc->read_index = 0;
	bt_svc->allocated_count = 0;
	bt_svc->freed_count = 0;
	bt_svc->iq_reports_enabled = false;
	bt_svc->iso_allocated_count = 0;
	bt_svc->iso_freed_count = 0;
	h4_irq_mask = 0;
	...
	if (bt_svc->bsmhcp_protocol != NULL) {
		if (h4_irq_mask & 1 << irq_num++)
			scsc_service_mifintrbit_unregister_tohost(
			    bt_svc->service, bt_svc->bsmhcp_protocol->header.bg_to_ap_int_src, SCSC_MIFINTR_TARGET_WPAN);
		if (h4_irq_mask & 1 << irq_num++)
			scsc_service_mifintrbit_free_fromhost(
			    bt_svc->service, bt_svc->bsmhcp_protocol->header.ap_to_bg_int_src, SCSC_MIFINTR_TARGET_WPAN);
		if (h4_irq_mask & 1 << irq_num++)
			scsc_service_mifintrbit_free_fromhost(
			    bt_svc->service, bt_svc->bsmhcp_protocol->header.ap_to_fg_int_src, SCSC_MIFINTR_TARGET_WPAN);
	}
	SCSC_TAG_DEBUG(BT_COMMON, "exit\n");
}

/* Clear all control structures of shared memory interface.
 *
 * Note: scsc_bt_shm_exit() should be called before this to unregister IRQs
 */
void scsc_bt_shm_cleanup(void)
{
	...
	bt_svc->data_paused = false;
	bt_svc->data_paused_conn_hdl = 0;
	bt_svc->bsmhcp_protocol = NULL;

	memset(bt_svc->allocated, 0, sizeof(bt_svc->allocated));
	memset(bt_svc->iso_allocated, 0, sizeof(bt_svc->iso_allocated));
	memset(bt_svc->connection_handle_list, 0, sizeof(bt_svc->connection_handle_list));
...
void scsc_bt_fw_log_init(size_t len, size_t threshold)
{
	...
	bt_svc->fw_log.fw_acl_read = kmalloc(threshold + L2CAP_HEADER_SIZE, GFP_KERNEL);
