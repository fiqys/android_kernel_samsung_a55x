...
#if defined(CONFIG_SOC_S5E8845)
		if (host->pll_mode)
			writel(host->pll_mode, host->cmu_addr + PLL_OFFSET);
#endif
...
static void dw_mci_init_card(struct mmc_host *mmc, struct mmc_card *card)
{
	...
	if (card->type == MMC_TYPE_SD)
		writel(MANUAL_MODE, host->cmu_addr + PLL_OFFSET);
#endif
...
#if defined(CONFIG_SOC_S5E8845)
	host->cmu_addr = ioremap(S5E8845_CMU_TOP_BASE, 0x200);
#endif
...
#if defined(CONFIG_SOC_S5E8845)
	host->pll_mode = readl(host->cmu_addr + PLL_OFFSET);
#endif
