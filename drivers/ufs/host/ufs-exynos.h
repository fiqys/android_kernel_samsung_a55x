...
struct ufs_eom40_result {
	u32 v_phase;
	u32 v_ref;
	u32 v_err;
};

struct ufs_eom40_data {
	int v_max_step;
	int v_max_offset;
	int t_max_step;
	int t_max_offset;
	...
	struct ufs_eom40_result *result;
};
...
struct irq_inform {
	...
	int irq_table[MAX_IRQ * 2];
};

struct exynos_ia_group {
	u32 enable;
	u32 cfg;
};
...
#else
static inline void exynos_ufs_fmp_hba_start(struct ufs_hba *hba)
{
}
#endif
...
static inline struct ufs_hba *exynos_ufs_get_ufs_hba(void)
{
	return NULL;
}
static inline struct scsi_device *exynos_ufs_get_wlun_sdev(void)
{
	return NULL;
}
#endif
