...
static const struct s3c2410_wdt_variant drv_data_s5e9945_v1 = {
	.noncpu_int_en = S5E9945_CLUSTER0_NONCPU_INT_EN,
	.mask_bit = 2,
	.rst_stat_reg = EXYNOS9945_RST_STAT_REG_OFFSET,
	.rst_stat_bit = 24,	/* CLUSTER0 WDTRESET */
	.pmu_reset_func = s3c2410wdt_noncpu_int_en,
	.quirks = QUIRK_HAS_PMU_CONFIG | QUIRK_HAS_RST_STAT | QUIRK_HAS_WTCLRINT_REG |
		  QUIRK_HAS_DBGACK_BIT | QUIRK_HAS_WTMINCNT_REG,
...
static const struct s3c2410_wdt_variant drv_data_s5e9945_v2 = {
	.noncpu_int_en = S5E9945_CLUSTER2_NONCPU_INT_EN,
	.mask_bit = 3,
	.rst_stat_reg = EXYNOS9945_RST_STAT_REG_OFFSET,
	.rst_stat_bit = 25,	/* CLUSTER2 WDTRESET */
	.pmu_reset_func = s3c2410wdt_noncpu_int_en,
...
static const struct s3c2410_wdt_variant drv_data_s5e8845_v1 = {
       .noncpu_int_en = S5E8845_CLUSTER0_NONCPU_INT_EN,
       .mask_bit = 2,
       .rst_stat_reg = EXYNOS8845_RST_STAT_REG_OFFSET,
       .rst_stat_bit = 24,     /* CLUSTER0 WDTRESET */
       .pmu_reset_func = s3c2410wdt_noncpu_int_en,
       .quirks = QUIRK_HAS_PMU_CONFIG | QUIRK_HAS_RST_STAT | QUIRK_HAS_WTCLRINT_REG |
                 QUIRK_HAS_DBGACK_BIT | QUIRK_HAS_WTMINCNT_REG,
...
static const struct s3c2410_wdt_variant drv_data_s5e8845_v2 = {
       .noncpu_int_en = S5E8845_CLUSTER1_NONCPU_INT_EN,
       .mask_bit = 3,
       .rst_stat_reg = EXYNOS8845_RST_STAT_REG_OFFSET,
...
{
	return clk_get_rate(wdt->rate_clock);
}
...
static ATOMIC_NOTIFIER_HEAD(s3c2410wdt_ping_notifier);

int s3c2410wdt_register_ping_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&s3c2410wdt_ping_notifier, nb);
}
EXPORT_SYMBOL_GPL(s3c2410wdt_register_ping_notifier);
...
static int s3c2410wdt_keepalive(struct watchdog_device *wdd)
{
	...
	if (wdt->cluster == LITTLE_CLUSTER)
		atomic_notifier_call_chain(&s3c2410wdt_ping_notifier, 0, wdt);
...
static struct exynos_s2i_ops s3c2410wdt_s2i_ops = {
	.suspend	= s3c2410wdt_syscore_suspend,
	.resume		= s3c2410wdt_syscore_resume,
};
...
static int s3c2410wdt_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(dev->of_node, "use_multistage_wdt", NULL)) {
		...
		if (cluster_index != LITTLE_CLUSTER) {
			dev_info(dev, "cl%u wdt is not allowed.\n", wdt->cluster);
			s3c_wdt[cluster_index] = NULL;
			devm_kfree(dev, wdt);
			return -EPERM;
		}
	}
	...
	/* register panic handler for watchdog reset */
		...
		register_exynos_s2i_ops(&s3c2410wdt_s2i_ops);
	}
