...
extern int xhci_vendor_init(struct xhci_hcd *xhci);
...
int xhci_exynos_bus_suspend(struct usb_hcd *hcd)
{
	...
	struct xhci_exynos_audio *xhci_audio = g_xhci_exynos_audio;
	int ret, main_hcd;
	...
	pr_info("%s, main_hcd %d\n", __func__, main_hcd);

	ret = xhci_bus_suspend(hcd);
#if 1
	if (!(xhci->xhc_state & XHCI_STATE_REMOVING))
		phy_set_mode_ext(xhci_audio->phy,
				PHY_MODE_BUS_SUSPEND, main_hcd);
#else
	xhci_exynos_wake_lock(main_hcd, 0);
#endif
...
#else
	xhci_exynos_wake_lock(main_hcd, 1);
#endif
...
void xhci_exynos_portsc_power_off(void __iomem * portsc, u32 on, u32 prt)
{
	...
	pr_info("power %s portsc, reg = 0x%x addr = %8llx\n", on ? "on" : "off", reg, (u64) portsc);
...
static int xhci_exynos_check_port(struct usb_device *dev, bool on)
{
	...
	usb_hub_for_each_child(hdev, port, udev) {
		...
		dev_dbg(ddev, "udev = 0x%8llx, state = %d\n", (u64)udev, udev->state);
...
int xhci_exynos_inform_dp_use(int use, int lane_cnt)
{
	...
	if (!g_xhci_exynos) {
		pr_info("[%s]: xhci_exynos is null. return!\n", __func__);
		return 0;
	}
...
EXPORT_SYMBOL_GPL(xhci_exynos_inform_dp_use);
...
static int xhci_exynos_probe(struct platform_device *pdev)
{
	...
	if (parent)
		...
		xhci_exynos_audio_init(parent, pdev);

	xhci_vendor_init(xhci);
	...
	xhci_exynos->usb_host_ready = true;
 ...
 */
int xhci_exynos_is_usb_ready(void)
{

	if (!g_xhci_exynos || (g_xhci_exynos->usb3_portsc == NULL)) {
		pr_info("%s: Need to wait xhci probe!!\n", __func__);
		return 0;
	}

	return g_xhci_exynos->usb_host_ready;
}
EXPORT_SYMBOL_GPL(xhci_exynos_is_usb_ready);
...
static int __maybe_unused xhci_exynos_resume(struct device *dev)
{
	...
	if (otg_is_connect() == 1) {	/* If it is OTG_CONNECT_ONLY */
		...
		if (g_hwinfo->mem_init == true) {
			ret = xhci_exynos_alloc_event_ring(xhci, GFP_KERNEL);
			if (ret)
				return ret;
			xhci_exynos_enable_event_ring(xhci);
		}
