...
static void __mfc_dec_fix_10bit_memtype(struct mfc_ctx *ctx, unsigned int format)
{
	...
	mfc_ctx_debug(2, "[10BIT] mem_type is %s\n", ctx->mem_type_10bit ? "P010" : "8+2");
}

static void __mfc_dec_set_internal_format(struct mfc_ctx *ctx)
{
	struct mfc_dev *dev = ctx->dev;
	int internal_fmt_comp = dev->pdata->internal_fmt_comp;

	if (dev->debugfs.feature_option & MFC_OPTION_INTER_SBWC_DISABLE) {
		internal_fmt_comp = 0;
		mfc_ctx_debug(2, "[PLUGIN] internal DPB SBWC disabled (option: %#x)\n",
				dev->debugfs.feature_option);
	}

	/*
	 * If the plugin_type is SW_MEMCPY,
	 * the format of the internal DPB and user DST buffer should be same.
	 */
	if (ctx->plugin_type == MFC_PLUGIN_SW_MEMCPY) {
		ctx->internal_fmt = ctx->dst_fmt;
		ctx->internal_raw_buf.num_planes = ctx->raw_buf.num_planes;
	} else {
		if (ctx->is_10bit) {
			if (internal_fmt_comp) {
				ctx->internal_fmt = __mfc_dec_find_format(ctx,
		...
		ctx->internal_raw_buf.num_planes = 2;
	}

	mfc_ctx_debug(2, "[PLUGIN] plugin internal pixelformat : %s\n", ctx->internal_fmt->name);
	mfc_dec_calc_dpb_size(ctx, &ctx->internal_raw_buf, ctx->internal_fmt);
}
...
static void __mfc_dec_change_format_sbwc_8bit(struct mfc_ctx *ctx)
{
	...
	case V4L2_PIX_FMT_NV12N_SBWC_256_10B:
		/* change to single plane format */
		ctx->dst_fmt = __mfc_dec_find_format(ctx, V4L2_PIX_FMT_NV12N_SBWC_256_8B);
		break;
	default:
		ctx->dst_fmt = __mfc_dec_find_format(ctx, V4L2_PIX_FMT_NV12N_SBWC_256_8B);
		break;
...
static void __mfc_dec_change_format_sbwc_10bit(struct mfc_ctx *ctx)
{
	...
	case V4L2_PIX_FMT_NV12N_SBWC_256_8B:
		/* change to single plane format */
		ctx->dst_fmt = __mfc_dec_find_format(ctx, V4L2_PIX_FMT_NV12N_SBWC_256_10B);
		break;
	case V4L2_PIX_FMT_NV12M_SBWC_8B:
		/* change to multi plane format */
		ctx->dst_fmt = __mfc_dec_find_format(ctx, V4L2_PIX_FMT_NV12M_SBWC_10B);
		break;
	case V4L2_PIX_FMT_NV21M_SBWC_8B:
		/* change to multi plane format */
		ctx->dst_fmt = __mfc_dec_find_format(ctx, V4L2_PIX_FMT_NV21M_SBWC_10B);
		break;
	default:
		ctx->dst_fmt = __mfc_dec_find_format(ctx, V4L2_PIX_FMT_NV12N_SBWC_256_10B);
		break;
			 ...
			 struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_MPEG_VIDEO_QOS_RATIO:
		...
		break;
	case V4L2_CID_MPEG_MFC_SET_DYNAMIC_DPB_MODE:
		/* is_dynamic_dpb is controled by driver */
