...
static void __mfc_dump_regs(struct mfc_core *core)
{
	...
	if (core->dev->debugfs.dbg_enable && core->dbg_info_buf.vaddr) {
...
void __mfc_dump_nal_q_buffer_info(struct mfc_core *core, int curr_ctx)
{
	...
	for (i = MFC_TRACE_NAL_QUEUE_PRINT - 1; i >= 0; i--) {
		...
		if (pDecOut->StartCode == NAL_Q_DECODER_MARKER)
			...
			dev_err(core->device, "[%d] Out ENC[ctx:%d] id %d frame [0]%#llx [1]%#llx stream %#llx type %d recon [0]%#llx [1]%#llx\n",
...
int __mfc_store_dump_state(struct mfc_dev *dev, char *buf)
{
	...
	for (i = 0; i < MFC_NUM_CONTEXTS; i++) {
		if (dev->ctx[i]) {
			if (dev->ctx[i]->type == MFCINST_DECODER) {
				codec = dev->ctx[i]->src_fmt;
				fmt = dev->ctx[i]->dst_fmt;
			} else {
				codec = dev->ctx[i]->dst_fmt;
				fmt = dev->ctx[i]->src_fmt;
			}
...
static int __mfc_store_dump_nal_q_buffer_info(struct mfc_core *core, int curr_ctx, char *buf)
{
	...
	for (i = MFC_TRACE_NAL_QUEUE_PRINT - 1; i >= 0; i--) {
		...
		if (pDecIn->StartCode == NAL_Q_DECODER_MARKER)
			ret = snprintf((buf + idx), 300,
					"[%d] In DEC[ctx:%d] id %d cpb %#llx used %#x[%d] dpb[0] %#llx [1] %#llx\n",
					cnt, pDecIn->InstanceId, pDecIn->CommandId,
					MFC_NALQ_DMA_READL(pDecIn->CpbBufferAddr),
					pDecIn->DynamicDpbFlagLower,
					ffs(pDecIn->DynamicDpbFlagLower) - 1,
			...
			ret = snprintf((buf + idx), 300,
					"[%d] In ENC[ctx:%d] id %d frame[0] %#llx [1] %#llx stream %#llx\n",
					cnt, pEncIn->InstanceId, pEncIn->CommandId,
					MFC_NALQ_DMA_READL(pEncIn->FrameAddr[0]),
					MFC_NALQ_DMA_READL(pEncIn->FrameAddr[1]),
					MFC_NALQ_DMA_READL(pEncIn->StreamBufferAddr));
	...
	for (i = MFC_TRACE_NAL_QUEUE_PRINT - 1; i >= 0; i--) {
		...
		if (pDecOut->StartCode == NAL_Q_DECODER_MARKER)
			ret = snprintf((buf + idx), 300,
					"[%d] Out DEC[ctx:%d] id %d disp stat %#x disp[0] %#llx [1] %#llx used %#x dec stat %#x dec[0] %#llx [1] %llx type %d\n",
					cnt, pDecOut->InstanceId, pDecOut->CommandId,
					pDecOut->DisplayStatus,
					MFC_NALQ_DMA_READL(pDecOut->DisplayAddr[0]),
					MFC_NALQ_DMA_READL(pDecOut->DisplayAddr[1]),
			...
			ret = snprintf((buf + idx), 300,
					"[%d] Out ENC[ctx:%d] id %d frame[0] %#llx [1] %#llx stream %#llx type %d recon[0] %#llx [1] %#llx\n",
					cnt, pEncIn->InstanceId, pEncOut->CommandId,
					MFC_NALQ_DMA_READL(pEncOut->EncodedFrameAddr[0]),
					MFC_NALQ_DMA_READL(pEncOut->EncodedFrameAddr[1]),
					MFC_NALQ_DMA_READL(pEncOut->StreamBufferAddr),
...
static int __mfc_store_dump_buffer_info(struct mfc_core *core, char *buf)
{
	...
	if (ctx->type == MFCINST_DECODER) {
		ret = snprintf((buf + idx), 300,
				"Decoder CPB:%#llx++%#x, scratch:%#llx++%#x, static(vp9):%#llx++%#x\n",
				MFC_CORE_DMA_READL(MFC_REG_D_CPB_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_D_CPB_BUFFER_SIZE),
				MFC_CORE_DMA_READL(MFC_REG_D_SCRATCH_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_D_SCRATCH_BUFFER_SIZE),
		...
		ret = snprintf((buf + idx), 300,
				"Encoder SRC %dplane, [0]:%#llx++%#x, [1]:%#llx++%#x, [2]:%#llx++%#x\n",
				ctx->src_fmt->num_planes,
				MFC_CORE_DMA_READL(MFC_REG_E_SOURCE_FIRST_ADDR),
				ctx->raw_buf.plane_size[0],
				MFC_CORE_DMA_READL(MFC_REG_E_SOURCE_SECOND_ADDR),
				...
				ctx->raw_buf.plane_size[2]);
		idx += ret;
		ret = snprintf((buf + idx), 300, "DST:%#llx++%#x, scratch:%#llx++%#x\n",
				MFC_CORE_DMA_READL(MFC_REG_E_STREAM_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_E_STREAM_BUFFER_SIZE),
				MFC_CORE_DMA_READL(MFC_REG_E_SCRATCH_BUFFER_ADDR),
				MFC_CORE_READL(MFC_REG_E_SCRATCH_BUFFER_SIZE));
...
static void __mfc_dump_info_broadcast(struct mfc_core *core)
{
	struct mfc_dev *dev = core->dev;
...
static void __mfc_dump_info_and_broadcast(struct mfc_core *core)
{
	/* If coredump is supported, recovery after coredump */
	if (__mfc_store_debug_info(core) > 0)
		return;

	__mfc_dump_info_broadcast(core);
}
...
static void __mfc_dump_info_and_stop_hw(struct mfc_core *core)
{
	...
	__mfc_dump_info_and_broadcast(core);
...
static void __mfc_dump_info_and_stop_hw_debug_mode(struct mfc_core *core)
{
	struct mfc_dev *dev = core->dev;
...
struct mfc_core_dump_ops mfc_core_dump_ops = {
	...
	.dump_and_broadcast		= __mfc_dump_info_and_broadcast,
	.dump_and_stop_always		= __mfc_dump_info_and_stop_hw,
	.dump_and_stop_debug_mode	= __mfc_dump_info_and_stop_hw_debug_mode,
};
