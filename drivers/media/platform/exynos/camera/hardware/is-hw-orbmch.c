...
static int __is_hw_orbmch_get_timeout_time(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	cur_freq = hardware->dvfs_freq[ORBMCH_DVFS_DOMAIN];
	process_time = orbmch_hw_g_process_time(cur_freq);
	hw_orbmch->timeout_time = process_time + ORBMCH_PROC_SW_MARGIN;

	msdbg_hw(2, "current dvfs freq(%d) orbmch timeout time is %dms\n", instance, hw_ip,
		 cur_freq, hw_orbmch->timeout_time);
			...
			bool flag, u32 f_type)
{
	...
	pablo_internal_subdev_probe(sd, instance, mem, "ORB_DESC");
