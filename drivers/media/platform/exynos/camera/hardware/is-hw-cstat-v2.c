...
#define IGNORE_COREX_DELAY 4 /* for 3 frames */
...
static const enum pablo_crta_buf_type data_to_buf[PABLO_CRTA_CSTAT_END_MAX] = {
	...
	PABLO_CRTA_BUF_MAX, /* invalid type */
};

static const enum pablo_crta_buf_type subdev_to_buf[IS_CSTAT_SUBDEV_NUM] = {
	PABLO_CRTA_BUF_CDAF,
	PABLO_CRTA_BUF_PRE_THUMB,
	PABLO_CRTA_BUF_AE_THUMB,
	PABLO_CRTA_BUF_AWB_THUMB,
...
static void is_hw_cstat_print_stall_status(struct is_hw_ip *hw_ip)
{
	...
	for (hw_id = DEV_HW_3AA0; hw_id <= DEV_HW_3AA3; hw_id++) {
		...
		cstat_hw_print_stall_status(base, hw_ip->ch);
	}
}
...
static int is_hw_cstat_dbg_set(const char *val, const struct kernel_param *kp)
{
	...
	case CSTAT_DBG_WDMA_OFF:
		...
		break;
	case CSTAT_DBG_REG_DUMP_ON_START:
		dbg_info.type = dbg_type;
		info("Set regdump on start");
		break;
	default:
...
static inline void is_hw_cstat_put_vc_buf(enum itf_vc_buf_data_type data_type,
					  struct is_hw_ip *hw_ip, u32 instance, u32 fcount)
{
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
	struct is_sensor_interface *sensor_itf = hw->sensor_itf[instance];
...
static inline void is_hw_cstat_frame_start(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (IS_ENABLED(CRTA_CALL)) {
		...
		ret = CALL_CRTA_BUFMGR_OPS(bufmgr, get_free_buf,
				hw_fcount, true, &pcsi_buf);
		if (ret)
...
static inline void is_hw_cstat_frame_end(struct is_hw_ip *hw_ip, u32 instance)
{
	int ret;
	struct is_hardware *hardware = hw_ip->hardware;
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
	...
	if (IS_ENABLED(CRTA_CALL)) {
		...
		if (test_and_clear_bit(HW_OVERFLOW_RECOVERY, &hw_ip->state)) {
			...
			for (buf_idx = PABLO_CRTA_CSTAT_END_PRE_THUMB; buf_idx < PABLO_CRTA_CSTAT_END_VPDAF; buf_idx++) {
				bufmgr = pablo_get_crta_bufmgr(data_to_buf[buf_idx],
						instance, hw_ip->ch);
				CALL_CRTA_BUFMGR_OPS(bufmgr, get_process_buf,
						start_fcount, &stat_buf[buf_idx]);
			}
		...
		if (frame && frame->shot && hw_ip->frame_type < LIB_FRAME_HDR_SHORT)
...
static int is_hw_cstat_frame_end_callback(void *caller, void *ctx, void *rsp_msg)
{
	...
	for (buf_idx = PABLO_CRTA_CSTAT_END_PRE_THUMB; buf_idx < PABLO_CRTA_CSTAT_END_CDAF_MW; buf_idx++) {
	...
	is_hw_cstat_put_vc_buf(VC_BUF_DATA_TYPE_SENSOR_STAT3, hw_ip, instance, fcount);
...
static void is_hw_cstat_set_config_lock_delay(struct is_hw_ip *hw_ip, u32 instance, u32 hw_fcount)
{
	struct is_framemgr *framemgr;
	struct is_frame *frame;

	mserr_hw("[F%d] config_lock isr is delayed", instance, hw_ip, hw_fcount);
	framemgr = hw_ip->framemgr;
	if (framemgr) {
...
static int is_hw_cstat_isr1(u32 hw_id, void *data)
{
	...
	if (!hw) {
		msinfo_hw("[INT1][F%d] Ignore ISR not in open state", instance, hw_ip, hw_fcount);
		return 0;
	}
	...
	while (int_status) {
		...
		case CSTAT_LINE:
			if (corex_end) {
				...
				if (!test_bit(HW_SUSPEND, &hw_ip->state))
					is_hw_cstat_set_config_lock_delay(hw_ip, instance, hw_fcount);
				is_hw_cstat_frame_end(hw_ip, instance);
				int_status &= ~fe;
				fe = 0;
			} else {
				goto skip_isr_event;
...
skip_isr_event:
	...
	if (cstat_hw_is_occurred(int1, CSTAT_ERR)) {
		...
		if (cstat_hw_is_occurred(int1, CSTAT_LIC_ERR)) {
			set_bit(HW_OVERFLOW_RECOVERY, &hw_ip->state);
			mswarn_hw("CSTAT COTF error", instance, hw_ip);
			is_debug_event_count(IS_EVENT_COTF_ERR, instance, hw_fcount);
		}
...
static inline void is_hw_cstat_cdaf(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = CALL_CRTA_BUFMGR_OPS(bufmgr, get_free_buf, hw_fcount, true, &cdaf_buf[PABLO_CRTA_CDAF_RAW]);
...
static int is_hw_cstat_cdaf_callback(void *caller, void *ctx, void *rsp_msg)
{
	...
	bufmgr = pablo_get_crta_bufmgr(PABLO_CRTA_BUF_CDAF, instance, hw_ip->ch);
	ret = CALL_CRTA_BUFMGR_OPS(bufmgr, get_process_buf, fcount,
					&buf_info);
	if (!ret)
		CALL_CRTA_BUFMGR_OPS(bufmgr, put_buf, &buf_info);
	...
	is_hw_cstat_put_vc_buf(VC_BUF_DATA_TYPE_SENSOR_STAT1, hw_ip, instance, fcount);
...
static int is_hw_cstat_isr2(u32 hw_id, void *data)
{
	...
	if (!hw) {
		mserr_hw("[INT2][F%d] Ignore ISR not in open state", instance, hw_ip, hw_fcount);
		return 0;
	}
...
static void is_hw_cstat_open_crta_bufmgr(struct is_hw_ip *hw_ip, u32 instance)
{
	u32 type;
	struct pablo_crta_bufmgr *bufmgr;

	for (type = PABLO_CRTA_BUF_BASE; type <= PABLO_CRTA_BUF_CDAF_MW; type++) {
		bufmgr = pablo_get_crta_bufmgr(type, instance, hw_ip->ch);
		CALL_CRTA_BUFMGR_OPS(bufmgr, open);
	}
}

static void is_hw_cstat_close_crta_bufmgr(struct is_hw_ip *hw_ip, u32 instance)
{
	u32 type;
	struct pablo_crta_bufmgr *bufmgr;
...
static int is_hw_cstat_stop(struct is_hw_ip *hw_ip, u32 instance)
{
	int ret = 0;
	long timetowait;
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
...
static void is_hw_cstat_clear(struct is_hw_ip *hw_ip, u32 instance)
{
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
...
static int is_hw_cstat_disable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	if (hw_ip->run_rsc_state) {
		if ((atomic_read(&hw_ip->instance) == instance) &&
			...
			ret = is_hw_cstat_stop(hw_ip, instance);

			spin_lock_irqsave(&hw->slock_shot, flag);
			is_hw_cstat_clear(hw_ip, instance);
...
#if defined(ENABLE_AWB_HAL)
	pindex = GET_CSTAT_CTX_PINDEX(PARAM_CSTAT_PRE, ctx);
	if (test_bit(pindex, pmap))
		memcpy(&p_set->dma_output_pre, &param->pre,
			sizeof(struct param_dma_output));

	pindex = GET_CSTAT_CTX_PINDEX(PARAM_CSTAT_AWB, ctx);
	if (test_bit(pindex, pmap))
		memcpy(&p_set->dma_output_awb, &param->awb,
			sizeof(struct param_dma_output));
#endif
}
...
#endif
#if defined(ENABLE_AWB_HAL)
	p_set->dma_output_pre.cmd = DMA_OUTPUT_COMMAND_DISABLE;
	p_set->dma_output_awb.cmd = DMA_OUTPUT_COMMAND_DISABLE;
#endif
}
...
#if defined(ENABLE_AWB_HAL)
	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_pre",
			hw_ip, frame, 0, 1,
			&p_set->dma_output_pre.cmd,
			p_set->dma_output_pre.plane,
			p_set->output_dva_pre,
			frame->dva_cstat_pre[ctx_idx]);

	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_awb",
			hw_ip, frame, 0, 1,
			&p_set->dma_output_awb.cmd,
			p_set->dma_output_awb.plane,
			p_set->output_dva_awb,
			frame->dva_cstat_awb[ctx_idx]);
#endif
}

static void is_hw_cstat_check_ignore_delay_corex(struct is_hw_ip *hw_ip, u32 instance, u32 fcount)
{
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
	...
	if (hw->sensor_itf[instance]) {
		...
		if (test_bit(HW_CONFIG, &hw_ip->state) && (vvalid_time != hw->vvalid_time)) {
			hw->ignore_corex_delay = IGNORE_COREX_DELAY;
			msinfo_hw("[F%d] detect seamless modechange & disable corex delay, vvalid_time(%d -> %d)",
				instance, hw_ip, fcount, hw->vvalid_time, vvalid_time);
		}
	}
		...
		u32 *vvalid_time, u32 *corex_delay_time, u32 *cinrow_time, u32 *cinrow_ratio)
{
	...
	*vvalid_time = hw->vvalid_time;
	...
	if (*vvalid_time && frame_time > req_min_frame_time) {
		...
		*corex_delay_time = 0;
		*cinrow_time = 0;
		*cinrow_ratio = (frame_rate < FAST_FPS) ? 0 : REQ_CINROW_RATIO_AT_FAST_FPS;
	}

	/* for seamless mode change */
	if (hw->ignore_corex_delay > 1) {
		...
		struct cstat_param_set *p_set)
{
	...
	for (sd_id = IS_CSTAT_PRE_THUMB; sd_id < IS_CSTAT_SUBDEV_NUM; sd_id++) {
		bufmgr = pablo_get_crta_bufmgr(subdev_to_buf[sd_id], instance, hw_ip->ch);
		ret = CALL_CRTA_BUFMGR_OPS(bufmgr, get_free_buf, p_set->fcount, true, &buf_info);

		if (ret || !buf_info.dva_cstat) {
			msdbg_hw(2, "Not enough frame(%d)", instance, hw_ip, sd_id);
			continue;
		}
		dma_out = cstat_hw_s_stat_cfg(sd_id, buf_info.dva_cstat, p_set);
		if (!dma_out)
			CALL_CRTA_BUFMGR_OPS(bufmgr, put_buf, &buf_info);
	}
}
...
static int is_hw_cstat_trigger(struct is_hw_ip *hw_ip, u32 instance, u32 fcount)
{
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
	...
	case VOTF:
		...
		is_hw_cstat_check_ignore_delay_corex(hw_ip, instance, fcount);
		...
		time_cfg.clk = hardware->dvfs_freq[IS_DVFS_CSIS] * MHZ;
...
static int _is_hw_cstat_shot(struct is_hw_ip *hw_ip, struct is_frame *frame, ulong hw_map)
{
	int ret;
	struct is_hw_cstat *hw;
	struct cstat_param_set *p_set;
...
static void pablo_hw_cstat_fixup_param(struct is_region *region)
{
	struct pablo_obte_sensor *obte_sensor;
	struct param_sensor_config *sensor_cfg;

	obte_sensor = pablo_obte_get_sensor();
	if (obte_sensor && obte_sensor->configured) {
		sensor_cfg = &region->parameter.sensor.config;
		sensor_cfg->calibrated_width = obte_sensor->width;
		sensor_cfg->calibrated_height = obte_sensor->height;
		sensor_cfg->sensor_binning_ratio_x = obte_sensor->binning_x;
		sensor_cfg->sensor_binning_ratio_y = obte_sensor->binning_y;
		sensor_cfg->freeform_sensor_crop_offset_x = obte_sensor->crop_x;
		sensor_cfg->freeform_sensor_crop_offset_y = obte_sensor->crop_y;
		sensor_cfg->width = obte_sensor->crop_width;
		sensor_cfg->height = obte_sensor->crop_height;
	}
}
		...
		IS_DECLARE_PMAP(pmap), u32 instance, ulong hw_map)
{
	...
	if (IS_ENABLED(CONFIG_PABLO_OBTE_SUPPORT))
		pablo_hw_cstat_fixup_param(region);
...
static void is_hw_cstat_suspend(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	is_hw_cstat_set_config(hw_ip, 0, instance, 0, &conf_default);

	set_bit(HW_SUSPEND, &hw_ip->state);
}

static int is_hw_cstat_reset(struct is_hw_ip *hw_ip, u32 instance)
{
	msinfo_hw("reset\n", instance, hw_ip);
...
/**
 * cstat_hw_apply_rms_crop() - Apply sensor ReMoSaicCrop region to user bayer crop region
 *
 * @prfi:	[in]		Pablo RTA frame information to refer sensor sizes
 * @param:	[in/out]	CSTAT HW control parameters
 * @size_cfg:	[out]		CSTAT HW size configuration info
 *
 * Sensor remosaic crop is bayer domain operation & it affects the sensor out FOV.
 * The bayer crop control, however, which is decided by user, doesn't care the modified FOV by sensor.
 * So, driver should move the user's bayer crop region into sensor out bayer region
 * including sensor crop operation.
 * For this, it moves the bayer crop region into sensor crop region which is
 * before applying sensor remosaic crop region on following sensor crop stage.
 *
 * Sensor crop stage:
 *	Stage 1. Crop for sensor margin.
 *		e.g. To get rid of OIS margin.
 *	Stage 2. Crop for aspect ratio.
 *		e.g. For video mode, 4:3 sensor out is cropped to make 16:9 aspect ratio.
 *		User bayer crop region is decided on this region.
 *	Stage 3. Crop for remosaic zoom.
 *
 * Context: Any context.
 */
static void cstat_hw_apply_rms_crop(struct pablo_rta_frame_info *prfi, struct cstat_param *param,
				    struct cstat_size_cfg *size_cfg)
{
	u32 ss_cal[2];
	u32 ss_crop[4];
	u32 ss_aspect_ratio_offset[2];
	u32 ss_rms_crop_ratio[2];
	u32 bcrop[4];
	u32 i;

	ss_cal[0] = prfi->sensor_calibration_size.width;
	ss_cal[1] = prfi->sensor_calibration_size.height;

	ss_crop[0] = prfi->sensor_crop.offset.x;
	ss_crop[1] = prfi->sensor_crop.offset.y;
	ss_crop[2] = prfi->sensor_crop.size.width;
	ss_crop[3] = prfi->sensor_crop.size.height;

	for (i = 0; i < 2; i++)
		ss_rms_crop_ratio[i] = prfi->sensor_remosaic_crop_zoom_ratio;

	for (i = 0; i < 2; i++)
		ss_aspect_ratio_offset[i] =
			(ss_cal[i] - (ss_crop[i + 2] * ss_rms_crop_ratio[i] / 10)) >> 1;

	/* Remove crop region offset for sensor aspect ratio from final sensor crop reigon */
	for (i = 0; i < 2; i++)
		ss_crop[i] = (ss_crop[i] > ss_aspect_ratio_offset[i]) ?
				     (ss_crop[i] - ss_aspect_ratio_offset[i]) :
				     0;

	size_cfg->rms_crop.x = ss_crop[0];
	size_cfg->rms_crop.y = ss_crop[1];
	size_cfg->rms_crop.w = ss_crop[2];
	size_cfg->rms_crop.h = ss_crop[3];

	if (param->otf_input.cmd) {
		param->otf_input.width = ss_crop[2];
		param->otf_input.height = ss_crop[3];

		bcrop[0] = param->otf_input.bayer_crop_offset_x;
		bcrop[1] = param->otf_input.bayer_crop_offset_y;
		bcrop[2] = param->otf_input.bayer_crop_width;
		bcrop[3] = param->otf_input.bayer_crop_height;
	} else {
		param->dma_input.width = ss_crop[2];
		param->dma_input.height = ss_crop[3];

		bcrop[0] = param->dma_input.bayer_crop_offset_x;
		bcrop[1] = param->dma_input.bayer_crop_offset_y;
		bcrop[2] = param->dma_input.bayer_crop_width;
		bcrop[3] = param->dma_input.bayer_crop_height;
	}
	 ...
	 */
	for (i = 0; i < 4; i++)
		bcrop[i] = bcrop[i] * ss_rms_crop_ratio[i % 2] / 10;
	 ...
	 */
	for (i = 0; i < 2; i++)
		bcrop[i] = (bcrop[i] > ss_crop[i]) ? (bcrop[i] - ss_crop[i]) :
						     0;
	 ...
	 */
	for (i = 2; i < 4; i++)
		bcrop[i] = (bcrop[i] > ss_crop[i]) ? ss_crop[i] : bcrop[i];
	 ...
	 */
	bcrop[2] = ALIGN_DOWN(bcrop[2], 2);

	if (param->otf_input.cmd) {
		param->otf_input.bayer_crop_offset_x = bcrop[0];
		param->otf_input.bayer_crop_offset_y = bcrop[1];
		param->otf_input.bayer_crop_width = bcrop[2];
		param->otf_input.bayer_crop_height = bcrop[3];
	} else {
		param->dma_input.bayer_crop_offset_x = bcrop[0];
		param->dma_input.bayer_crop_offset_y = bcrop[1];
		param->dma_input.bayer_crop_width = bcrop[2];
		param->dma_input.bayer_crop_height = bcrop[3];
	}
}
				...
				struct pablo_rta_frame_info *prfi)
{
	...
	if (test_bit(IS_SENSOR_RMS_CROP_ON, &sensor->rms_crop_state) &&
			prfi->sensor_remosaic_crop_zoom_ratio > 10) {
		size_cfg->rms_crop_ratio = prfi->sensor_remosaic_crop_zoom_ratio;
		cstat_hw_apply_rms_crop(prfi, cstat_p, size_cfg);
	} else {
...
static const struct is_hw_ip_ops is_hw_cstat_ops = {
...
EXPORT_SYMBOL_GPL(is_hw_cstat_probe);

void is_cstat_hw_s_int1_status(struct is_hw_ip *hw_ip, enum cstat_event_type type)
{
	cstat_hw_s_int1_status(hw_ip->regs[REG_SETA], type);
}
KUNIT_EXPORT_SYMBOL(is_cstat_hw_s_int1_status);

void is_cstat_hw_s_int2_status(struct is_hw_ip *hw_ip, enum cstat_event_type type)
{
	cstat_hw_s_int2_status(hw_ip->regs[REG_SETA], type);
}
KUNIT_EXPORT_SYMBOL(is_cstat_hw_s_int2_status);

void is_cstat_hw_c_int1(struct is_hw_ip *hw_ip)
{
	cstat_hw_c_int1(hw_ip->regs[REG_SETA]);
}
KUNIT_EXPORT_SYMBOL(is_cstat_hw_c_int1);

void is_cstat_hw_c_int2(struct is_hw_ip *hw_ip)
{
	cstat_hw_c_int2(hw_ip->regs[REG_SETA]);
}
KUNIT_EXPORT_SYMBOL(is_cstat_hw_c_int2);
