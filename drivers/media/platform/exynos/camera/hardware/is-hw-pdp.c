...
#define dbg_pdp(level, fmt, pdp, args...) \
	dbg_common((debug_pdp & (BIT(PDP_DBG_LOG_LEV_1) \
		| BIT(PDP_DBG_LOG_LEV_2))) >= level, \
		"[PDP%d]", fmt, pdp->id, ##args)

#define pd_dump_mode()	\
	((debug_pdp & (BIT(PDP_DBG_STAT_HPD_DUMP) | BIT(PDP_DBG_STAT_VPD_DUMP)))	\
		>> PDP_DBG_STAT_HPD_DUMP)

static DEFINE_MUTEX(cmn_reg_lock);
static ulong debug_pdp;

static int param_get_debug_pdp(char *buffer, const struct kernel_param *kp)
{
	int ret;

	ret = sprintf(buffer, "PDP debug features\n");
	ret += sprintf(buffer + ret, "\tb[0] : log lev 1\n");
	ret += sprintf(buffer + ret, "\tb[1] : log lev 2\n");
	ret += sprintf(buffer + ret, "\tb[2] : dump reg\n");
	ret += sprintf(buffer + ret, "\tb[3] : set default config\n");
	ret += sprintf(buffer + ret, "\tb[4] : stat + hpd dump\n");
	ret += sprintf(buffer + ret, "\tb[5] : stat + vpd dump\n");
	ret += sprintf(buffer + ret, "\tcurrent value : %ld\n", debug_pdp);

	return ret;
}

static const struct kernel_param_ops param_ops_debug_pdp = {
	.set = param_set_ulong,
	.get = param_get_debug_pdp,
};

module_param_cb(debug_pdp, &param_ops_debug_pdp, &debug_pdp, S_IRUGO | S_IWUSR);
...
static u32 is_hw_pdp_g_input(struct pablo_camif_otf_info *otf_info, struct is_pdp *pdp)
{
	...
	u32 otf_out_id, vc_mux_id;

	sensor_cfg = pdp->sensor_cfg;
	...
	if (pdp->vc_mux_base) {
		...
		pdp_hw_g_input_vc(pdp->vc_mux_val[vc_mux_id],
				&pdp->img_vc, &pdp->af_vc);
	} else {
		pdp->img_vc = sensor_cfg->img_vc[otf_out_id];
		pdp->af_vc = sensor_cfg->hpd_vc[otf_out_id];
	}
...
static int is_hw_pdp_init_config(struct is_hw_ip *hw_ip, u32 instance, struct is_frame *frame)
{
	...
	if (path == OTF) {
		...
		/**
		 * Clear frame core_flag
		 *
		 * It ignores the PDP core end event to trigger shot_done
		 * because PDP core end doesn't guarantee the end of frame processing.
		 */
	...
	if (enable && test_bit(PDP_DBG_DEFAULT_CONFIG, &debug_pdp)) {
	...
	if (path == OTF) {
		if (pdp->mux_base || IS_ENABLED(CONFIG_PDP_INPUT_MUX))
			pdp_hw_s_input_mux(pdp, csi->otf_info.otf_lc[otf_out_id]);
	...
	if (test_bit(PDP_DBG_REG_DUMP, &debug_pdp))
	...
	struct is_hw_ip *hw_ip, u32 instance, struct is_pdp *pdp, struct is_sensor_cfg *sensor_cfg)
{
	...
	if (test_bit(PDP_DBG_STAT_HPD_DUMP, &debug_pdp) ||
		test_bit(PDP_DBG_STAT_VPD_DUMP, &debug_pdp)) {
		w = sensor_cfg->input[pdp->img_vc].width;
		h = sensor_cfg->input[pdp->img_vc].height;
		b = 16;
	...
	if (pdp->icpu_dev) {
		framemgr = GET_SUBDEV_I_FRAMEMGR(isd);
		for (frame_idx = 0; frame_idx < framemgr->num_frames; frame_idx++) {
			...
			if (ret) {
				merr("failed to attach_ext(%d)", isd, ret);
				/* goto free */
				return ret;
			}
		}
...
static int pdp_deinit_internal_subdev(struct is_pdp *pdp, u32 instance)
{
	...
	return CALL_I_SUBDEV_OPS(isd, free, isd);
}
