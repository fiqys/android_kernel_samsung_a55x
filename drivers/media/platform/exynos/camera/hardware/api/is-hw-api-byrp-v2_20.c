...
#include "sfr/is-sfr-byrp-v2_20.h"
#include "pmio.h"
...
static void byrp_hw_s_coutfifo(struct pablo_mmio *base, u32 set_id, bool enable)
{
	...
	BYRP_SET_F(base, BYRP_R_BYR_COUTFIFO_AUTO_VBLANK, BYRP_F_BYR_COUTFIFO_AUTO_VBLANK, 0);
	...
	if (comp_sbwc_en == 0) {
		if (strip_enable)
			stride_1p = is_hw_dma_get_img_stride(memory_bitwidth, pixelsize, hwformat,
							     param_set->stripe_input.full_width, 1,
							     true);
		else
			stride_1p = is_hw_dma_get_img_stride(memory_bitwidth, pixelsize, hwformat,
							     width, 1, true);
...
static void byrp_hw_s_block_crc(struct pablo_mmio *base, u32 set_id, u32 seed)
{
	...
	BYRP_SET_F(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_CGRAS_CRC, BYRP_F_BYR_CGRAS_CRC_SEED,
		   seed);
...
static void byrp_hw_s_pixel_order(struct pablo_mmio *base, u32 set_id, u32 pixel_order)
{
	...
	BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_BPCFLATDETECTOR_PIXEL_ORDER, pixel_order);
	BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_BPCDIRDETECTOR_PIXEL_ORDER, pixel_order);
}
...
*/
	...
	/* 0: Bayer, 1: Tetra(only) */
	BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_BPCSUSPMAP_CFA_TYPE, non_byr_pattern == 1 ? 1 : 0);
	...
	/* 0: Bayer, 1: Tetra(only) */
	BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_BPCFLATDETECTOR_CFA_TYPE, non_byr_pattern == 1 ? 1 : 0);

	/* 0: Bayer, 1: Tetra(only) */
	BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_BPCDIRDETECTOR_CFA_TYPE, non_byr_pattern == 1 ? 1 : 0);
}
...
{
	bool cinfifo_en, coutfifo_en, mono_mode_en, hdr_en;
	u32 bit_in;
	u32 width, height;
	u32 pixel_order;
	u32 non_byr_pattern;
