...
static const struct cstat_stat_buf_info stat_buf_info[IS_CSTAT_SUBDEV_NUM] = {
	[IS_CSTAT_CDAF] = { .w = 211, .h = 1, .bpp = 32, .num = 1 },
	[IS_CSTAT_PRE_THUMB] = { .w = 98304, .h = 1, .bpp = 8, .num = CSAT_STAT_BUF_NUM },
	[IS_CSTAT_AE_THUMB] = { .w = 65536, .h = 1, .bpp = 8, .num = CSAT_STAT_BUF_NUM },
...
static void _cstat_hw_s_ds_fdpig(void __iomem *base, bool en, struct cstat_ds_cfg *cfg)
{
	...
	if (!en) {
		CSTAT_SET_F(base, CSTAT_R_RGB_DS_FDPIG_CROP_EN, CSTAT_F_RGB_DS_FDPIG_CROP_EN, 0);
		...
		struct cstat_param_set *param, u32 num_buffers)
{
	...
	dbg_hw(2, "[CSTAT][%s]dma_addr: img[0] 0x%llx\n",
			dma->name,
			cfg.addrs[0]);
		...
		struct cstat_param_set *p_set)
{
	...
	if (total_w > in_w || total_h > in_h) {
		if (size_cfg->rms_crop_ratio > 10) {
			u32 sensor_crop_offset_x, sensor_crop_offset_y;
			...
			sensor_crop_offset_x =
				CSTAT_GET_BNS_OUT_SIZE(size_cfg->rms_crop.x, size_cfg->bns_r);
			sensor_crop_offset_y =
				CSTAT_GET_BNS_OUT_SIZE(size_cfg->rms_crop.y, size_cfg->bns_r);

			if (dma_out->dma_crop_offset_x >= sensor_crop_offset_x)
				dma_out->dma_crop_offset_x -= sensor_crop_offset_x;
			else
				dma_out->dma_crop_offset_x = 0;

			if (dma_out->dma_crop_offset_y >= sensor_crop_offset_y)
				dma_out->dma_crop_offset_y -= sensor_crop_offset_y;
			else
				dma_out->dma_crop_offset_y = 0;

			/* Update total size & check incrop boundary again */
			total_w = dma_out->dma_crop_offset_x +
				  dma_out->dma_crop_width;
			total_h = dma_out->dma_crop_offset_y +
				  dma_out->dma_crop_height;

			if (total_w > in_w)
				dma_out->dma_crop_width =
					in_w - dma_out->dma_crop_offset_x;

			if (total_h > in_h)
				dma_out->dma_crop_height =
					in_h - dma_out->dma_crop_offset_y;
		} else {
		...
		struct cstat_param_set *param, u32 num_buffers, int disable)
{
	...
	case CSTAT_W_RGBYHIST:
		...
		break;
	case CSTAT_W_THSTATPRE:
		dma_out = &param->dma_output_pre;
		cfg.addrs = param->output_dva_pre;
		cfg.num_buffers = 1;
		break;
	case CSTAT_W_THSTATAWB:
		dma_out = &param->dma_output_awb;
		cfg.addrs = param->output_dva_awb;
		cfg.num_buffers = 1;
		break;
	case CSTAT_W_THSTATAE:
		dma_out = &param->ae;
...
void cstat_hw_s_dma_cfg(struct cstat_param_set *p_set, struct is_cstat_config *conf)
{
	...
	p_set->dma_output_pre.cmd = !conf->thstatpre_bypass;
	if (p_set->dma_output_pre.cmd) {
		p_set->dma_output_pre.width = conf->pre_grid_w * 2 * 12;
		p_set->dma_output_pre.height = conf->pre_grid_h;

		if (!p_set->dma_output_pre.width || !p_set->dma_output_pre.height) {
			...
			warn_hw("[%d][CSTAT][F%d] Invalid PRE size %dx%d", p_set->instance_id, p_set->fcount,
					p_set->dma_output_pre.width, p_set->dma_output_pre.height);
		}
	...
	p_set->dma_output_awb.cmd = !conf->thstatawb_bypass;
	if (p_set->dma_output_awb.cmd) {
		p_set->dma_output_awb.width = conf->awb_grid_w * 2 * 8;
		p_set->dma_output_awb.height = conf->awb_grid_h;

		if (!p_set->dma_output_awb.width || !p_set->dma_output_awb.height) {
			...
			warn_hw("[%d][CSTAT][F%d] Invalid AWB size %dx%d", p_set->instance_id, p_set->fcount,
					p_set->dma_output_awb.width, p_set->dma_output_awb.height);
		}
