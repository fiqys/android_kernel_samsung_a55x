	...
	enum yuvp_input_path_type input_path, enum yuvp_chain_demux_type output_path)
{
	...
	ret = yuvp_hw_s_cnr_size(get_base(hw_ip), set_id, strip_start_pos, frame_width,
...
#if IS_ENABLED(CONFIG_YUVP_DRC_CONTENTS_AWARE_EN)
		if (hw_yuvp->config.drc_contents_aware_isp_en)
			*output_path |= YUVP_DEMUX_SEGCONF_DRCTMC;
#endif
	}
	...
	ulong hw_map)
{
	...
	if (param_set->stripe_input.repeat_scenario == PABLO_REPEAT_YUVP_CLAHE) {
		if (!param_set->stripe_input.repeat_idx)
			...
			conf.svhist_bypass = 1;

		msinfo_hw("idx %d, clahe_bypass(%d), svhist_bypass(%d)\n", instance, hw_ip,
			param_set->stripe_input.repeat_idx, conf.clahe_bypass, conf.svhist_bypass);
	...
	yuvp_hw_s_svhist_bypass(get_base(hw_ip), set_id, conf.svhist_bypass);
	...
	bypass = (!conf.yuvnr_contents_aware_isp_en &&
			!conf.ccm_contents_aware_isp_en &&
			!conf.sharpen_contents_aware_isp_en
#if IS_ENABLED(CONFIG_YUVP_DRC_CONTENTS_AWARE_EN)
			&& !conf.drc_contents_aware_isp_en
