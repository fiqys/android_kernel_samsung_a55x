...
static int is_hw_mcsc_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (hw_mcsc->rdma_max_cnt) {
		hw_mcsc->rdma = pablo_calloc(hw_mcsc->rdma_max_cnt, sizeof(struct is_common_dma),
			GFP_KERNEL);
		if (!hw_mcsc->rdma) {
	...
	hw_mcsc->wdma = pablo_calloc(hw_mcsc->wdma_max_cnt, sizeof(struct is_common_dma),
			GFP_KERNEL);
	if (!hw_mcsc->wdma) {
...
err_alloc_dma:
	pablo_free(hw_mcsc->rdma);
	pablo_free(hw_mcsc->wdma);
