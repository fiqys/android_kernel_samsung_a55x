...
static ulong debug_rgbp;
module_param(debug_rgbp, ulong, 0644);
	...
	ulong hw_map)
{
	...
	if (unlikely(test_bit(RGBP_DBG_DTP, &debug_rgbp)))
	...
	if (unlikely(test_bit(RGBP_DBG_DUMP_REG, &debug_rgbp)))
		rgbp_hw_dump(hw_ip->pmio, HW_DUMP_CR);
