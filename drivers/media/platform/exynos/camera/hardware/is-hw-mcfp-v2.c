...
static int is_hw_mcfp_handle_interrupt(u32 id, void *context)
{
	...
	irq_index = id == 0 ? MCFP_INTR_0 : MCFP_INTR_1;

	status = mcfp_hw_g_int_state(get_base(hw_ip), true,
			(hw_ip->num_buffers & 0xffff),
			irq_index, &hw_mcfp->irq_state[irq_index])
			& mcfp_hw_g_int_mask(get_base(hw_ip), irq_index);
	...
	if (id == 0)
		...
		f_err = mcfp_hw_is_occurred1(status, INTR_ERR);

	if (f_err) {
		msinfo_hw("[F:%d] Ocurred error interrupt%d (%d,%d) status(0x%x)\n",
			instance, hw_ip, hw_fcount, id, hl, vl, status);

		if (id == 0) {
			mcfp_hw_dump(hw_ip->pmio, HW_DUMP_DBG_STATE);
			mcfp_hw_dump(hw_ip->pmio, HW_DUMP_CR);
		}
	}
				...
				u32 scenario)
{
	...
	u32 i, sbwc_t, sbwc_en, comp_64b_align = 1, lossy_byte32num = 0;
	...
	case MCFP_SUBDEV_PREV_YUV_2NR:
	case MCFP_SUBDEV_YUV_2NR:
		if (config->still_en)
			return -EINVAL;
	...
	case MCFP_SUBDEV_W:
	case MCFP_SUBDEV_PREV_W_2NR:
	case MCFP_SUBDEV_W_2NR:
		sd->width = width;
		sd->height = height;
		sd->num_planes = 1;
		sd->num_batch = 1;
...
static void __is_hw_mcfp_free_buffer_2nr(u32 instance, struct is_hw_ip *hw_ip)
{
	enum is_hw_mcfp_subdev buf_id;
	struct is_hw_mcfp *hw_mcfp = (struct is_hw_mcfp *)hw_ip->priv_info;
	struct pablo_internal_subdev *sd;
...
static int __is_hw_mcfp_alloc_buffer_2nr(u32 instance, struct is_hw_ip *hw_ip,
					struct mcfp_param_set *param_set,
					struct is_mcfp_config *config)
{
	int ret;
...
err_sd:
	__is_hw_mcfp_free_buffer_2nr(instance, hw_ip);
...
static int __nocfi is_hw_mcfp_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	hw_ip->priv_info = pablo_zalloc(sizeof(struct is_hw_mcfp), GFP_KERNEL);
	if (!hw_ip->priv_info) {
			...
			bool flag, u32 f_type)
{
	...
	mem = CALL_HW_CHAIN_INFO_OPS(hw_ip->hardware, get_iommu_mem, GROUP_ID_MCFP);
	for (buf_id = 0; buf_id < MCFP_SUBDEV_MAX; buf_id++)
					...
					u32 instance)
{
	...
	if (test_bit(PARAM_MCFP_HF, pmap))
		memcpy(&param_set->otf_input_hf, &param->hf,
				sizeof(struct param_otf_input));
#endif
}
	...
	u32 instance, const struct is_frame *frame, u32 set_id)
{
...
#if defined(MCFP_STRIP_OTF_MARGIN_CROP)
	if (strip_enable) {
		otf_crop_bypass = 0;

		if (param_set->stripe_input.left_margin > 0 &&
		    crop_img_width > MCFP_STRIP_OTF_MARGIN_CROP) {
			crop_img_x = MCFP_STRIP_OTF_MARGIN_CROP;
			crop_img_width -= MCFP_STRIP_OTF_MARGIN_CROP;
		}
	...
	msdbgs_hw(3, "[otf] in_crop_width %d, crop_img_x %d, crop_img_width %d\n",
			instance, hw_ip, param_set->dma_input.dma_crop_width,
			crop_img_x, crop_img_width);
#endif

	mcfp_hw_s_crop_clean_img_otf(get_base(hw_ip), set_id,
					crop_img_x, crop_img_width, dma_height,
					otf_crop_bypass);
	mcfp_hw_s_crop_wgt_otf(get_base(hw_ip), set_id,
	...
	if (strip_enable) {
		...
		crop_img_x = param_set->stripe_input.left_margin > crop_margin_for_align ?
				param_set->stripe_input.left_margin - crop_margin_for_align : 0;
	...
	param_set->dma_output_w.dma_crop_offset_x = crop_img_start_x;
	param_set->dma_output_w.dma_crop_width = crop_img_width;
	...
	msdbgs_hw(3, "in_crop(ofs:%d, width:%d), out_crop(ofs:%d,%d, width:%d), margin(%d, %d)\n",
			...
			crop_img_x, crop_img_start_x, crop_img_width,
	...
	mcfp_hw_s_mixer_stitch_lme_map_width(get_base(hw_ip), set_id, mcfp_config,
			dma_width, dma_height);
	mcfp_hw_s_mixer_stitch_drc_map_width(get_base(hw_ip), set_id,
			mcfp_config->drc_grid_w,
			mcfp_config->drc_grid_h);

	return 0;
}

static int __is_hw_mcfp_set_hf_regs(struct is_hw_ip *hw_ip, struct mcfp_param_set *param_set,
	u32 instance, const struct is_frame *frame, u32 set_id)
{
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	if (param_set->otf_input_hf.cmd != OTF_INPUT_COMMAND_ENABLE) {
		mcfp_hw_s_hf_otf(get_base(hw_ip), set_id, 0);
		return 0;
	}

	mcfp_hw_s_hf_otf(get_base(hw_ip), set_id, 1);
	mcfp_hw_s_hf_input_size(get_base(hw_ip), set_id,
			param_set->otf_input_hf.width, param_set->otf_input_hf.height);
#endif
					...
					u32 p_idx, u32 c_idx)
{
	...
	i_fmgr_p = GET_SUBDEV_I_FRAMEMGR(&hw_mcfp->subdev[instance][p_idx]);
	i_fmgr_c = GET_SUBDEV_I_FRAMEMGR(&hw_mcfp->subdev[instance][c_idx]);
...
static int __config_mixer_buffers(struct is_hw_ip *hw_ip, struct is_frame *frame,
		struct is_mcfp_config *config,
		u32 cmd_sf_in,
		u32 cmd_prev_wgt_in, u32 cmd_prev_yuv_in,
		u32 cmd_yuv_out, u32 cmd_wgt_out)
{
	int ret;
	...
	if (config->mixer_enable) {
		...
		if (cmd_prev_yuv_in && cmd_yuv_out) {
			...
			is_hw_mcfp_set_yuv_i_buffer(instance, hw_ip, hw_mcfp, frame, swap_frame, swap_fro,
					MCFP_SUBDEV_PREV_YUV, MCFP_SUBDEV_YUV);
		}
		...
		if (cmd_prev_wgt_in && cmd_wgt_out) {
			...
			is_hw_mcfp_set_wgt_i_buffer(instance, hw_ip, hw_mcfp, frame, swap_frame, swap_fro,
					MCFP_SUBDEV_PREV_W, MCFP_SUBDEV_W);
		}
	} else {
		__is_hw_mcfp_free_buffer(instance, hw_ip);
	}
...
static int __config_mixer_buffers_2nr(struct is_hw_ip *hw_ip, struct is_frame *frame,
		struct is_mcfp_config *config)
{
	int ret;
	bool swap_frame, swap_fro;
	...
	if (config->mixer_enable) {
		ret = __is_hw_mcfp_alloc_buffer_2nr(instance, hw_ip, param_set, config);
		if (ret)
			return ret;
		...
		is_hw_mcfp_set_yuv_i_buffer(instance, hw_ip, hw_mcfp, frame, swap_frame, swap_fro,
				MCFP_SUBDEV_PREV_YUV_2NR, MCFP_SUBDEV_YUV_2NR);

		is_hw_mcfp_set_wgt_i_buffer(instance, hw_ip, hw_mcfp, frame, swap_frame, swap_fro,
				MCFP_SUBDEV_PREV_W_2NR, MCFP_SUBDEV_W_2NR);
	} else {
		__is_hw_mcfp_free_buffer_2nr(instance, hw_ip);
	}
	...
	ulong hw_map)
{
	...
	if (IS_ENABLED(SKIP_ISP_SHOT_FOR_MULTI_SHOT)) {
		if (hw_mcfp->repeat_instance != instance)
			hw_mcfp->repeat_state = 0;

		if (batch_num > 1 || strip_total_count > 1 || strip_repeat_num > 1)
			hw_mcfp->repeat_state++;
		else
			hw_mcfp->repeat_state = 0;

		if (hw_mcfp->repeat_state > 1 &&
		...
		hw_mcfp->repeat_instance = instance;
	}
	...
	ret = __is_hw_mcfp_set_hf_regs(hw_ip, param_set, instance, frame, set_id);
	if (ret) {
		mserr_hw("__is_hw_mcfp_set_hf_regs is fail", instance, hw_ip);
		goto shot_fail_recovery;
	}
	...
	if (__check_recursive_nr_2nd_iter(frame)) {
		...
		ret = __config_mixer_buffers(hw_ip, frame, config,
				cmd_sf_in,
				cmd_prev_wgt_in, cmd_prev_yuv_in,
				cmd_yuv_out, cmd_wgt_out);
		if (ret)
			goto shot_fail_recovery;
	}

#if IS_ENABLED(CONFIG_PABLO_DLFE)
	mcfp_hw_s_dlfe_path(get_base(hw_ip), config->dlfe_en);
#endif
...
#if IS_ENABLED(CONFIG_PABLO_DLFE)
	if (hw_mcfp->config[instance].dlfe_en != mcfp_config->dlfe_en)
		msinfo_hw("[F:%d] MCFP DLFE enable: %d -> %d", instance, hw_ip, fcount,
			  hw_mcfp->config[instance].dlfe_en,
			  mcfp_config->dlfe_en);
#endif
