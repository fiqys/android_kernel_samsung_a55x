...
static int gdc_alloc_pmio_mem(struct gdc_dev *gdc)
{
	...
	gdc->pb_c_loader_payload = CALL_PTR_MEMOP(mem, alloc,
						mem->priv, 0x8000,
						NULL, 0);
	...
	gdc->pb_c_loader_header = CALL_PTR_MEMOP(mem, alloc,
					mem->priv, 0x2000,
					NULL, 0);
...
static int gdc_open(struct file *file)
{
	...
	dev_info(gdc->dev, "gdc open refcnt = %d\n", atomic_read(&gdc->m2m.in_use));
...
static int gdc_release(struct file *file)
{
	...
	dev_info(gdc->dev, "gdc close refcnt = %d\n", atomic_read(&gdc->m2m.in_use));
...
static int gdc_run_next_job(struct gdc_dev *gdc)
{
	...
	if (ret) {
		dev_err(gdc->dev, "%s: failed to update gdc param(%d)", __func__, ret);
		spin_lock_irqsave(&gdc->ctxlist_lock, flags);
		gdc->current_ctx = NULL;
		gdc->votf_ctx = NULL;
		spin_unlock_irqrestore(&gdc->ctxlist_lock, flags);
...
static inline void gdc_get_bufaddr_8_plus_2_fmt(
	struct gdc_frame *frame, unsigned int w, unsigned int h)
{
		...
		struct vb2_buffer *vb2buf, struct gdc_frame *frame)
{
	...
	case 2:
		...
		break;
	case 3:
		ret = gdc_get_bufaddr_frame_fmt(frame, pixsize, bytesize, vb2buf);
		if (ret)
			return ret;
		break;
	default:
...
static void gdc_fill_curr_frame(struct gdc_dev *gdc, struct gdc_ctx *ctx)
{
	...
	gdc_dbg("gdc_src : pixelformat = %c%c%c%c, w = %d, h = %d\n",
		...
		s_frame->width, s_frame->height);
	gdc_dbg("gdc_dst : pixelformat = %c%c%c%c, w = %d, h = %d\n",
...
static void gdc_out_votf_otf(struct gdc_dev *gdc, struct gdc_ctx *ctx)
{
	unsigned long flags;
...
static inline void *__get_dev_for_votf(void *device)
{
	if (device)
...
int gdc_device_run(unsigned long i_ino)
{
	...
	struct gdc_ctx *ctx;
	struct gdc_dev *gdc =
		(struct gdc_dev *)__get_dev_for_votf(gdc_device);
	int ret;
...
{
	struct gdc_dev *gdc = (struct gdc_dev *)__get_dev_for_votf(gdc_device);
...
static int gdc_register_m2m_device(struct gdc_dev *gdc, int dev_id)
{
	...
	dev_info(gdc->dev, "video node register: %d\n", EXYNOS_VIDEONODE_CAMERAPP(node_num));
...
static int gdc_pmio_init(struct gdc_dev *gdc)
{
	...
	if (IS_ERR(gdc->pmio)) {
		dev_err(gdc->dev, "failed to init gdc PMIO: %ld", PTR_ERR(gdc->pmio));
		return -ENOMEM;
	}
...
static int gdc_probe(struct platform_device *pdev)
{
	...
	if (votf_np) {
		...
		if (!gdc_device) {
			...
			dev_err(&pdev->dev, "VOTF is busy. Already connected to MFC\n");
		}
		gdc->has_votf_mfc = 1;
	...
	if (gdc->use_cloader_iommu_group) {
		ret = of_property_read_u32(np, "iommu_group_for_cloader", &gdc->cloader_iommu_group_id);
		if (ret)
...
struct device *gdc_get_device(void)
{
	return gdc_device;
}
KUNIT_EXPORT_SYMBOL(gdc_get_device);

void gdc_set_device(struct device *data)
{
	gdc_device = data;
}
KUNIT_EXPORT_SYMBOL(gdc_set_device);
#endif
