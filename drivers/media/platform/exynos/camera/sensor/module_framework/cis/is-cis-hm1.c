...
int sensor_hm1_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(cis->client, 0x0704, &data16);
	...
	else goto i2c_err;
	{
		int i = 0;
		for (i = 0; i < 96; i += 2) {
			ret = cis->ixc_ops->read16(cis->client, 0xE580 + i, &data16);
...
static int sensor_hm1_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	...
	ret = cis->ixc_ops->write8(cis->client, 0x0104, hold); /* api_rw_general_setup_grouped_parameter_hold */
	if (ret < 0)
		goto p_err;
...
int sensor_hm1_cis_write16_burst(void *vclient, u16 addr, u8 *val, u32 num, bool endian)
{
	int ret = 0;
...
#define HM1_CALIBRATION 0
#if IS_ENABLED(HM1_CALIBRATION)
...
int sensor_hm1_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (ex_mode == EX_REMOSAIC_CAL
		...
		if (!sensor_hm1_eeprom_cal_available) {
			...
			ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
			ret |= cis->ixc_ops->write16(cis->client, 0x6000, 0x0005);
			ret |= cis->ixc_ops->write16(cis->client, 0x0D02, 0x0000);
			ret |= cis->ixc_ops->write16(cis->client, 0x0B00, 0x0001);
			ret |= cis->ixc_ops->write16(cis->client, 0x6000, 0x0085);
		} else {
	...
	if (mode == SENSOR_HM1_12000X9000_10FPS
		|| mode == SENSOR_HM1_7200X5400_14FPS) {
		cis->cis_data->remosaic_mode = true;
	} else {
		cis->cis_data->remosaic_mode = false;
	}
...
int sensor_hm1_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	if (sensor_hm1_fake_retention_status) {
		...
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write16(cis->client, 0x6B04, 0xB000);
		info("[%s] mipi strength 490mV\n", __func__);
#else
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write16(cis->client, 0x6B04, 0xE000);
...
int sensor_hm1_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	...
	ret = cis->ixc_ops->write16(cis->client, 0x0402, 0x0000);
	if (ret < 0)
		goto p_err_i2c_unlock;
	/* down_scale_m: 1 to 16 upwards (scale_n: 16(fixed))
	 * down scale factor = down_scale_m / down_scale_n
	 */
	ret = cis->ixc_ops->write16(cis->client, 0x0404, 0x10);
	if (ret < 0)
		goto p_err_i2c_unlock;
...
int sensor_hm1_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	cis->ixc_ops->write16(cis->client, 0x602C, 0x2001);
	pr_info("[%s] 0x2001 page\n", __func__);

	cis->ixc_ops->write16(cis->client, 0x602E, 0x20FC);
	cis->ixc_ops->read16(cis->client, 0x6F12, &data16);
	pr_info("[%s] 0x20FC(0x%x)\n", __func__, data16);

	cis->ixc_ops->write16(cis->client, 0x602E, 0x20FE);
	cis->ixc_ops->read16(cis->client, 0x6F12, &data16);
	pr_info("[%s] 0x20FE(0x%x)\n", __func__, data16);

	cis->ixc_ops->write16(cis->client, 0x602E, 0x2100);
	cis->ixc_ops->read16(cis->client, 0x6F12, &data16);
...
int sensor_hm1_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->read16(cis->client, 0x0D08, &data16);
	pr_info("[%s] 0x0D08(0x%x)\n", __func__, data16);

	cis->ixc_ops->read16(cis->client, 0x0D0A, &data16);
...
static int cis_hm1_probe_i2c(struct i2c_client *client,
