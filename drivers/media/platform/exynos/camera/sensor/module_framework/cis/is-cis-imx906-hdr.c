...
#include "is-dt.h"
#include "is-vendor.h"
#include "is-cis-imx906-hdr.h"
#include "is-cis-imx906-hdr-setA.h"
...
void sensor_imx906_cis_set_mode_group(u32 mode)
{
	...
	sensor_imx906_mode_groups[SENSOR_IMX906_MODE_AEB] = MODE_GROUP_NONE;
	...
	case SENSOR_IMX906_4080X3060_30FPS_R12:
		...
		sensor_imx906_mode_groups[SENSOR_IMX906_MODE_AEB] =
...
int sensor_imx906_cis_write_aeb_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_imx906_private_data *priv;

	priv = (struct sensor_imx906_private_data *)cis->sensor_info->priv;
...
int sensor_imx906_cis_write_aeb_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_imx906_private_data *priv;
...
int sensor_imx906_cis_force_aeb_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
	unsigned int mode = cis_data->sens_config_index_cur;
	unsigned int next_mode = sensor_imx906_mode_groups[SENSOR_IMX906_MODE_NORMAL];
	...
	if (cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_2VC
		...
		ret |= sensor_imx906_cis_write_aeb_off(subdev);
		info("[%s] disable AEB\n", __func__);
	}

	// Changing sensor mode to 30fps during aeb_force_off
	if ((mode != sensor_imx906_mode_groups[SENSOR_IMX906_MODE_AEB]) || (next_mode == MODE_GROUP_NONE))
		return ret;
	...
	if (next_mode_info->setfile_aeb_fcm.size != 0)
		ret |= sensor_cis_write_registers(subdev, next_mode_info->setfile_aeb_fcm);
	else
...
bool sensor_imx906_cis_check_aeb(struct v4l2_subdev *subdev)
{
	...
	if (cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_2VC
		...
		info("[%s] Not support AEB stream_on(%d) cur_hdr_mode(%d)\n",
			__func__, cis_data->stream_on, cis_data->cur_hdr_mode);
		cis_data->pre_hdr_mode = cis_data->cur_hdr_mode = SENSOR_HDR_MODE_SINGLE;
		sensor_imx906_cis_write_aeb_off(subdev);
		info("[%s] disable AEB\n", __func__);
		return false;
	}
	...
	if (cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_2VC) {
		info("[%s] enable AEB\n", __func__);
		sensor_imx906_cis_write_aeb_on(subdev);
	} else {
		info("[%s] disable AEB\n", __func__);
		sensor_imx906_cis_write_aeb_off(subdev);
	}
...
int sensor_imx906_cis_check_hdr_mode(cis_shared_data *cis_data, u32 *next_mode)
{
	int ret = 0;

	if (sensor_imx906_mode_groups[SENSOR_IMX906_MODE_AEB] == MODE_GROUP_NONE) {
		ret = -1;
		goto EXIT;
	}

	switch (cis_data->cur_hdr_mode) {
	case SENSOR_HDR_MODE_2AEB_2VC:
		*next_mode = sensor_imx906_mode_groups[SENSOR_IMX906_MODE_AEB];
		break;
	default:
		ret = -1;
		break;
...
int sensor_imx906_cis_check_cropped_remosaic(cis_shared_data *cis_data, unsigned int mode, unsigned int *next_mode)
{
	...
	*next_mode = sensor_imx906_mode_groups[SENSOR_IMX906_MODE_RMS_CROP];
	...
	if (zoom_ratio == cis_data->pre_remosaic_zoom_ratio) {
		dbg_sensor(1, "%s : cur zoom ratio is same pre zoom ratio (%u)\n", __func__, zoom_ratio);
		return ret;
	}
...
int sensor_imx906_cis_update_seamless_mode(struct v4l2_subdev *subdev)
{
	...
	bool use_aeb_full_setting = false;
	ktime_t st = ktime_get();
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	if (sensor_imx906_cis_check_aeb(subdev) == true) {
		is_aeb_changed = true;
		info("[%s] aeb mode changed\n", __func__);
	}

	if (cis_data->cur_hdr_mode != SENSOR_HDR_MODE_2AEB_2VC) { //To block other seamless mode changes if aeb on
		...
		//selecting fcm mode to switch to 60fps aeb mode
		sensor_imx906_cis_check_hdr_mode(cis->cis_data, &next_mode);
	}
	...
	if (is_aeb_changed == true && cis->cis_data->cur_lownoise_mode == IS_CIS_LN4) {
		info("[%s][%d] check the RTA seamless ctrl : LN[%d] AEB[%d]\n",
			__func__, cis->cis_data->sen_vsync_count,
			cis->cis_data->cur_lownoise_mode, cis_data->cur_hdr_mode);
		use_aeb_full_setting = true;
	}

	if (is_aeb_changed == true && use_aeb_full_setting == false && next_mode_info->setfile_aeb_fcm.size != 0)
		ret |= sensor_cis_write_registers(subdev, next_mode_info->setfile_aeb_fcm);
	else
		ret |= sensor_cis_write_registers(subdev, next_mode_info->setfile_fcm);
	...
	cis->cis_data->sens_config_index_pre = cis->cis_data->sens_config_index_cur;
	cis->cis_data->sens_config_index_cur = next_mode;
	cis->cis_data->cur_12bit_mode = next_mode_info->state_12bit;

	CALL_CISOPS(cis, cis_data_calculation, subdev, next_mode);

	if (priv_runtime->min_frame_us_time_based_setfps > cis_data->min_frame_us_time) {
		info("%s : min_frame_us_time_based_setfps(%d), min_frame_us_time(%d)\n",
			__func__, priv_runtime->min_frame_us_time_based_setfps, cis_data->min_frame_us_time);
		cis_data->min_frame_us_time = priv_runtime->min_frame_us_time_based_setfps;
	}
...
int sensor_imx906_cis_get_seamless_mode_info(struct v4l2_subdev *subdev)
{
	...
	if (cis->sensor_info->mode_infos[mode]->aeb_support) {
		...
		sensor_cis_get_mode_info(subdev, sensor_imx906_mode_groups[SENSOR_IMX906_MODE_AEB],
			&cis_data->seamless_mode_info[cnt]);
		cnt++;
	}
...
int sensor_imx906_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (is_sensor_check_aeb_mode(device)) {
		...
		ret = sensor_cis_write_registers_locked(subdev, priv->aeb_init);
		if (ret < 0)
			err("aeb_init setting fail!!");
		else
			info("[%s] aeb_init setting done!!\n", __func__);
	...
	if (cis->cis_data->seamless_ctl_before_stream.mode & SENSOR_MODE_AEB)
...
int sensor_imx906_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	sensor_imx906_cis_force_aeb_off(subdev);
...
int sensor_imx906_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
	...
	ret = sensor_cis_set_frame_rate(subdev, min_fps);
	priv_runtime->min_frame_us_time_based_setfps = cis_data->min_frame_us_time;

	dbg_sensor(1, "%s set FPS(%d), min_frame_us_time_based_setfps(%d)\n",
			__func__, min_fps, priv_runtime->min_frame_us_time_based_setfps);
	return ret;
}
...
static struct is_cis_ops cis_ops_imx906 = {
	...
	.cis_set_frame_rate = sensor_imx906_cis_set_frame_rate,
	...
	const struct i2c_device_id *id)
{
	...
	probe_info("%s started for IMX906-hdr\n", __func__);
	...
	priv_runtime->lte_vsync = 0;
	priv_runtime->min_frame_us_time_based_setfps = 0;
...
static const struct of_device_id sensor_cis_imx906_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-imx906-hdr",
	},
	{},
};
