...
int sensor_gc05a3_cis_init_state(struct v4l2_subdev *subdev, int mode)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
...
int sensor_gc05a3_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	/* fix for blink issue */
	if (mode != SENSOR_GC05A3_640x480_120FPS) {
		IXC_MUTEX_LOCK(cis->ixc_lock);
		ret = cis->ixc_ops->write8(cis->client, 0x0204, 0x04);
		ret = cis->ixc_ops->write8(cis->client, 0x0205, 0x00);
...
static struct is_cis_ops cis_ops_gc05a3 = {
	...
	.cis_init_state = sensor_gc05a3_cis_init_state,
