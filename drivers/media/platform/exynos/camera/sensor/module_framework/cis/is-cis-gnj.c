...
#define SENSOR_NAME "GNJ"

/* temp change, remove once PDAF settings done */
//#define PDAF_DISABLE
#ifdef PDAF_DISABLE
const u16 sensor_gnj_pdaf_off[] = {
	0x6000, 0x0005, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x0720, 0x0000, 0x02,
	0x6000, 0x0085, 0x02,
...
const struct sensor_regs pdaf_off = SENSOR_REGS(sensor_gnj_pdaf_off);
#endif

void sensor_gnj_cis_set_mode_group(u32 mode)
{
	sensor_gnj_mode_groups[SENSOR_GNJ_MODE_NORMAL] = mode;
	sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP] = MODE_GROUP_NONE;
	sensor_gnj_mode_groups[SENSOR_GNJ_MODE_LN4] = MODE_GROUP_NONE;
	sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG] = MODE_GROUP_NONE;

	switch (mode) {
	case SENSOR_GNJ_MODE_4080x3060_60FPS_R12:
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP] =
			SENSOR_GNJ_MODE_4080x3060_30FPS_REMOSAIC_CROP_R12;
		break;
	case SENSOR_GNJ_MODE_4080x2296_60FPS_R12:
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP] =
			SENSOR_GNJ_MODE_4080x2296_30FPS_REMOSAIC_CROP_R12;
		break;
	case SENSOR_GNJ_MODE_4080x3060_30FPS_IDCG_R12:
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG] =
			SENSOR_GNJ_MODE_4080x3060_30FPS_IDCG_R12;
		break;
	case SENSOR_GNJ_MODE_4080x2296_30FPS_IDCG_R12:
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG] =
	...
	info("[%s] normal(%d) ln4(%d) idcg(%d) rms_crop(%d)\n", __func__,
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_NORMAL],
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_LN4],
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG],
		sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP]);
}

#if IS_ENABLED(CIS_CALIBRATION)
#ifdef GNJ_BURST_WRITE
int sensor_gnj_cis_write16_burst(struct i2c_client *client, u16 addr, u8 *val, u32 num, bool endian)
{
	int ret = 0;
...
int sensor_gnj_cis_set_cal(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	char *cal_buf = NULL;
	int rom_id = 0;
	struct sensor_gnj_private_data *priv = (struct sensor_gnj_private_data *)cis->sensor_info->priv;
	...
	ret |= cis->ixc_ops->write16(cis->client, 0x602A, GNJ_XTC_ADDR);

	/* XTC data */
	info("[%s] rom_xtc_cal burst write start(0x%X) size(%d)\n",
			__func__, start_addr, cal_size);
	ret = sensor_gnj_cis_write16_burst(cis->client, 0x6F12,
			(u8 *)&cal_buf[start_addr], cal_size/2, GNJ_BIG_ENDIAN);
	if (ret < 0) {
		err("sensor_gnj_cis_write16_burst fail!!");
		goto p_err;
	}
	ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
	ret |= cis->ixc_ops->write16(cis->client, 0x6004, 0x0002); // Burst mode disable
#else
	ret = cis->ixc_ops->write8_sequential(cis->client, GNJ_XTC_ADDR, (u8 *)&cal_buf[start_addr], cal_size);
#endif
...
/* CIS OPS */
int sensor_gnj_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	cis->cis_data->sens_config_index_pre = SENSOR_GNJ_MODE_MAX;
...
static const struct is_cis_log log_gnj[] = {
...
int sensor_gnj_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);

	sensor_cis_log_status(cis, log_gnj, ARRAY_SIZE(log_gnj), (char *)__func__);

	return ret;
}

int sensor_gnj_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_gnj_private_data *priv = (struct sensor_gnj_private_data *)cis->sensor_info->priv;
	...
	/*write fcm sram settings*/
	ret = sensor_cis_write_registers_locked(subdev, priv->load_sram);
	if (ret < 0)
		err("sram setting fail!!");

#if IS_ENABLED(CIS_CALIBRATION)
	/* XTC caliberation write */
	sensor_gnj_cis_set_cal(subdev);
#endif

	return ret;
}

int sensor_gnj_cis_check_12bit(cis_shared_data *cis_data, u32 *next_mode)
{
	int ret = 0;

	if (sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG] == MODE_GROUP_NONE) {
		ret = -1;
		goto EXIT;
	}
	...
	case SENSOR_12BIT_STATE_REAL_12BIT:
		*next_mode = sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG];
		break;
...
int sensor_gnj_cis_check_lownoise(cis_shared_data *cis_data, u32 *next_mode)
{
	int ret = 0;
	u32 temp_mode = MODE_GROUP_NONE;

	if (sensor_gnj_mode_groups[SENSOR_GNJ_MODE_LN4] == MODE_GROUP_NONE) {
		ret = -1;
		goto EXIT;
	}
	...
	case IS_CIS_LN4:
		temp_mode = sensor_gnj_mode_groups[SENSOR_GNJ_MODE_LN4];
		break;
	case IS_CIS_LN2:
	case IS_CIS_LNOFF:
	default:
...
int sensor_gnj_cis_check_cropped_remosaic(cis_shared_data *cis_data, unsigned int mode, unsigned int *next_mode)
{
	int ret = 0;
	u32 zoom_ratio = 0;

	if (sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP] == MODE_GROUP_NONE)
		return -EINVAL;

	zoom_ratio = cis_data->cur_remosaic_zoom_ratio;

	if (zoom_ratio != GNJ_REMOSAIC_ZOOM_RATIO_X_2)
		return -EINVAL; //goto default

	*next_mode = sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP];
...
int sensor_gnj_cis_update_seamless_mode(struct v4l2_subdev *subdev)
{
	int ret = 0;
	unsigned int mode = 0;
	...
	next_mode = sensor_gnj_mode_groups[SENSOR_GNJ_MODE_NORMAL];
	...
	if (cis_data->stream_on == false
		&& cis_data->seamless_ctl_before_stream.mode == 0) {
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	sensor_gnj_cis_check_cropped_remosaic(cis->cis_data, mode, &next_mode);
	...
	if (next_mode_info->setfile_fcm.size == 0)
		...
		ret |= cis->ixc_ops->write8(cis->client, 0x0B32, 0x01);
	...
	info("[%s][%d] pre(%d)->cur(%d), 12bit[%d] LN[%d] AEB[%d] ZOOM[%d], time %lldus\n",
		...
		cis_data->cur_hdr_mode, cis_data->cur_remosaic_zoom_ratio,
...
int sensor_gnj_cis_get_seamless_mode_info(struct v4l2_subdev *subdev)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	if (sensor_gnj_mode_groups[SENSOR_GNJ_MODE_LN4] != MODE_GROUP_NONE) {
		...
		sensor_cis_get_mode_info(subdev, sensor_gnj_mode_groups[SENSOR_GNJ_MODE_LN4],
	...
	if (sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG] != MODE_GROUP_NONE) {
		...
		sensor_cis_get_mode_info(subdev, sensor_gnj_mode_groups[SENSOR_GNJ_MODE_IDCG],
			&cis_data->seamless_mode_info[cnt]);
		cnt++;
	}
	...
	if (sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP] != MODE_GROUP_NONE) {
		...
		cis_data->seamless_mode_info[cnt].extra[0] = GNJ_REMOSAIC_ZOOM_RATIO_X_2;
		sensor_cis_get_mode_info(subdev, sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP],
			&cis_data->seamless_mode_info[cnt]);
		cnt++;
	}
...
int sensor_gnj_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	...
	if (ret < 0) {
		err("sensor_gnj_set_registers fail!!");
		goto p_err;
	}

	sensor_gnj_cis_set_mode_group(mode);
	sensor_gnj_cis_get_seamless_mode_info(subdev);
...
int sensor_gnj_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct is_device_sensor *device;
	...
	cis->ixc_ops->write16(cis->client, 0xFCFC, 0x2001);
	cis->ixc_ops->write16(cis->client, 0x7A7E, 0x0000);
...
int sensor_gnj_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	u8 frame_count = 0;
...
int sensor_gnj_cis_set_test_pattern(struct v4l2_subdev *subdev, camera2_sensor_ctl_t *sensor_ctl)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
...
int sensor_gnj_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	int ret = 0;
	int mode = 0;
	...
	if (ret < 0)
		err("sensor_gnj_cis_set_wb_gain fail!!");
...
int sensor_gnj_cis_get_updated_binning_ratio(struct v4l2_subdev *subdev, u32 *binning_ratio)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
	u32 rms_mode = sensor_gnj_mode_groups[SENSOR_GNJ_MODE_RMS_CROP];
	u32 zoom_ratio;
	...
	if (zoom_ratio == GNJ_REMOSAIC_ZOOM_RATIO_X_2 && sensor_gnj_rms_binning_ratio[rms_mode])
		*binning_ratio = sensor_gnj_rms_binning_ratio[rms_mode];

	return 0;
}

int sensor_gnj_cis_wait_seamless_update_delay(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
...
static struct is_cis_ops cis_ops_gnj = {
	.cis_init = sensor_gnj_cis_init,
	.cis_init_state = sensor_cis_init_state,
	.cis_log_status = sensor_gnj_cis_log_status,
	.cis_group_param_hold = sensor_cis_set_group_param_hold,
	.cis_set_global_setting = sensor_gnj_cis_set_global_setting,
	.cis_mode_change = sensor_gnj_cis_mode_change,
	.cis_stream_on = sensor_gnj_cis_stream_on,
	.cis_stream_off = sensor_gnj_cis_stream_off,
	...
	.cis_set_wb_gains = sensor_gnj_cis_set_wb_gain,
	.cis_update_seamless_mode = sensor_gnj_cis_update_seamless_mode,
	...
	.cis_wait_seamless_update_delay = sensor_gnj_cis_wait_seamless_update_delay,
	.cis_get_updated_binning_ratio = sensor_gnj_cis_get_updated_binning_ratio,
	.cis_set_test_pattern = sensor_gnj_cis_set_test_pattern,
};

int cis_gnj_probe_i2c(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	cis->cis_ops = &cis_ops_gnj;
	/* belows are depend on sensor cis. MUST check sensor spec */
	cis->bayer_order = OTF_INPUT_ORDER_BAYER_GR_BG;
	cis->use_wb_gain = true;
	cis->reg_addr = &sensor_gnj_reg_addr;
	...
	cis->sensor_info = &sensor_gnj_info_A;
...
static const struct of_device_id sensor_cis_gnj_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-gnj",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_gnj_match);

static const struct i2c_device_id sensor_cis_gnj_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_gnj_driver = {
	.probe	= cis_gnj_probe_i2c,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_gnj_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_gnj_idt
};

#ifdef MODULE
builtin_i2c_driver(sensor_cis_gnj_driver);
#else
static int __init sensor_cis_gnj_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_gnj_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_gnj_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_gnj_init);
#endif
