...
static int sensor_module_init(struct v4l2_subdev *subdev, u32 val)
{
	int ret = 0;
	struct is_module_enum *module;
	struct is_device_sensor_peri *sensor_peri = NULL;
...
int sensor_module_deinit(struct v4l2_subdev *subdev)
{
	...
	if (sensor_peri->flash != NULL) {
		...
		if (sensor_peri->flash->use_pre_config_work)
			cancel_work_sync(&sensor_peri->flash->flash_data.flash_set_pre_config_work);
	}
...
static int sensor_module_g_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;
	struct is_module_enum *module = NULL;
	struct is_device_sensor *device = NULL;
...
static int sensor_module_s_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;
	struct is_device_sensor *device = NULL;
	struct is_module_enum *module = NULL;
...
#if IS_ENABLED(CONFIG_CAMERA_HW_BIG_DATA)
		is_sec_update_hw_param_af_info(device, ctrl->value, HW_PARAM_STREAMING);
#endif
	...
	case V4L2_CID_SENSOR_SET_FACTORY_CONTROL:
		...
		break;
	case V4L2_CID_SENSOR_SET_STREAM_ON_IN_RC_MODE:
		ret = CALL_CISOPS(&sensor_peri->cis, cis_seamless_ctl_before_stream,
			sensor_peri->subdev_cis, SENSOR_MODE_CROPPED_RMS, ctrl->value);
		info("V4L2_CID_SENSOR_SET_STREAM_ON_IN_RC_MODE : zoom_ratio(%d)\n", ctrl->value);
		break;
#if 0 // TO DO
...
static int sensor_module_g_ext_ctrls(struct v4l2_subdev *subdev, struct v4l2_ext_controls *ctrls)
{
	int ret = 0;
	struct is_module_enum *module;
...
static int sensor_module_s_ext_ctrls(struct v4l2_subdev *subdev, struct v4l2_ext_controls *ctrls)
{
	int ret = 0;
	int i;
	struct is_module_enum *module;
	...
	for (i = 0; i < ctrls->count; i++) {
		...
		case V4L2_CID_SENSOR_SET_TOF_AUTO_FOCUS:
		{
			...
			vendor = &core->vendor;
			is_vendor_store_af(vendor, (struct tof_data_t *)ext_ctrl->ptr);
#endif
			break;
		}
		case V4L2_CID_IS_OBTE_CONFIG_SENSOR:
			ret = pablo_obte_config_sensor(subdev, ext_ctrl);
			if (ret) {
				err("failed to configure OBTE sensor(%d)", ret);
				goto p_err;
			}

			info("OBTE sensor configured (W:%dxH:%d)\n",
							device->obte_sensor->width,
							device->obte_sensor->height);
			break;
...
static long sensor_module_ioctl(struct v4l2_subdev *subdev, unsigned int cmd, void *arg)
{
	int ret = 0;
	struct v4l2_control *ctrl;
...
static int sensor_module_s_routing(struct v4l2_subdev *sd, u32 input, u32 output, u32 config)
{
	int ret = 0;
...
static int sensor_module_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret;
...
static inline void __sensor_module_calculate_data(struct is_cis *cis,
						  struct is_device_sensor *device,
						  struct v4l2_subdev_format *fmt)
{
	cis->cis_data->request_min_frame_us_time = ((1 * 1000 * 1000) / device->image.framerate);
...
static inline bool __sensor_module_is_new_mode(struct is_cis *cis, struct is_device_sensor *device,
					       struct is_device_sensor_peri *sensor_peri)
{
	return (cis->cis_data->sens_config_index_cur != device->cfg->mode) ||
	       (cis->cis_data->sens_config_ex_mode_cur != device->cfg->ex_mode) ||
	       (sensor_peri->mode_change_first == true);
}

static int sensor_module_s_format(
	struct v4l2_subdev *subdev, v4l2_subdev_config_t *cfg, struct v4l2_subdev_format *fmt)
{
	int ret = 0;
	bool new_mode;
	struct is_device_sensor *device = NULL;
	...
	new_mode = __sensor_module_is_new_mode(cis, device, sensor_peri);

	is_sensor_flush_prev_mode_change(cis);

	CALL_CISOPS(cis, cis_check_wdr_mode, sensor_peri->subdev_cis, device->cfg->mode);

	__sensor_module_calculate_data(cis, device, fmt);

	if (new_mode) {
		dbg_sensor(1, "[%s] mode changed(%d:%d->%d:%d)\n", __func__,
			   cis->cis_data->sens_config_index_cur, device->cfg->mode,
			   cis->cis_data->sens_config_ex_mode_cur, device->cfg->mode);

		ret = is_sensor_mode_change(cis);
		if (ret) {
...
static int sensor_module_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_module_enum *module = NULL;
	struct is_device_sensor_peri *sensor_peri = NULL;
...
static void is_hotjoin_chk_dwork_fn(struct work_struct *data)
{
	...
	if (test_bit(IS_MODULE_GPIO_ON, &module->state)) {
		module->pdata->cis_i3c_probed = false;
