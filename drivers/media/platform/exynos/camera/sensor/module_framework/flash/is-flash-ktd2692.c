...
static int __init flash_ktd2692_platform_probe_i2c(struct platform_device *pdev)
{
	int ret = 0;
	struct device *dev;
...
static int __init is_sensor_flash_ktd2692_init_i2c(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_flash_ktd2692_platform_driver,
				flash_ktd2692_platform_probe_i2c);
	if (ret)
...
late_initcall_sync(is_sensor_flash_ktd2692_init_i2c);
