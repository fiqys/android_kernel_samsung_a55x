...
extern int sm5714b_create_sysfs(struct class*);
static struct task_struct *sm5714b_fled_ctrl_thread = NULL;
static int sm5714b_fled_mode_torch_flash(void *data)
{
	sm5714b_fled_mode_ctrl(SM5714B_FLED_MODE_TORCH_FLASH, 0);
	return 0;
}
#endif

static int flash_sm5714b_init(struct v4l2_subdev *subdev, u32 val)
{
	int ret = 0;
...
#if IS_ENABLED(CONFIG_LEDS_SM5714B)
	ret = sm5714b_fled_mode_ctrl(SM5714B_FLED_MODE_OFF, 0);
#else
...
static int sensor_sm5714b_flash_control(struct v4l2_subdev *subdev, enum flash_mode mode, u32 intensity)
{
	int ret = 0;
	...
	if (mode == CAM2_FLASH_MODE_OFF) {
		...
		ret = sm5714b_fled_mode_ctrl(SM5714B_FLED_MODE_MAIN_FLASH, 0);
		if (ret)
			err("%s capture flash on fail", __func__);
	} else if (mode == CAM2_FLASH_MODE_TORCH) {
		sm5714b_fled_ctrl_thread = kthread_run(sm5714b_fled_mode_torch_flash, NULL, "sm5714b_fled_ctrl_thread");
		if (IS_ERR(sm5714b_fled_ctrl_thread)) {
			WARN_ON(1);
			return 0;
		}
	} else {
...
int flash_sm5714b_s_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;
	struct is_flash *flash = NULL;
	...
	case V4L2_CID_FLASH_SET_FIRE:
		ret = sensor_sm5714b_flash_control(subdev, flash->flash_data.mode, ctrl->value);
		if (ret) {
			err("sensor_sm5714b_flash_control(mode:%d, val:%d) is fail(%d)",
...
long flash_sm5714b_ioctl(struct v4l2_subdev *subdev, unsigned int cmd, void *arg)
{
	int ret = 0;
	...
	case SENSOR_IOCTL_FLS_S_CTRL:
		ret = flash_sm5714b_s_ctrl(subdev, ctrl);
		if (ret) {
...
static const struct v4l2_subdev_core_ops core_ops = {
	.init = flash_sm5714b_init,
	.ioctl = flash_sm5714b_ioctl,
};
...
static int flash_sm5714b_probe(struct device *dev, struct i2c_client *client)
{
	int ret = 0;
	struct is_core *core;
	...
	sm5714b_create_sysfs(camera_class);
#endif
...
static int flash_sm5714b_platform_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct device *dev;
	...
	ret = flash_sm5714b_probe(dev, NULL);
	if (ret < 0) {
		probe_err("flash sm5714b probe fail(%d)\n", ret);
		goto p_err;
	}
...
static const struct of_device_id exynos_is_sensor_flash_sm5714b_match[] = {
	{
		.compatible = "samsung,sensor-flash-sm5714b",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_flash_sm5714b_match);

/* register platform driver */
static struct platform_driver sensor_flash_sm5714b_platform_driver = {
	.probe = flash_sm5714b_platform_probe,
	.driver = {
		.name   = "IS-SENSOR-FLASH-SM5714B-PLATFORM",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_flash_sm5714b_match,
	}
};

#ifdef MODULE
builtin_platform_driver(sensor_flash_sm5714b_platform_driver);
#else
static int __init sensor_flash_sm5714b_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_flash_sm5714b_platform_driver,
				flash_sm5714b_platform_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_flash_sm5714b_platform_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_flash_sm5714b_init);
#endif
