...
static int flash_s2mf301_adj_current(struct is_flash *flash, enum flash_mode mode, u32 intensity)
{
	...
	max_current = s2mf301_get_max_current(mode);
	if (max_current < 0) {
		warn("%s cannot get max_current for mode %d", __func__, mode);
		return 0;
	}
...
int flash_s2mf301_g_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;
	struct is_flash *flash = NULL;
	int voltage;
...
long flash_s2mf301_ioctl(struct v4l2_subdev *subdev, unsigned int cmd, void *arg)
{
	...
	case SENSOR_IOCTL_FLS_G_CTRL:
		ret = flash_s2mf301_g_ctrl(subdev, ctrl);
		if (ret) {
