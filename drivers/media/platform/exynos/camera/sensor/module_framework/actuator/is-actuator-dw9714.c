...
static int sensor_dw9714_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = actuator->ixc_ops->data_write16(client, val_high, val_low);
...
int sensor_dw9714_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	/* STEP 1 : Protect Off */
	ret = actuator->ixc_ops->data_write16(client, 0xEC, 0xA3);
	if (ret < 0)
		goto p_err;

	/* SETP 2 : T_SRC setting (default is 0x00) */
	ret = actuator->ixc_ops->data_write16(client, 0xF2, (0x9 << 3));
	if (ret <0)
		goto p_err;

	/* STEP 3 : Protection On */
	ret = actuator->ixc_ops->data_write16(client, 0xDC, 0x51);
	if (ret <0)
		goto p_err;
...
int sensor_dw9714_actuator_get_status(struct v4l2_subdev *subdev, u32 *info)
{
	 ...
	 */
	ret = actuator->ixc_ops->data_read16(client, &val);
...
static int sensor_dw9714_actuator_probe_i2c(struct i2c_client *client,
...
static struct i2c_driver sensor_actuator_dw9714_driver = {
	.probe  = sensor_dw9714_actuator_probe_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
