...
int is_dump_init_reg_dump(u32 count)
{
	...
	lib->dump_to_file.is_dump_set = pablo_zalloc(sizeof(struct is_dump_set) * count,
			GFP_KERNEL);
...
bool is_get_secure_scenario(void)
{
	...
	if (core->resourcemgr.scenario == IS_SCENARIO_SECURE)
		ret = true;
...
static int __nocfi pablo_lib_register_sensor_itf(void *sensor_itf)
{
	int ret;
