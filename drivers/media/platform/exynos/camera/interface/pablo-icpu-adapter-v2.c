...
static inline u32 __pablo_icpu_get_mbox_id(struct icpu_adt_v2 *adt, u32 instance)
{
	u32 ch = adt->get_ch_fn ? adt->get_ch_fn(instance) : 0;
	bool rep = !adt->sensor_adt[instance];

	if (rep)
		return PABLO_RX_MBOX_MAX - 1;
	else
		return ch % PABLO_RX_MBOX_MAX;
}

static inline u32 __pablo_icpu_adt_get_msg(struct icpu_adt_v2 *adt, u32 instance,
	enum pablo_message_type type,
	enum pablo_hic_cmd_id cmd, enum pablo_response_id res)
{
	...
	msg |= SET_MSG_VALUE(__pablo_icpu_get_mbox_id(adt, instance), MSG_MBOX_ID_MASK, MSG_MBOX_ID_SHIFT);
...
static void __pablo_icpu_adt_handle_msg_control_sensor(struct icpu_adt_v2 *adt, u32 instance, u32 *data)
{
	...
	CALL_CRTA_BUFMGR_OPS(adt->bufmgr[instance], get_process_buf, buf_idx, &buf_info);

	/* compare dva */
	dva = GET_DVA(data[CONTROL_SENSOR_BUF_DVA_HIGH], 0);
	if (buf_info.dva != dva) {
...
static void __pablo_icpu_adt_fw_err_handler(void *cookie, char *str)
{
	int i;
...
static void __pablo_icpu_adt_control_sensor_callback(void *caller, u32 instance, u32 ret, dma_addr_t dva, u32 idx)
{
	...
	data[CONTROL_SENSOR_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_RESPONSE, cmd, rsp);
					...
					u32 hw_id, u32 rep_flag, u32 position, u32 f_type)
{
	...
	/* prepare data */
	data[OPEN_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND, cmd, PABLO_RSP_SUCCESS);
						...
						struct pablo_crta_buf_info *buf)
{
	...
	minfo_adt("send_msg_put_buf: type(%s), idx(%d), dva(%pad), kva(0x%p), size(0x%zx)", instance,
		buf_name[type], buf->idx, &buf->dva, buf->kva, buf->size);
	...
	/* prepare data */
	data[PUT_BUF_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
	data[PUT_BUF_TYPE] = SET_MSG_VALUE(buf->idx, BUF_IDX_MASK, BUF_IDX_SHIFT) |
						...
						u32 fcount)
{
	...
	/* prepare data */
	data[SET_BUF_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
					...
					struct pablo_crta_buf_info *pcsi_buf)
{
	...
	minfo_adt("send_msg_start: dva(0x%llx) kva(%p)", instance, pcsi_buf->dva, pcsi_buf->kva);
	...
	ret = CALL_SS_ADT_OPS(adt->sensor_adt[instance], get_sensor_info, pcsi_buf->kva, 0);
	if (ret)
	...
	data[START_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
	...
	u32 fcount, struct pablo_crta_buf_info *shot_buf, struct pablo_crta_buf_info *pcfi_buf)
{
	...
	/* prepare data */
	data[SHOT_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
	...
	u32 fcount, struct pablo_crta_buf_info *pcsi_buf)
{
	...
	/* prepare data */
	ret = CALL_SS_ADT_OPS(adt->sensor_adt[instance], get_sensor_info, pcsi_buf->kva, fcount);
	if (ret)
		merr_adt("failed to sensor_adt get_sensor_info", instance);
	...
	data[FRAME_START_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
...
static int pablo_icpu_adt_send_msg_cstat_cdaf_end(struct pablo_icpu_adt *icpu_adt, u32 instance,
	void *cb_caller, void *cb_ctx, u32 fcount, struct pablo_crta_buf_info *stat_buf)
{
	int ret;
	...
	mdbg_adt(1, "[F%d]send_msg_cdaf_end: stat(0x%llx)", instance,
		fcount, stat_buf->dva);
	...
	/* prepare data */
	/* cmd */
	data[CDAF_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
	/* fcount */
	...
	void *cb_caller, void *cb_ctx, u32 fcount, struct pablo_crta_buf_info *stat_buf)
{
	...
	mdbg_adt(1, "[F%d]send_msg_pdp_stat0_end: stat(0x%llx)", instance,
		fcount, stat_buf->dva);
	...
	/* prepare data */
	data[PDP_STAT0_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
	...
	void *cb_caller, void *cb_ctx, u32 fcount, struct pablo_crta_buf_info *stat_buf)
{
	...
	mdbg_adt(1, "[F%d]send_msg_pdp_stat1_end: stat(0x%llx)", instance,
		fcount, stat_buf->dva);
	...
	/* prepare data */
	data[PDP_STAT1_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
	...
	struct pablo_crta_buf_info *stat_buf, u32 edge_score)
{
	...
	mdbg_adt(1, "[F%d]send_msg_cstat_frame_end: shot(%pad), edge_score(%d)", instance,
		fcount, &shot_buf->dva, edge_score);
	...
	/* prepare data */
	/* cmd */
	data[FRAME_END_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
	/* fcount */
...
static int pablo_icpu_adt_send_msg_stop(struct pablo_icpu_adt *icpu_adt, u32 instance, u32 suspend)
{
	...
	/* prepare data */
	data[STOP_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
...
static int pablo_icpu_adt_send_msg_close(struct pablo_icpu_adt *icpu_adt, u32 instance)
{
	...
	/* prepare data */
	data[CLOSE_CMD] = __pablo_icpu_adt_get_msg(adt, instance, PABLO_MESSAGE_COMMAND,
				       cmd, PABLO_RSP_SUCCESS);
...
static int __pablo_icpu_adt_open_icpu_itf(struct pablo_icpu_adt *icpu_adt, u32 instance, bool secure_mode)
{
	int ret;

	ret = ITF_API(__itf, open, secure_mode);
	if (ret) {
	...
	u32 scenario)
{
	...
	if (!atomic_read(&icpu_adt->rsccount)) {
		...
		secure_mode = scenario == IS_SCENARIO_SECURE;
		ret = __pablo_icpu_adt_open_icpu_itf(icpu_adt, instance, secure_mode);
		if (ret)
			goto err_itf;
	} else {
		adt = (struct icpu_adt_v2 *)icpu_adt->priv;
...
static int pablo_icpu_adt_register_get_ch_fn(struct pablo_icpu_adt *icpu_adt, u32 instance,
	pablo_get_ch_fn cb)
{
	int ret;
	struct icpu_adt_v2 *adt;
...
const static struct pablo_icpu_adt_ops icpu_adt_ops = {
	...
	.register_get_ch_fn	= pablo_icpu_adt_register_get_ch_fn,
};
...
PST_KUNIT_EXPORT_SYMBOL(pablo_get_icpu_adt);

void pablo_set_icpu_adt(struct pablo_icpu_adt *icpu_adt)
{
	__icpu_adt = icpu_adt;
}
PST_KUNIT_EXPORT_SYMBOL(pablo_set_icpu_adt);
