	...
	unsigned char *data, int size)
{
	...
	if (size > MAX_NOTIFIER_BUF_SIZE - 3 || size < 0) {
		MCU_ERR_PRINT("size is not available %d", size);
		return NOTIFY_BAD;
	}
	...
	if (ret & NOTIFY_BAD) {
		MCU_ERR_PRINT("read_reg notifier failed (0x%x)", ret);
		ret = -1;
	} else
		ret = 2;
	...
	unsigned char *data, int size)
{
	...
	if (ret & NOTIFY_BAD) {
		MCU_ERR_PRINT("reg_write notifier failed (0x%x)", ret);
		ret = 0;
	} else
		ret = 1;
...
int cam_ois_factory_mode_notifier_call_chain(unsigned long val, void *v)
{
	...
	if (ret & NOTIFY_BAD) {
		MCU_ERR_PRINT("factory_mode notifier failed (0x%x)", ret);
		ret = 1;
	} else
		ret = 0;
...
int cam_ois_set_fac_mode(enum cam_ois_aois_fac_mode op)
{
	...
	if (ret & NOTIFY_BAD) {
		MCU_ERR_PRINT("notify bad ret = 0x%x", ret);
		ret = -1;
	} else
		ret = 0;
