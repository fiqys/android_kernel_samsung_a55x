...
static int pmio_cache_flat_eager_init(struct pablo_mmio *pmio)
{
	struct pmio_cache_flat_ctx *ctx;
	int ret;

	ret = pmio_cache_flat_init(pmio);
	if (ret)
		return ret;

	ctx = pmio->cache;
	set_bit(PMIO_CACHE_S_SYNC_TO_DEV, &ctx->cache_state);

	return 0;
}

static int pmio_cache_flat_eager_write(struct pablo_mmio *pmio,
				 unsigned int reg, unsigned int value)
{
	struct pmio_cache_flat_ctx *ctx = pmio->cache;
	unsigned int idx = pmio_get_index_by_order(pmio, reg);
	...
	if (ctx->cache[idx] != value) {
		ctx->cache[idx] = value;
		bitmap_set(ctx->cache_dirty_bitmap, idx, 1);
	} else if (test_bit(PMIO_CACHE_S_SYNC_TO_DEV, &ctx->cache_state)) {
		bitmap_set(ctx->cache_dirty_bitmap, idx, 1);
	}
...
static int pmio_cache_flat_eager_sync(struct pablo_mmio *pmio, unsigned int min,
				unsigned int max)
{
	struct pmio_cache_flat_ctx *ctx = pmio->cache;
	int ret;

	ret = pmio_cache_flat_sync(pmio, min, max);
	if (ret)
		return ret;

	clear_bit(PMIO_CACHE_S_SYNC_TO_DEV, &ctx->cache_state);

	return 0;
}

struct pmio_cache_ops pmio_cache_flat_eager_ops = {
	.type = PMIO_CACHE_FLAT_EAGER,
	.name = "flat eager",
	.init = pmio_cache_flat_eager_init,
	.exit = pmio_cache_flat_exit,
	.reset = pmio_cache_flat_reset,
	.read = pmio_cache_flat_read,
	.write = pmio_cache_flat_eager_write,
	.raw_write = pmio_cache_flat_raw_write,
	.sync = pmio_cache_flat_eager_sync,
	.fsync = pmio_cache_flat_fsync,
	.drop = pmio_cache_flat_drop,
};
...
int pmio_cache_init(struct pablo_mmio *pmio, const struct pmio_config *config)
{
	...
	if (!pmio->cache_ops->read || !pmio->cache_ops->write || !pmio->cache_ops->name)
		return -EINVAL;
...
KUNIT_EXPORT_SYMBOL(pmio_cache_init);
...
int pmio_cache_reset(struct pablo_mmio *pmio)
{
	if (pmio->cache_type == PMIO_CACHE_NONE)
		return -EPERM;

	if (pmio->cache_ops && pmio->cache_ops->reset) {
		pr_debug("resetting %s cache for %s\n",
				pmio->cache_ops->name, pmio->name);
		return pmio->cache_ops->reset(pmio);
	}
...
KUNIT_EXPORT_SYMBOL(pmio_cache_read);
...
KUNIT_EXPORT_SYMBOL(pmio_cache_write);
			 ...
			 const void *val, size_t len)
{
	...
	if (!pmio_volatile(pmio, reg)) {
		if (pmio->cache_ops && pmio->cache_ops->raw_write)
...
KUNIT_EXPORT_SYMBOL(pmio_cache_raw_write);
...
KUNIT_EXPORT_SYMBOL(pmio_cache_drop_region);
...
KUNIT_EXPORT_SYMBOL(pmio_cache_mark_dirty);
...
KUNIT_EXPORT_SYMBOL(pmio_cache_lookup_reg);
