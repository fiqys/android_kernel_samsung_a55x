...
const struct pablo_dma_buf_ops pdb_ops = {
		...
		struct is_vb2_buf *vbuf, u32 plane, u32 buf_idx, struct device *dev_ext)
{
	...
	if (dbuf->size < vb->planes[plane].length) {
		pr_err("[P%d]Too small dmabuf. fd %d size %zu < %u", plane, vb->planes[plane].m.fd,
		       dbuf->size, vb->planes[plane].length);
		goto err_size;
	}
...
static long is_dbufcon_prepare(struct is_vb2_buf *vbuf, struct device *dev)
{
	...
	for (p = 0; p < num_i_planes; p++) {
		...
		if (dbufcon->size < vb->planes[p].length) {
			err("[P%d]Too small dmabuf. fd %d size %zu < %u", p,
			    vb->planes[p].m.fd, dbufcon->size, vb->planes[p].length);
			goto err_size_dbufcon;
		}
		...
		for (b = 0, i = p; b < count; b++, i = (b * num_i_planes) + p) {
			...
			if (vbuf->dbuf[i]->size < vb->planes[p].length) {
				err("[I%d][P%d]Too small dmabuf. fd %d size %zu < %u", i, p,
				    vb->planes[p].m.fd, vbuf->dbuf[i]->size, vb->planes[p].length);
				goto err_size_dbuf;
			}
		...
		struct device *dev)
{
	...
	for (p = 0; p < num_p; p++) {
		...
		if (dbuf->size < plane[p].length) {
			err("[V%d][P%d]Too small dmabuf. fd %d size %zu < %u", buf->vid, p,
			    plane[p].m.fd, dbuf->size, plane[p].length);
			goto err_get_dbuf;
		}
		...
		for (b = 0, i = p; b < cnt; b++, i = (num_p * b) + p) {
			...
			if (buf->dbuf[i]->size < plane[p].length) {
				err("[V%d][I%d][P%d]Too small dmabuf. fd %d size %zu < %u",
				    buf->vid, i, p, plane[p].m.fd, buf->dbuf[i]->size,
				    plane[p].length);
				goto err_get_dbuf;
			}
...
#if defined(USE_CAMERA_HEAP)
	dbg_mem(1, "%s: heap[%s] size(%zu) total[%d]\n",
		__func__, pbuf->dma_buf->exp_name, pbuf->size, rmem_allocated);
#else
	dbg_mem(1, "%s: size(%zu)\n", __func__, pbuf->size);
#endif
		...
		size_t size, const char *heapname, unsigned int flags)
{
	...
	if ((pdhc->dh_camera && dh == pdhc->dh_camera) ||
		...
		if (rmem_allocated + size < CAMERA_HEAP_RESERVE_MEM_SIZE) {
			...
			if (IS_ERR(buf->dma_buf)) {
				info("Camera Heap: alloc error, try system heap, size = %zu\n", size);
				...
				rmem_allocated += size;
				info("Camera Heap: alloc success, size = %zu\n total = %d", size, rmem_allocated);
				goto camera_heap_alloc_success;
			}
		} else {
			info("Camera Heap: Kernel Reserve mem limit reached! try system heap, size = %zu\n", size);
			if (dh == pdhc->dh_camera_uncached)
				dh = pdhc->dh_system_uncached;
			else
				dh = pdhc->dh_system;
...
const struct pablo_dmabuf_container_ops pdbc_ops = {
