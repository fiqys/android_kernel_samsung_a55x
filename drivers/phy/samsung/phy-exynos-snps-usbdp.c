...
{
	int usbdp_mode = SNPS_USBDP_ROM_MODE;

	if (info->add_info)
		usbdp_mode = ((struct snps_add_info *)(info->add_info))->usbdp_mode;
...
void phy_exynos_snps_usbdp_tune_each_param(struct exynos_usbphy_info *info, char *name, int val)
{
...
void phy_exynos_snps_usbdp_tune(struct exynos_usbphy_info *info)
{
	...
	for (int cnt = 0; info->tune_param[cnt].value != EXYNOS_USB_TUNE_LAST; cnt++) {
		char *para_name;
		int val;
		...
		if (!para_name)
			break;
		phy_exynos_snps_usbdp_tune_each_param(info, para_name, val);
	}
}
...
u16 phy_exynos_snps_usbdp_get_atb_data(struct exynos_usbphy_info *info, u16 rt_sel, void *atb_item)
{
	...
	// master atb switch for RX block
	//atb_reg = phy_exynos_snps_usbdp_cr_read(info, 0x11fa);
	atb_reg = 0;
	atb_reg |= (1 << 0);
	phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fa), atb_reg);
	phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fc), 0);

	/* select item */
	if (!strcmp(atb_item, "vreg_clk")) {
		phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fa), 0x11);
	} else if (!strcmp(atb_item, "vreg_iqc")) {
		phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fa), 0x21);
	} else if (!strcmp(atb_item, "vreg_vco")) {
		phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fa), 0x09);
	} else if (!strcmp(atb_item, "cdr_vco_vosc")) {
		phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fc), 0x08);
	} else if (!strcmp(atb_item, "vibias_cdr_vco")) {
		phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fc), 0x04);
	} else if (!strcmp(atb_item, "cdr_vco_viref")) {
		phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fc), 0x02);
	} else if (!strcmp(atb_item, "cdr_vco_gd")) {
		phy_exynos_snps_usbdp_cr_write(info, CRREG_LANE_RX(0x10fc), 0x01);
	}

	pr_info("RX_ATB_MEAS1= %x\n", phy_exynos_snps_usbdp_cr_read(info, CRREG_LANE_RX(0x10fa)));
	pr_info("RX_ATB_MEAS3= %x\n", phy_exynos_snps_usbdp_cr_read(info, CRREG_LANE_RX(0x10fc)));

	pr_info("atb_measure = %x\n", atb_reg);
	// setup ip adc
	phy_exynos_snps_usbdp_cr_write(info, 0x41, 0x89);
	phy_exynos_snps_usbdp_cr_write(info, 0x80, 0x00);
	phy_exynos_snps_usbdp_cr_write(info, 0x41, 0xc8);

	// select "P"
	if (rt_sel == 1) {
		phy_exynos_snps_usbdp_cr_write(info, 0x41, 0xca);
	} else {
		// select "N"
		phy_exynos_snps_usbdp_cr_write(info, 0x41, 0xc8);
	}

	for (spl = 0 ; spl<256 ; spl++) {
		/* RTUNE DEBUG */
		phy_exynos_snps_usbdp_cr_write(info, 0x80, 0x02);
		// read rtune value
		rtune_stat[spl] = phy_exynos_snps_usbdp_cr_read(info, 0x82);
		rtune_sum += rtune_stat[spl];
		pr_info(" RTUNE_STAT = %d\n", rtune_stat[spl]);
	...
	return ((rtune_sum/256) * 1200 ) / 1023;
}
