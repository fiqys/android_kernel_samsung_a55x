...
{
	size_t i, row, len;
	int cur = hdr->cur_hw_type;
	const void *in_buf = hdr->base_regs[cur].regs + hdr_dump_off;
	...
	if ((hdr_dump_off + hdr_dump_size) > hdr->base_regs[cur].size)
		return;
				...
				const struct exynos_drm_plane_state *exynos_plane_state)
{
	...
	if (IS_ERR_OR_NULL(buf)) {
		...
		hdr_warn(hdr, "failed to get dma buf [%p] of fd [%lld]\n", buf, hdr_fd);
		goto error;
	}
...
static int hdr_dummy_remap(struct exynos_hdr *hdr)
{
	...
	if (hdr->base_regs[HDR_HW_TYPE_DUMMY].regs) {
		if (hdr->base_regs[HDR_HW_TYPE_DUMMY].addr == addr &&
			hdr->base_regs[HDR_HW_TYPE_DUMMY].size == size) {
			return 0;
		}

		iounmap(hdr->base_regs[HDR_HW_TYPE_DUMMY].regs);
		hdr->base_regs[HDR_HW_TYPE_DUMMY].regs = NULL;
		hdr->base_regs[HDR_HW_TYPE_DUMMY].addr = 0;
		hdr->base_regs[HDR_HW_TYPE_DUMMY].size = 0;
	}

	hdr->base_regs[HDR_HW_TYPE_DUMMY].regs = ioremap(addr, size);
	if (!hdr->base_regs[HDR_HW_TYPE_DUMMY].regs)
		return -EINVAL;
	hdr->base_regs[HDR_HW_TYPE_DUMMY].addr = addr;
	hdr->base_regs[HDR_HW_TYPE_DUMMY].size = size;
	hdr_regs_desc_init(hdr->id, hdr->base_regs[HDR_HW_TYPE_DUMMY].regs, "hdr_dummy", HDR_HW_TYPE_DUMMY);

	hdr->initialized = true;
	hdr_info(hdr, "HDR/WCG initialized (0x%llx, %zu)\n", addr, size);
			...
			struct exynos_drm_plane_state *exynos_plane_state)
{
	...
	coef_h = (struct hdr_coef_header *)ctx;
	coef_h->type.unpack.hw_type = HDR_HW_TYPE_DUMMY;
...
static int hdr_set_cur_hw_type(struct exynos_hdr *hdr, int hw_type)
{
	hdr->cur_hw_type = hw_type;
	return 0;
}

static int hdr_reset_module(struct exynos_hdr *hdr, int hw_type)
{
	int i, cur;
	u32 offset, val;

	if (hdr->cur_hw_type != hw_type) {
		cur = hdr->cur_hw_type;
		for (i = 0; i < hdr->num_regs_reset[cur]; i++) {
			offset = hdr->regs_reset[cur][i].offset;
			val = hdr->regs_reset[cur][i].val;
			hdr_reg_set_lut(hdr->id, cur, offset, val);
		}
	}
	hdr_set_cur_hw_type(hdr, hw_type);
	return 0;
}
			...
			const struct exynos_drm_plane_state *exynos_plane_state)
{
	...
	if (hw_type == HDR_HW_TYPE_NONE || hw_type >= HDR_HW_TYPE_MAX) {
		hdr_err(hdr, "invalid hw type %d\n", coef_h->type.unpack.hw_type);
		goto error_dump;
	}
	if (!hdr_reg_check_hw_type(hdr->id, hw_type)) {
		hdr_err(hdr, "hw type %d not defined nor properly initialized\n", coef_h->type.unpack.hw_type);
		goto error_dump;
	}
	hdr_reset_module(hdr, hw_type);
	...
	while (size < coef_h->total_bytesize) {
		...
		if (((lut_h->byte_offset + lut_size) > hdr->base_regs[hw_type].size) ||
		...
		if (count < coef_h->num.unpack.shall ||
			...
			hdr_reg_set_multiple_lut(id, hw_type, lut_h->byte_offset, lut_h->length, data);
			if (coef_h->type.unpack.log_level >= 10) {
...
static int hdr_remap_regs(struct exynos_hdr *hdr, struct device *dev)
{
	...
	if (i >= 0) {
		...
		hdr->base_regs[HDR_HW_TYPE_BASE].regs = devm_ioremap_resource(dev, &res);
		if (!hdr->base_regs[HDR_HW_TYPE_BASE].regs) {
			hdr_err(hdr, "failed to remap hdr registers\n");
			goto err;
		}
		hdr->base_regs[HDR_HW_TYPE_BASE].addr = (phys_addr_t)res.start;
		hdr->base_regs[HDR_HW_TYPE_BASE].size = resource_size(&res);

		hdr_regs_desc_init(id, hdr->base_regs[HDR_HW_TYPE_BASE].regs, "hdr_lsi", HDR_HW_TYPE_BASE);
		hdr_info(hdr, "hdr_lsi (0x%llx, 0x%zx)\n", hdr->base_regs[HDR_HW_TYPE_BASE].addr, hdr->base_regs[HDR_HW_TYPE_BASE].size);

		hdr->num_regs_reset[HDR_HW_TYPE_BASE] = of_property_count_u32_elems(np, "reset-hdr,hdr_lsi");
		if (hdr->num_regs_reset[HDR_HW_TYPE_BASE] > 0) {
			hdr->regs_reset[HDR_HW_TYPE_BASE] = devm_kzalloc(dev, hdr->num_regs_reset[HDR_HW_TYPE_BASE] * sizeof(u32), GFP_KERNEL);
			if (!(hdr->regs_reset[HDR_HW_TYPE_BASE])) {
				dev_err(dev, "failed to allocate memory for reset-hdr,hdr_lsi\n");
				goto err;
			}
			if (of_property_read_u32_array(np, "reset-hdr,hdr_lsi", (u32 *)hdr->regs_reset[HDR_HW_TYPE_BASE], hdr->num_regs_reset[HDR_HW_TYPE_BASE])) {
				dev_err(dev, "failed to parse reset-hdr,hdr_lsi\n");
				goto err;
			}
		} else {
			hdr->num_regs_reset[HDR_HW_TYPE_BASE] = 0;
			hdr_info(hdr, "no reset-hdr,hdr_lsi exists\n");
		}
	}

	i = of_property_match_string(np, "reg-names", "hdr_custom");
	if (i >= 0) {
		if (of_address_to_resource(np, i, &res)) {
			hdr_err(hdr, "failed to map to resource\n");
			goto err;
		...
		hdr->base_regs[HDR_HW_TYPE_CUSTOM].regs = devm_ioremap_resource(dev, &res);
		if (!hdr->base_regs[HDR_HW_TYPE_CUSTOM].regs) {
			hdr_err(hdr, "failed to remap hdr registers\n");
			goto err;
		}
		hdr->base_regs[HDR_HW_TYPE_CUSTOM].addr = (phys_addr_t)res.start;
		hdr->base_regs[HDR_HW_TYPE_CUSTOM].size = resource_size(&res);

		hdr_regs_desc_init(id, hdr->base_regs[HDR_HW_TYPE_CUSTOM].regs, "hdr_custom", HDR_HW_TYPE_CUSTOM);
		hdr_info(hdr, "hdr_custom (0x%llx, 0x%zx)\n", hdr->base_regs[HDR_HW_TYPE_CUSTOM].addr, hdr->base_regs[HDR_HW_TYPE_CUSTOM].size);

		hdr->num_regs_reset[HDR_HW_TYPE_CUSTOM] = of_property_count_u32_elems(np, "reset-hdr,hdr_custom");
		if (hdr->num_regs_reset[HDR_HW_TYPE_CUSTOM] > 0) {
			hdr->regs_reset[HDR_HW_TYPE_CUSTOM] = devm_kzalloc(dev, hdr->num_regs_reset[HDR_HW_TYPE_CUSTOM] * sizeof(u32), GFP_KERNEL);
			if (!(hdr->regs_reset[HDR_HW_TYPE_CUSTOM])) {
				dev_err(dev, "failed to allocate memory for reset-hdr,hdr_custom\n");
				goto err;
			}
			if (of_property_read_u32_array(np, "reset-hdr,hdr_custom", (u32 *)hdr->regs_reset[HDR_HW_TYPE_CUSTOM], hdr->num_regs_reset[HDR_HW_TYPE_CUSTOM])) {
				dev_err(dev, "failed to parse reset-hdr,hdr_custom\n");
				goto err;
			}
		} else {
			hdr->num_regs_reset[HDR_HW_TYPE_CUSTOM] = 0;
			hdr_info(hdr, "no reset-hdr,hdr_custom exists\n");
		}
	}
...
err:
	if (hdr->base_regs[HDR_HW_TYPE_BASE].regs)
		iounmap(hdr->base_regs[HDR_HW_TYPE_BASE].regs);

	if (hdr->base_regs[HDR_HW_TYPE_CUSTOM].regs)
		iounmap(hdr->base_regs[HDR_HW_TYPE_CUSTOM].regs);
...
static int hdr_init_resources(struct exynos_hdr *hdr, struct device *dev)
{
	...
	hdr_set_cur_hw_type(hdr, HDR_HW_TYPE_NONE);
	...
	hdr->dev = device_create(hdr_cls, NULL, 0, &hdr, "%s", name);
...
{
	if (hdr->cur_hw_type >= HDR_HW_TYPE_MAX) {
		hdr_err(hdr, "invalid cur_hw_type [%d]\n", hdr->cur_hw_type);
		return;
	}
	if (!hdr->base_regs[hdr->cur_hw_type].regs) {
		hdr_err(hdr, "null base reg: cur_hw_type: [%d]\n", hdr->cur_hw_type);
		return;
	}

	__hdr_dump(hdr->id, &hdr->base_regs[hdr->cur_hw_type]);
}
...
struct exynos_hdr *exynos_hdr_register(struct dpp_device *dpp)
{
	...
	hdr->funcs = &hdr_funcs;
	hdr->state = HDR_STATE_DISABLE;
	if (hdr->base_regs[HDR_HW_TYPE_BASE].regs || hdr->base_regs[HDR_HW_TYPE_CUSTOM].regs) {
		hdr->initialized = true;
