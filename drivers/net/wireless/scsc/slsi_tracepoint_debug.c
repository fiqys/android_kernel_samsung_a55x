...
void qdisc_create_callback(void *ignore, const struct Qdisc_ops *ops, struct net_device *dev, unsigned int parent)
{
	struct slsi_dev *sdev = NULL;
	struct netdev_vif *ndev_vif = NULL;

	SLSI_UNUSED_PARAMETER(parent);
	...
	if (!sdev) {
		SLSI_ERR_NODEV("sdev is null\n");
		return;
	}

	if (memcmp(dev->name, "wlan0", 5) && memcmp(dev->name, "p2p-", 4) && memcmp(dev->name, "aware", 5)) {
		SLSI_ERR_NODEV("netdev %s is not proper for traffic control\n", dev->name);
		return;
	}

	SLSI_INFO_NODEV("qdisc created (iface:%s qdisc:%s)\n", dev->name, ops->id);
#ifdef CONFIG_SCSC_WLAN_RX_NAPI_GRO
	write_lock_bh(&ndev_vif->gro_lock);
	if (!strcmp(ops->id, "htb")) {
		ndev_vif->gro_enabled = SLSI_GRO_DISABLED;
		SLSI_INFO_NODEV("GRO disabled (vif_index:%d)\n", ndev_vif->ifnum);
	}
	write_unlock_bh(&ndev_vif->gro_lock);
#endif
}

void qdisc_destroy_callback(void *ignore, struct Qdisc *qdisc)
{
	struct slsi_dev *sdev = NULL;
	struct netdev_vif *ndev_vif = NULL;
	struct net_device *dev = NULL;

	dev = qdisc->dev_queue->dev;
	...
	SLSI_INFO_NODEV("qdisc destroyed (iface:%s qdisc:%s)\n", dev->name, qdisc->ops->id);
#ifdef CONFIG_SCSC_WLAN_RX_NAPI_GRO
	write_lock_bh(&ndev_vif->gro_lock);
	if (!strcmp(qdisc->ops->id, "htb")) {
		ndev_vif->gro_enabled = SLSI_GRO_ENABLED;
		SLSI_INFO_NODEV("GRO enabled (vif_index:%d)\n", ndev_vif->ifnum);
	}
	write_unlock_bh(&ndev_vif->gro_lock);
#endif
}

int register_tc_debugpoints(void)
{
	int ret = 0;

	ret = slsi_register_tracepoint_callback("qdisc_create", qdisc_create_callback, NULL);
	if (ret < 0) {
		SLSI_ERR_NODEV("Register Failed:qdisc_create %d\n", ret);
		return ret;
	}

	ret = slsi_register_tracepoint_callback("qdisc_destroy", qdisc_destroy_callback, NULL);
	if (ret < 0) {
		SLSI_ERR_NODEV("Register Failed:qdisc_destroy %d\n", ret);
		slsi_unregister_tracepoint_callback("qdisc_create", qdisc_create_callback);
		return ret;
	}
...
void unregister_tc_debugpoints(void)
{
	slsi_unregister_tracepoint_callback("qdisc_create", qdisc_create_callback);
	slsi_unregister_tracepoint_callback("qdisc_destroy", qdisc_destroy_callback);
}
