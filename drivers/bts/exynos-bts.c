...
int bts_update_type(unsigned int index, unsigned int info_val)
{
	...
	enable = BTS_CMD_SET(info_val, BTS_ONE_BIT_MASK, BTS_HINT_EN_SHIFT);
	bpc = BTS_CMD_SET(info_val, BTS_ONE_BIT_MASK, BTS_HINT_BPC_SHIFT);
...
int bts_update_bw(unsigned int index, struct bts_bw bw)
{
	...
	if (btsdev->btsbw_ops) {
		BTSDBG_LOG(btsdev->dev, "[before]read: %d, write: %d\n", btsdev->bts_bw[index].read, btsdev->bts_bw[index].write);
		btsdev->btsbw_ops(btsdev, index);
		BTSDBG_LOG(btsdev->dev, "[after]read: %d, write: %d\n", btsdev->bts_bw[index].read, btsdev->bts_bw[index].write);
	}
	spin_unlock(&btsdev->lock);
...
int bts_del_scenario(unsigned int index)
{
	...
	if (scen[index].usage_count < 0) {
		dev_warn(btsdev->dev, "Scen[%u] %s: Usage count is below 0!\n",
				index, scen[index].name);
...
static int exynos_bts_hwstatus_open_show(struct seq_file *buf, void *d)
{
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if (info.ops->get_bts != NULL && (info.type == IP_BTS || info.type == SCI_GPU_BTS)) {
			spin_lock(&btsdev->lock);
			ret = info.ops->get_bts(info.va_base, &stat);
			spin_unlock(&btsdev->lock);

			if (!ret) {
				seq_printf(buf, "[%d] %s:\tARQOS 0x%X, AWQOS 0x%X, RMO 0x%.4X, WMO 0x%.4X, "\
					"QUR(0x%X) TH_R 0x%.2X, TH_W 0x%.2X, ",
					i, info.name, stat.arqos, stat.awqos, stat.rmo, stat.wmo,
...
static int exynos_bts_bw_open_show(struct seq_file *buf, void *d)
{
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if(bw->name == NULL)
			continue;
		seq_printf(buf, "[%s] R: %u,\t W: %u,\t Peak: %u\n",
					bw->name, bw->read, bw->write, bw->peak);
	}

	mif_freq = (btsdev->total_bw * 100) / bw_params->mif_bus_width / bw_params->mif_util;
	int_freq = (btsdev->peak_bw * 100) / bw_params->bus_width / bw_params->int_util;

	seq_printf(buf, "total_bw: %u, peak_bw: %u\n", btsdev->total_bw, btsdev->peak_bw);
...
static int exynos_bts_bw_qos_open(struct inode *inode, struct file *file)
{
	return single_open(file, exynos_bts_bw_open_show, inode->i_private);
}
...
static int exynos_bts_trex_open_show(struct seq_file *buf, void *d)
{
	struct bts_info info;
	int ret, i = 0;
	unsigned int reg[5];
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if(info.ops->get_trexbts != NULL){
			spin_lock(&btsdev->lock);
			ret = info.ops->get_trexbts(info.va_base, reg);
			spin_unlock(&btsdev->lock);

			if(!ret)
				seq_printf(buf, "[%d] %s\n \tSCI_CTRL:0x%.8X\n \tTH_IMM_R:0x%.8X\n "\
						"\tTH_IMM_W:0x%.8X\n \tTH_HIGH_R:0x%.8X\n \tTH_HIGI_W:0x%.8X\n",
						i, info.name, reg[0], reg[1], reg[2], reg[3], reg[4]);
		}
	}
...
static int exynos_bts_trex_open(struct inode *inode, struct file *file)
{
	return single_open(file, exynos_bts_trex_open_show, inode->i_private);
}

static int exynos_bts_smc_open_show(struct seq_file *buf, void *d)
{
	struct bts_info info;
	int ret, i = 0;
	unsigned int reg;

	seq_printf(buf, "read smc config\n");
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if(info.ops->get_smcbts != NULL){
			spin_lock(&btsdev->lock);
			ret = info.ops->get_smcbts(info.va_base, &reg);
			spin_unlock(&btsdev->lock);
...
static int exynos_bts_smc_open(struct inode *inode, struct file *file)
{
	return single_open(file, exynos_bts_smc_open_show, inode->i_private);
}

static int exynos_bts_busc_open_show(struct seq_file *buf, void *d)
{
	struct bts_info info;
	int ret, i = 0;
	unsigned int reg[2];

	seq_printf(buf, "read busc config\n");
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if(info.ops->get_buscbts != NULL){
			spin_lock(&btsdev->lock);
			ret = info.ops->get_buscbts(info.va_base, reg);
			spin_unlock(&btsdev->lock);

			if(!ret)
				seq_printf(buf, "[%d]%s\n \tQMAX_THRESHOLD_R:0x%.8X\n \tQMAX_THRESHOLD_W:0x%.8X\n",
						i, info.name, reg[0], reg[1]);
		}
	}
...
static int exynos_bts_busc_open(struct inode *inode, struct file *file)
{
	return single_open(file, exynos_bts_busc_open_show, inode->i_private);
}

static int exynos_bts_sci_open_show(struct seq_file *buf, void *d)
{
	struct bts_info info;
	struct bts_stat stat;
	int ret, i = 0;

	seq_printf(buf, "read sci config\n");
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if(info.ops->get_bts != NULL && info.type == SCI_BTS){
			spin_lock(&btsdev->lock);
			ret = info.ops->get_bts(info.va_base, &stat);
			spin_unlock(&btsdev->lock);

			if(!ret)
				seq_printf(buf, "[%d]%s\n \tHurryLevel3MO_0:0x%.8X\n \tHurryLevel3MO_1:0x%.8X\n",
						i, info.name, stat.hurrylevel3mo_0, stat.hurrylevel3mo_1);
		}
	}
...
static int exynos_bts_sci_open(struct inode *inode, struct file *file)
{
	return single_open(file, exynos_bts_sci_open_show, inode->i_private);
}

static ssize_t exynos_bts_sci_write(struct file *file, const char __user *user_buf,
					size_t count, loff_t *ppos)
{
	char buf[64];
	ssize_t buf_size;
	...
	struct bts_info info;
	int i, ret, scen, hl3mo_0, hl3mo_1;
	...
	ret = sscanf(buf, "%d %d %d\n", &scen, &hl3mo_0, &hl3mo_1);
	if (ret != 3) {
		pr_err("%s: sscanf failed. We need 3 inputs. count=(%d)\n",
			__func__, ret);
		return -EINVAL;
	}
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if(info.ops->set_bts != NULL && info.pd_on && info.type == SCI_BTS){
			spin_lock(&btsdev->lock);
			info.stat[scen].stat_on = true;
			info.stat[scen].hurrylevel3mo_0 = hl3mo_0;
			info.stat[scen].hurrylevel3mo_1 = hl3mo_1;

			ret = info.ops->set_bts(info.va_base, &info.stat[scen]);
			spin_unlock(&btsdev->lock);
		}
	}
...
#if IS_ENABLED(CONFIG_SUPPORT_DREX_BTS)
...
static const struct file_operations debug_bts_trex_fops = {
	.open		= exynos_bts_trex_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static const struct file_operations debug_bts_smc_fops = {
	.open		= exynos_bts_smc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static const struct file_operations debug_bts_busc_fops = {
	.open		= exynos_bts_busc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static const struct file_operations debug_bts_sci_fops = {
	.open		= exynos_bts_sci_open,
	.read		= seq_read,
	.write		= exynos_bts_sci_write,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
int exynos_bts_debugfs_init(void)
{
	...
	debugfs_create_file("qos", 0440, den, NULL,
				&debug_bts_qos_fops);
	debugfs_create_file("bw", 0440, den, NULL,
				&debug_bts_bw_fops);
	...
	debugfs_create_file("log", 0440, den, NULL,
				&debug_bts_log_fops);
	debugfs_create_file("trex", 0440, den, NULL,
				&debug_bts_trex_fops);
	debugfs_create_file("smc", 0440, den, NULL,
				&debug_bts_smc_fops);
	debugfs_create_file("busc", 0440, den, NULL,
				&debug_bts_busc_fops);
	debugfs_create_file("sci", 0440, den, NULL,
				&debug_bts_sci_fops);
#if IS_ENABLED(CONFIG_SUPPORT_DREX_BTS)
...
static int exynos_bts_suspend(struct device *dev)
{
	return 0;
}
...
static int exynos_bts_resume(struct device *dev)
{
	...
	spin_unlock(&btsdev->lock);
	pr_info("%s: restore_done\n", __func__);

	return 0;
}

/* Syscore operation */
static struct dev_pm_ops exynos_bts_pm_ops = {
	SET_NOIRQ_SYSTEM_SLEEP_PM_OPS(exynos_bts_suspend, exynos_bts_resume)
};
...
static int bts_parse_setting(struct device_node *np, struct bts_stat *stat)
{
	...
	if (stat->stat_on) {
		...
		if (of_property_read_bool(np, "odd_qurgent_offset"))
			stat->odd_qurgent_offset = true;
		...
		if (of_property_read_u32(np, "hurrylevel3mo_0", &(stat->hurrylevel3mo_0)))
			stat->hurrylevel3mo_0 = MAX_MO_SCI;
		if (of_property_read_u32(np, "hurrylevel3mo_1", &(stat->hurrylevel3mo_1)))
			stat->hurrylevel3mo_1 = MAX_MO_SCI;
...
#if defined(CONFIG_EXYNOS_ESCA) || defined(CONFIG_EXYNOS_ESCA_MODULE) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2) || \
	defined(CONFIG_EXYNOS_USE_SCI_LITE)
static void exynos_bts_base_cmd(struct exynos_bts_cmd_info *cmd_info,
					unsigned int *cmd)
{
	cmd[0] |= BTS_CMD_SET(cmd_info->cmd_index,
				BTS_CMD_IDX_MASK, BTS_CMD_IDX_SHIFT);
	cmd[0] |= BTS_CMD_SET(cmd_info->direction,
				BTS_ONE_BIT_MASK, BTS_IPC_DIR_SHIFT);
	cmd[0] |= BTS_CMD_SET(cmd_info->data,
				BTS_DATA_MASK, BTS_DATA_SHIFT);
}

static int __exynos_bts_ipc_send_data(enum exynos_bts_cmd_index cmd_index,
				struct bts_device *data,
				unsigned int *cmd)
{
#if defined(CONFIG_EXYNOS_ACPM) || defined(CONFIG_EXYNOS_ACPM_MODULE) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	struct ipc_config config;
	unsigned int *bts_cmd;
#endif
	int ret = 0;

	if (cmd_index >= BTS_CMD_MAX) {
		pr_err("%s: Invalid CMD Index: %u\n", __func__, cmd_index);
		ret = -EINVAL;
		goto out;
	}
...
#endif
	if (ret) {
		pr_err("%s: Failed to send IPC(%d:%u) data\n",
...
static int exynos_bts_ipc_send_data(enum exynos_bts_cmd_index cmd_index,
				struct bts_device *data,
				unsigned int *cmd)
{
	return __exynos_bts_ipc_send_data(cmd_index, data, cmd);
}

static int exynos_bts_vch_pd_sync(struct bts_device *data,
					enum exynos_bts_ipc_dir direction,
					unsigned int *cal_pdid)
{
	struct exynos_bts_cmd_info cmd_info;
	...
	cmd_info.cmd_index = BTS_VCH_SET;
	cmd_info.direction = direction;

	exynos_bts_base_cmd(&cmd_info, cmd);

	/* send command for BTS */
	pr_info("%s: cal-pdid: 0x%x\n", __func__, *cal_pdid);
	ret = exynos_bts_ipc_send_data(cmd_info.cmd_index, data, cmd);
	if (ret) {
		pr_err("%s: Failed send data\n", __func__);
		goto out;
	}
out:
	return ret;
...
int bts_vch_pd_sync(unsigned int cal_pdid, bool on)
{
	int ret;
	unsigned long flags;

	spin_lock_irqsave(&btsdev->lock, flags);
	ret = exynos_bts_vch_pd_sync(btsdev, BTS_IPC_SET, &cal_pdid);
	spin_unlock_irqrestore(&btsdev->lock, flags);
...
static int bts_parse_data(struct device_node *np, struct bts_device *data)
{
	...
	if (of_have_populated_dt()) {
		...
		if (size < 0) {
			pr_err("%s: Failed to get number of CAL IDs for virtual channel settings\n", __func__);
			ret = size;
			goto err;
		}
		data->vch_size = size;
		...
		for (i = 0; i < data->vch_size; i++) {
			...
			if (ret) {
				pr_err("%s: Failed to get vch_pd_calid(index: %d)\n",
						__func__, i);
				goto err;
			}
		}
		...
		if (data->vch_size)
			exynos_cal_pd_sci_sync = bts_vch_pd_sync;
#endif /* CONFIG_EXYNOS_ESCA */
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	/* esca_ipc_request_channel */
	ret = esca_ipc_request_channel(btsdev->dev->of_node, NULL,
				&btsdev->ipc_ch_num, &btsdev->ipc_ch_size);

	pr_info("%s: esca request channel, ipc_ch: %u, size: %u\n",
				__func__, btsdev->ipc_ch_num, btsdev->ipc_ch_size);
	if (ret) {
		pr_info("%s: esca request channel is failed, ipc_ch: %u, size: %u\n",
				__func__, btsdev->ipc_ch_num, btsdev->ipc_ch_size);
		return ret;
	}
#elif defined(CONFIG_EXYNOS_ESCA) || defined(CONFIG_EXYNOS_ESCA_MODULE) || \
	defined(CONFIG_EXYNOS_USE_SCI_LITE)
	/* acpm_ipc_request_channel */
	ret = acpm_ipc_request_channel(btsdev->dev->of_node, NULL,
				&btsdev->ipc_ch_num, &btsdev->ipc_ch_size);

	pr_info("%s: acpm request channel, ipc_ch: %u, size: %u\n",
				__func__, btsdev->ipc_ch_num, btsdev->ipc_ch_size);
	if (ret) {
		pr_info("%s: acpm request channel is failed, ipc_ch: %u, size: %u\n",
				__func__, btsdev->ipc_ch_num, btsdev->ipc_ch_size);
		return ret;
	}
#endif
