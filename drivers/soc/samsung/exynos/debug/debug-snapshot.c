				  ...
				  unsigned long size)
{
	...
	if (!paddr || !size) {
		dev_err(dss_desc.dev, "Invalid address(%lx) or size(0x%zx)\n",
				paddr, size);
		return -EINVAL;
	}
...
static void dbg_snapshot_set_sjtag_status(void)
{
	...
	val = res.a0;
	if (!!val) {
		/* check SJTAG unlock status */
		arm_smccc_smc(SMC_CMD_GET_SJTAG_STATUS, 0x15, 0, 0, 0, 0, 0, 0, &res);
		val = !(res.a0 & BIT(8));
	}
	dss_desc.sjtag_status = (int)val;
...
static void dbg_snapshot_kevents_setup(void)
{
	...
	pr_info("dbg_snapshot_log struct size: %luKB\n", size / SZ_1K);
}
...
static void *_dbg_snapshot_rmem_set_vmap(struct reserved_mem *rmem)
{
	...
	vaddr = vmap(pages, page_size, VM_MAP, prot);
	kfree(pages);
...
static void __maybe_unused exynos_arm64_serror_panic_rvh(void *data, struct pt_regs *regs, unsigned long esr)
{
	console_unlock();
}
...
static int dbg_snapshot_probe(struct platform_device *pdev)
{
	...
	if (register_trace_android_rvh_arm64_serror_panic(exynos_arm64_serror_panic_rvh, NULL))
		dev_err(dss_desc.dev, "Failed to register_trace_android_rvh_gic_v3_set_affinity\n");
#endif

	if (register_dss_builtin_wdt(&pdev->dev))
		dev_err(dss_desc.dev, "Failed to register dss builtin wdt\n");
