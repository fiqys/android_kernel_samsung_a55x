...
static void simulate_IRQ_OVERUTIL(char **argv, int argc);
...
enum {
	...
	FORCE_IRQ_OVERUTIL,		/* IRQ OVERUTIL */
...
struct force_error force_error_vector = {
	.item = {
		...
		{"irqoverutil",	&simulate_IRQ_OVERUTIL},
...
static int str_to_num(const char *s)
{
	...
	if (!s) {
		pr_err("%s: null string\n", __func__);
		return -1;
	}

	ret = kstrtoint(s, 0, &num);
	if (ret < 0) {
		pr_err("%s: fail to parse(%d): %s\n", __func__, ret, s);
		return -1;
	}
...
static void simulate_WP(char **argv, int argc)
{
	...
	np = of_find_node_by_name(NULL, "exynos-debug-test");
	if (!np) {
		pr_err("There is no exynos-debug-test node\n");
		return;
	}

	ret = of_property_read_u32(np, "ps_hold_control_offset",
			&ps_hold_control_offset);
	if (ret) {
		pr_err("no data(ps_hold_control offset)\n");
		return;
	}

	exynos_pmu_read(ps_hold_control_offset, &ps_hold_control);
	exynos_pmu_write(ps_hold_control_offset, ps_hold_control & 0xFFFFFEFF);
}
...
struct overutil_param {
	unsigned long latency;
	unsigned long period;
};

static void simulate_IRQ_OVERUTIL_handler(void *info)
{
	unsigned long latency = *((unsigned long *)info);

	mdelay(latency);
}

static int secdbg_irq_overutil_task(void *param_info)
{
	struct overutil_param *param = (struct overutil_param *)param_info;

	pr_info("start %s\n", __func__);

	while (!kthread_should_stop()) {
		pr_crit("%s() generate irq [latency:%lu duration:%lu]\n", __func__, param->latency, param->period);
		smp_call_function_single(0,
			simulate_IRQ_OVERUTIL_handler, &param->latency, 0);
		schedule_timeout_interruptible(msecs_to_jiffies(param->period));
	}
...
static void simulate_IRQ_OVERUTIL(char **argv, int argc)
{
	int ret;
	struct task_struct *tsk;
	static struct overutil_param param_info = {
		.latency = 20,
		.period = 10
	};
	...
	if (argc >= 2) {
		ret = kstrtoul(argv[0], 10, &param_info.latency);
		if (ret == 0)
			ret = kstrtoul(argv[1], 10, &param_info.period);
	}

	tsk = kthread_create(secdbg_irq_overutil_task, &param_info, "irq_overutil_task");
	if (IS_ERR(tsk)) {
		pr_err("Failed to create thread\n");
		return;
	}

	pr_info("%s: Created: %s:%d\n", __func__, tsk->comm, task_pid_nr(tsk));
...
{
	struct task_struct __maybe_unused *tsk = current;
...
static noinline void secdbg_test_stack_corruption_type0(unsigned long cdata)
{
...
static noinline void secdbg_test_stack_corruption_type1(unsigned long cdata)
{
...
#if 0 /* TODO: error: exynos_mach_restart undefined */
...
static void simulate_POWER_OFF(char **argv, int argc)
{
#if 0 /* TODO: error: sec_power_off_notifier_list undefined */
	int enable = 0;
	int ret;
...
#else
	pr_crit("%s: not supported temporarily\n", __func__);
#endif
}
/* for debugging power off */
...
struct fpsimd_crash_info {
	bool use_neon;
	unsigned int timeout_ms;
	struct task_struct *task;
};

static void secdbg_fpsimd_make_mess(struct user_fpsimd_state *state)
{
	int i;
...
static int secdbg_fpsimd_crash_task(void *info)
{
	struct fpsimd_crash_info *cinfo = (struct fpsimd_crash_info *)info;

	pr_info("start %s\n", __func__);

	while (!kthread_should_stop()) {
		struct user_fpsimd_state this_state;
		...
		if (cinfo->use_neon)
			kernel_neon_begin();
		secdbg_fpsimd_save_state(&this_state);

		secdbg_fpsimd_make_mess(&this_state);

		secdbg_fpsimd_load_state(&this_state);

		pr_crit_ratelimited("modified\n");
		if (cinfo->use_neon)
			kernel_neon_end();
		preempt_enable();

		schedule_timeout_interruptible(
			msecs_to_jiffies(cinfo->timeout_ms));
	}
...
static void simulate_FPSIMD_CORRUPT(char **argv, int argc)
{
	static struct fpsimd_crash_info cinfo = {
		.use_neon = false,
		.timeout_ms = 0,
	};
	struct task_struct *tsk;
	...
	if (argc > 0) {
		ret = strncmp(argv[0], "neon", 5);
		if (!ret)
			cinfo.use_neon = true;
		else
			cinfo.use_neon = false;
	}

	if (argc > 1) {
		ret = kstrtouint(argv[1], 0, &cinfo.timeout_ms);
		if (ret)
			pr_err("%s: failed to parse: %s\n", __func__, argv[1]);
	}

	pr_info("%s: use_neon:%d, timeout:%ums\n", __func__,
				cinfo.use_neon, cinfo.timeout_ms);

	if (cinfo.task) {
		pr_info("%s: Try to stop: %s:%d\n", __func__,
			cinfo.task->comm, task_pid_nr(cinfo.task));
		ret = kthread_stop(cinfo.task);
		pr_info("%s: Exit: %d\n", __func__, ret);
		cinfo.task = NULL;
	} else if (argc > 0) {
		tsk = kthread_create(secdbg_fpsimd_crash_task, &cinfo, "fpsimd_crash_task");
		if (IS_ERR(tsk)) {
			pr_err("Failed to create thread\n");
			return;
		}
...
static void __test_cfi_func(void)
{
	pr_info("%s: run\n", __func__);
}
...
struct cfi_test cfi_test_block = {
	.test_func = __test_cfi_func,
};

static void simulate_CFI(char **argv, int argc)
{
	pr_crit("%s: org %pS\n", __func__, cfi_test_block.test_func);

	cfi_test_block.test_func -= 0x4;

	pr_crit("%s: call %pS\n", __func__, cfi_test_block.test_func);
	cfi_test_block.test_func();
	pr_crit("%s: end\n", __func__);
