...
static irqreturn_t vts_record_period_elapsed_handler(int irq, void *dev_id)
{
	...
	if (data->mic_ready & (0x1 << VTS_MICCONF_FOR_RECORD)) {
		...
		vts_dev_dbg(dev, "%s:[%s] 0x%x:0x%x\n",
			 __func__,
			 (dma_data->id ? "REC" : "TRI"),
			 (data->dma_area_vts + DMA_BUF_REC_OFFSET), pointer);

		if (pointer)
			dma_data->pointer = (pointer -
				(data->dma_area_vts + DMA_BUF_REC_OFFSET));
		vts_ipc_ack(dev, 1);
#ifdef DEF_VTS_PCM_DUMP
		if (vts_pcm_dump_get_file_started(PCM_DUMP_NODE_REC))
			vts_pcm_dump_period_elapsed(PCM_DUMP_NODE_REC, dma_data->pointer);
#endif
		snd_pcm_period_elapsed(dma_data->substream);
	}
...
static irqreturn_t vts_internal_rec_elapsed_handler(int irq, void *dev_id)
{
	...
	vts_mailbox_read_shared_register(data->pdev_mailbox, &pointer, 1, 1);

	if (pointer && ((pointer - base_addr) >= 0))
...
static int vts_do_reg(struct device *dev, bool enable, int cmd) {
	...
	if (IS_ENABLED(CONFIG_SOC_S5E8825) || IS_ENABLED(CONFIG_SOC_S5E9935)
			|| IS_ENABLED(CONFIG_SOC_S5E8835) || IS_ENABLED(CONFIG_SOC_S5E9945) || IS_ENABLED(CONFIG_SOC_S5E8845)) {
		if (enable)
...
#if defined(CONFIG_SOC_S5E8835) || defined(CONFIG_SOC_S5E8845)
static ssize_t vts_dmic_ahb_dump_show(struct device *dev,
               struct device_attribute *attr, char *buf)
{
	if(vts_is_on()) {
		return vts_soc_dmic_ahb_dump_show(dev, attr, buf);
	} else {
		char *pbuf = buf;
		pbuf += sprintf(pbuf, "Warning: VTS Dmic AHB registers only can dump when vts is on.\n");
...
#endif
#if defined(CONFIG_SOC_S5E8835) || defined(CONFIG_SOC_S5E8845)
static DEVICE_ATTR_RO(vts_dmic_ahb_dump);
#endif
...
static int samsung_vts_probe(struct platform_device *pdev)
{
	...
	data->sm_info.max_sz = data->sm_info.max_sz;
...
#if defined(CONFIG_SOC_S5E8835) || defined(CONFIG_SOC_S5E8845)
	result = device_create_file(dev, &dev_attr_vts_dmic_ahb_dump);
	if (result < 0)
		vts_dev_warn(dev, "Failed to create file: %s\n",
			"vts_dmic_ahb_dump");
#endif
