...
ssize_t vts_soc_dmic_ahb_dump_show(struct device *dev,
               struct device_attribute *attr, char *buf)
{
	struct vts_data *data = dev_get_drvdata(dev);
	char *pbuf = buf;
	...
	pbuf += sprintf(pbuf, "VTS Dmic_ahb0 registers dump:\n");
	for(i=0; i<sizeof(vts_dmic_ahb0_offset)/sizeof(unsigned int); i++) {
		pbuf += sprintf(pbuf, "offset = 0X%8x : 0X%08x\n", vts_dmic_ahb0_offset[i], readl(data->dmic_ahb0_base + vts_dmic_ahb0_offset[i]));
	}
