...
static int aw_dev_reg_fw_update(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < data_len; i += 2) {
		...
		if (reg_addr == aw_dev->volume_desc.reg) {
			if (aw_dev->dv_control_flag) {
				reg_val &= aw_dev->volume_desc.mask;
				reg_val |= (aw_dev->dv_control_volume_set << aw_dev->volume_desc.shift);
			}
			aw_dev->volume_desc.ctl_volume = (reg_val & (~aw_dev->volume_desc.mask)) >> aw_dev->volume_desc.shift;
		}
...
static void aw_dev_fade_in(struct aw_device *aw_dev)
{
	...
	if (fade_step == 0 || g_fade_in_time == 0) {
		aw_dev->ops.aw_set_volume(aw_dev, desc->ctl_volume);
		return;
	}
	/*volume up*/
	for (i = desc->mute_volume; i >= desc->ctl_volume; i -= fade_step) {
		aw_dev->ops.aw_set_volume(aw_dev, i);
		usleep_range(g_fade_in_time, g_fade_in_time + 10);
	}
	if (i != desc->ctl_volume)
		aw_dev->ops.aw_set_volume(aw_dev, desc->ctl_volume);
}
...
void aw_dev_mute(struct aw_device *aw_dev, bool mute)
{
	...
	if (aw_dev->mute_flag) {
		...
		aw_dev_info(aw_dev->dev, "enter, mute_flag is true, force mute: %d", mute_flag);
	} else {
		aw_dev_dbg(aw_dev->dev, "enter, mute: %d", mute_flag);
	}
...
int aw_device_init(struct aw_device *aw_dev, struct aw_container *aw_cfg)
{
	...
	aw_dev->cur_prof = AW_PROFILE_MAX;
...
int aw_device_start(struct aw_device *aw_dev)
{
	...
	if (aw_dev->dv_control_flag) {
		vol_desc->ctl_volume = aw_dev->dv_control_volume_set;
		aw_dev->ops.aw_set_volume(aw_dev, vol_desc->ctl_volume);
	}
