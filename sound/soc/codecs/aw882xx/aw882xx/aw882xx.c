...
#define AW882XX_DRIVER_VERSION "v1.7.0.32"
...
static void aw_i2c_err_report(struct aw882xx *aw882xx, int err_type)
{
	...
	if (aw882xx->aw_pa == NULL) {
		aw_dev_err(aw882xx->dev, "aw_pa is NULL");
		return;
	}
	...
	unsigned char reg_addr, unsigned int mask, unsigned int reg_data)
{
	...
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "i2c write error, ret=%d", ret);
		return ret;
	}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	aw882xx->aw_pa->dv_control_flag = 1;
	if (aw882xx->pstream) {
		vol_desc->ctl_volume = value;
		aw882xx->aw_pa->ops.aw_set_volume(aw882xx->aw_pa, vol_desc->ctl_volume);
	}
	aw882xx->aw_pa->dv_control_volume_set = value;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aw882xx->pstream)
		aw_dev_mute(aw882xx->aw_pa, aw882xx->aw_pa->mute_flag);
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx)
{
	...
	aw882xx_dev_control = devm_kzalloc(aw882xx->codec->dev, sizeof(struct snd_kcontrol_new) * count, GFP_KERNEL);
	...
	if (!kctl_name)
		return -ENOMEM;
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_hmute", aw882xx->index);

	aw882xx_dev_control[3].name = kctl_name;
	aw882xx_dev_control[3].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw882xx_dev_control[3].info = aw882xx_hmute_info;
	aw882xx_dev_control[3].put = aw882xx_hmute_set;
	aw882xx_dev_control[3].get = aw882xx_hmute_get;

	aw_componet_codec_ops.add_codec_controls(aw882xx->codec, aw882xx_dev_control, count);
...
static ssize_t aw882xx_surface_time_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	...
	if (1 == sscanf(buf, "%d", &databuf[0])) {
		aw_dsp_write_surface_temperature(AW_DEV_CH_PRI_L, databuf[0]);
	}
...
static ssize_t aw882xx_surface_time_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int surface_temperature = 0;

	aw_dsp_read_surface_temperature(AW_DEV_CH_PRI_L, &surface_temperature);
...
static DEVICE_ATTR(surface_temp_0, S_IWUSR | S_IRUGO,
	aw882xx_surface_time_show, aw882xx_surface_time_store);
