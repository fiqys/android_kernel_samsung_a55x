...
static void aw_monitor_set_gain(struct aw_device *aw_dev, uint16_t gain)
{
	...
	/*add offset*/
	set_volume = gain + aw_dev->volume_desc.ctl_volume;
...
static int aw_monitor_abox_sync_temp(struct aw_device *aw_dev)
{
	int ret = -1;
	int temp = 0;
...
static int aw_monitor_abox_sync_voltage(struct aw_device *aw_dev)
{
	int ret = -1;
	int volt = 0;

	ret = aw_monitor_get_data_form_system(aw_dev, &volt, POWER_SUPPLY_PROP_VOLTAGE_NOW);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get voltage from system failed!");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "get voltage from system %d!", volt);

	ret = aw_dsp_write_battery_voltage(aw_dev, volt);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "write battery voltage failed");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "set voltage done!");	
...
{
	aw_monitor_abox_sync_temp(aw_dev);
	aw_monitor_abox_sync_voltage(aw_dev);
