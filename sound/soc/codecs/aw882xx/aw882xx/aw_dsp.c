...
int aw_dsp_write_battery_voltage(struct aw_device *aw_dev, int volt)
{
	uint32_t msg_id;
	int ret;
	int voltage = 0;
	uint32_t data_len = sizeof(voltage);

	msg_id = AW_MSG_ID_BAT_VOLTAGE;
	voltage = volt;

	ret = aw_dsp_write_dsp_msg(aw_dev, msg_id, (char *)&voltage, data_len);
	if (ret) {
		aw_dev_err(aw_dev->dev, "write battery voltage failed ");
		return ret;
	}
	aw_dev_dbg(aw_dev->dev, "write battery voltage [%d] done", voltage);
	return 0;	
}
